...
# Register pytest assert rewriting on all submodules of `gatecoin/tests/utils`.
# This is necessary due to our split fixture setup since pytest doesn't detect these
# imports automatically.
for module_info in pkgutil.iter_modules(["gatecoin/tests/utils"]):
    pytest.register_assert_rewrite(f"gatecoin.tests.utils.{module_info.name}")
...
from gatecoin.network.transport.matrix.rtc.aiogevent import yield_future
from gatecoin.network.transport.matrix.rtc.utils import (
    ASYNCIO_LOOP_RUNNING_TIMEOUT,
    setup_asyncio_event_loop,
)
from gatecoin.tests.fixtures.blockchain import *  # noqa: F401,F403
from gatecoin.tests.fixtures.variables import *  # noqa: F401,F403
from gatecoin.tests.integration.exception import RetryTestError
from gatecoin.tests.utils.transport import make_requests_insecure
from gatecoin.utils.cli import LogLevelConfigType
from gatecoin.utils.debugging import enable_monitoring_signal
...
def logging_level(request, logs_storage):
    ...
    debug_path = os.path.join(logs_storage, f"gatecoin-debug_{time}.log")
...
def dont_exit_pytest():
    """Gatecoin will quit on any unhandled exception.
