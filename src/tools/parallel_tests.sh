...
logs=${tmpdir}/logs
session_prefixes="${output}/gatecoin-tests"
base_port=10500
...
if (type tmux &> /dev/null); then
    ...
    for f in "${session_prefixes}"*; do
        ...
        logfile=$(basename "${f}")
        args+=("${python_exe} -m pytest gatecoin/tests --select-from-file ${f} -v --color yes --base-port $(( base_port + ( idx * 500) )) | tee -a ${logs}/${logfile}; echo 'Ctrl-C to exit'; read")
        args+=(";")
    ...
    for f in "${session_prefixes}"*; do
        logfile=$(basename "${f}")
        pytest gatecoin/tests --select-from-file "${f}" -v --base-port $(( base_port + ( idx * 1000) )) > "${logs}/${logfile}" &
        idx=$(( idx + 1 ))
    done
