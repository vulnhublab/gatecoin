...
from gatecoin.transfer.state import NetworkState
from gatecoin.utils.formatting import pex
from gatecoin.utils.nursery import Janitor, Nursery
from gatecoin.utils.typing import Address, Host, Port, TokenAmount
...
@dataclass
class RunningNode:
    """A running node, this has a Gatecoin instance running in the background
...
def wait_for_reachable(
    ...
    while channels_not_reachable:
        ...
        for url in channels_not_reachable.copy():
            ...
            # condition in the Gatecoin client REST API.
...
def start_and_wait_for_server(
    ...
    """Start the Gatecoin node and waits for the REST API to be available,
...
def transfer_and_assert_successful(
    ...
    # TODO: Add an UUID to the transfer, change Gatecoin to log the UUID and for
...
def run_stress_test(
    ...
    for iteration in config.iteration_counter:
        ...
        for concurent_paths, concurrency, transfer_planner, scheduler in zip(
            ...
            # reset, otherwise there is a bug in the planner or Gatecoin.
...
def main() -> None:
    ...
    for section in config:
        if NODE_SECTION_RE.match(section):
            ...
            gatecoin_args.extend(chain.from_iterable(node.items()))
            ...
            nodes_config.append(NodeConfig(gatecoin_args, interface, address, nodedir))
...
if __name__ == "__main__":
    # TODO:
    # - The script should quit if the vpn is closed (and therefore the gatecoin
    # process is killed)
