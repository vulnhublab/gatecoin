...
SCENARIO_REMOTE_URL_1="root@scenario-player.ci.gatecoin.network"
SCENARIO_REMOTE_URL_2="root@scenario-player2.ci.gatecoin.network"
...
function download_service_logs {
    ...
    for file in "$@"; do
        if [[ ! -e "${DESTINATION_DIR}/${file}" ]]; then
            sources+=("root@services-dev.gatecoin.network:/home/services/${file}")
        fi
    done
