...
# This script will read a `gatecoin-debug*.log` file and fiter the logs by the
...
usage() {
    echo "$0 <gatecoin-debug-log> <output-dir>"
    exit 1
}
...
process_node_log() {
    ...
    jq -c "select((.state_changes | length) > 0 or (.gatecoin_events | length) > 0)" "${full_log}" > "${state_machine_log}"
