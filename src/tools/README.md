...
- `startcluster.py`: This tool will generate a genesis file with a few
  pre-funded accounts to be used as Gatecoin accounts, and then run a local
...
are endpoints of running Gatecoin nodes.
...
The key `nodes` gives the local endpoint of *running* Gatecoin nodes, which the
