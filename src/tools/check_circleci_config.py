...
def _check_workflows_align(config):
    """
    Ensure that the common shared jobs in the `gatecoin-default` and `nightly` workflows are
    identical.
    """

    jobs_default = config["workflows"]["gatecoin-default"]["jobs"]
    ...
    message = [
        _yellow(
            "Mismatch in common items of workflow definitions 'gatecoin-default' and 'nightly':\n"
        )
    ]
