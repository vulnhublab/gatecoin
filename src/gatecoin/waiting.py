...
from gatecoin.storage.restore import get_state_change_with_transfer_by_secrethash
from gatecoin.transfer import channel, views
from gatecoin.transfer.events import EventPaymentReceivedSuccess
from gatecoin.transfer.identifiers import CanonicalIdentifier
from gatecoin.transfer.mediated_transfer.events import EventUnlockClaimFailed
...
if TYPE_CHECKING:
    from gatecoin.gatecoin_service import GatecoinService  # pylint: disable=unused-import
...
class ChannelExpiredCoopSettle(ChannelStateCondition):
    ...
    def evaluate(
        ...
        if channel_state is not None:
            ...
            return self.gatecoin.get_block_number() > coop_settle.expiration
        return False
...
class ChannelCoopSettleSuccess(ChannelStateCondition):
    ...
    def __post_init__(self) -> None:
        assert self.gatecoin.wal is not None, "must be set"
        self._stream = self.gatecoin.wal.storage.get_state_changes_stream(0.1)
...
class ChannelStateWaiter:
    ...
    def wait(self, condition: ChannelStateCondition) -> None:
        ...
        while not condition(chain_state, self._get_channel_state(chain_state)):
            assert self.gatecoin.is_running(), ALARM_TASK_ERROR_MSG
            assert self.gatecoin.alarm.is_running(), ALARM_TASK_ERROR_MSG
            log.debug(
                "Waiting on channel",
                node=to_checksum_address(self.gatecoin.address),
...
def wait_for_block(
    gatecoin: "GatecoinService", block_number: BlockNumber, retry_timeout: float
) -> None:  # pragma: no unittest
    current = gatecoin.get_block_number()
    ...
    while current < block_number:
        assert gatecoin, ALARM_TASK_ERROR_MSG
        assert gatecoin.alarm, ALARM_TASK_ERROR_MSG
...
def wait_for_newchannel(
    ...
    """
    ChannelStateWaiter(
        gatecoin, retry_timeout, token_network_registry_address, token_address, partner_address
    ).wait(ChannelExists())
...
def wait_single_channel_deposit(
    app_deposit: "GatecoinService",
    app_partner: "GatecoinService",
...
def wait_for_channel_in_states(
    ...
    for channel_id in channel_ids:
        canonical_id = _get_canonical_identifier_by_channel_id(
            gatecoin, token_network_registry_address, token_address, channel_id
        )
...
def wait_for_token_network(
    ...
    """Wait until the token network is visible to the GatecoinService.

    Note:
        This does not time out, use gevent.Timeout.
    """
    token_network = views.get_token_network_by_token_address(
        views.state_from_gatecoin(gatecoin), token_network_registry_address, token_address
    )
    log_details = {
...
def wait_for_received_transfer_result(
    ...
    assert gatecoin, TRANSPORT_ERROR_MSG
    assert gatecoin.wal, TRANSPORT_ERROR_MSG
    assert gatecoin.transport, TRANSPORT_ERROR_MSG
    stream = gatecoin.wal.storage.get_state_changes_stream(retry_timeout=retry_timeout)

    result = None
    while result is None:

        state_events = gatecoin.wal.storage.get_events()
        ...
        for state_change in state_changes:
            ...
            if registered_onchain:
                state_change_record = get_state_change_with_transfer_by_secrethash(
                    gatecoin.wal.storage, secrethash
                )
                ...
                if (
                    ...
                    and gatecoin.get_block_number() <= transfer.lock.expiration
                ):
