...
from gatecoin import routing
from gatecoin.api.objects import Notification
from gatecoin.api.python import GatecoinAPI
from gatecoin.api.rest import APIServer, RestAPI
from gatecoin.blockchain.decode import blockchainevent_to_statechange
from gatecoin.blockchain.events import BlockchainEvents, DecodedEvent
from gatecoin.blockchain.filters import GatecoinContractFilter
...
from gatecoin.exceptions import (
    ...
    PaymentConflict,
    GatecoinRecoverableError,
    GatecoinUnrecoverableError,
    SerializationError,
)
from gatecoin.message_handler import MessageHandler
from gatecoin.messages.abstract import Message, SignedMessage
from gatecoin.messages.encode import message_from_sendevent
from gatecoin.network.pathfinding import PFSProxy
from gatecoin.network.proxies.proxy_manager import ProxyManager
from gatecoin.network.proxies.secret_registry import SecretRegistry
from gatecoin.network.proxies.service_registry import ServiceRegistry
from gatecoin.network.proxies.token_network_registry import TokenNetworkRegistry
from gatecoin.network.proxies.user_deposit import UserDeposit
from gatecoin.network.rpc.client import JSONRPCClient
from gatecoin.network.transport import populate_services_addresses
from gatecoin.network.transport.matrix.transport import MatrixTransport, MessagesQueue
from gatecoin.gatecoin_event_handler import EventHandler
from gatecoin.services import send_pfs_update, update_monitoring_service_from_balance_proof
from gatecoin.settings import GatecoinConfig
from gatecoin.storage import sqlite, wal
from gatecoin.storage.serialization import DictSerializer, JSONSerializer
from gatecoin.storage.sqlite import HIGH_STATECHANGE_ULID, Range
from gatecoin.storage.wal import WriteAheadLog
from gatecoin.tasks import AlarmTask
from gatecoin.transfer import node, views
from gatecoin.transfer.architecture import (
    BalanceProofSignedState,
    ContractSendEvent,
    Event as GatecoinEvent,
    StateChange,
)
from gatecoin.transfer.channel import get_capacity
...
from gatecoin.transfer.mediated_transfer.mediation_fee import (
    FeeScheduleState,
    calculate_imbalance_fees,
)
from gatecoin.transfer.mediated_transfer.state import TransferDescriptionWithSecretState
from gatecoin.transfer.mediated_transfer.state_change import (
    ActionInitInitiator,
    ReceiveLockExpired,
    ReceiveTransferCancelRoute,
    ReceiveTransferRefund,
)
from gatecoin.transfer.mediated_transfer.tasks import InitiatorTask
from gatecoin.transfer.state import ChainState, RouteState, TokenNetworkRegistryState
from gatecoin.transfer.state_change import (
...
from gatecoin.ui.startup import GatecoinBundle, ServicesBundle
from gatecoin.utils.formatting import lpex, to_checksum_address
from gatecoin.utils.gevent import spawn_named
from gatecoin.utils.logging import redact_secret
from gatecoin.utils.runnable import Runnable
from gatecoin.utils.secrethash import sha256_secrethash
from gatecoin.utils.signer import LocalSigner, Signer
from gatecoin.utils.transfers import random_secret
...
from gatecoin.utils.upgrades import UpgradeManager
from gatecoin_contracts.constants import ChannelEvent
from gatecoin_contracts.contract_manager import ContractManager
...
# https://gatecoin-specification.readthedocs.io/en/latest/pathfinding_service.html
...
def initiator_init(
    ...
    transfer_state = TransferDescriptionWithSecretState(
        ...
        initiator=InitiatorAddress(gatecoin.address),
    ...
    if route_states is None:
        our_address_metadata = gatecoin.transport.address_metadata

        msg = "Transport is not initialized with gatecoin-service"
        assert our_address_metadata is not None, msg

        error_msg, route_states, feedback_token = routing.get_best_routes(
            chain_state=views.state_from_gatecoin(gatecoin),
            token_network_address=token_network_address,
            one_to_n_address=gatecoin.default_one_to_n_address,
            from_address=InitiatorAddress(gatecoin.address),
            to_address=target_address,
            amount=transfer_amount,
            previous_address=None,
            privkey=gatecoin.privkey,
            our_address_metadata=our_address_metadata,
            pfs_proxy=gatecoin.pfs_proxy,
        )

        # Only prepare feedback when token is available
        if feedback_token is not None:
            for route_state in route_states:
                gatecoin.route_to_feedback_token[tuple(route_state.route)] = feedback_token
...
class PaymentStatus(NamedTuple):
    """Value type for GatecoinService.targets_to_identifiers_to_statuses.
...
class GatecoinService(Runnable):
    """A Gatecoin node."""
    ...
    def __init__(
        ...
        gatecoin_event_handler: EventHandler,
        ...
        settlement_timeout_min = gatecoin_bundle.token_network_registry.settlement_timeout_min(
            BLOCK_ID_LATEST
        )
        settlement_timeout_max = gatecoin_bundle.token_network_registry.settlement_timeout_max(
            BLOCK_ID_LATEST
        )
        ...
        if invalid_settle_timeout:
            contract = to_checksum_address(gatecoin_bundle.token_network_registry.address)
        ...
        self.default_registry: TokenNetworkRegistry = gatecoin_bundle.token_network_registry
        ...
        self.default_secret_registry: SecretRegistry = gatecoin_bundle.secret_registry
        ...
        self.config: GatecoinConfig = config
        ...
        if self.config.database_path != ":memory:":
            ...
            # Two gatecoin processes must not write to the same database. Even
        ...
        # Flag used to skip the processing of all Gatecoin events during the
        # startup.
        #
    ...
    def start(self) -> None:
        ...
        chain_state = views.state_from_gatecoin(self)
        ...
        log.debug("Gatecoin Service started", node=to_checksum_address(self.address))
        super().start()
    ...
    def _run(self, *args: Any, **kwargs: Any) -> None:  # pylint: disable=method-hidden
        ...
        self.greenlet.name = f"GatecoinService._run node:{to_checksum_address(self.address)}"
        try:
    ...
    def stop(self) -> None:
        ...
        log.debug("Gatecoin Service stopped", node=to_checksum_address(self.address))
    ...
    def _start_transport(self) -> None:
        ...
        self.transport.start(gatecoin_service=self, prev_auth_data=None)

    def _make_initial_state(self) -> ChainState:
        # On first run Gatecoin needs to fetch all events for the payment
        ...
        # Gatecoin has to poll all events until the last confirmed block,
        ...
        initial_state.identifiers_to_tokennetworkregistries[token_network_registry_address] = (
    ...
    def _initialize_wal(self) -> None:
        if self.database_dir is not None:
            try:
                ...
                raise GatecoinUnrecoverableError(
                    "Could not aquire database lock. Maybe a Gatecoin node for this account "
        ...
        try:
            ...
            raise GatecoinUnrecoverableError(
                ...
                "the current version of Gatecoin. Consider using a stable "
                "version of the Gatecoin client."
            )

        if state_change_qty_snapshot == 0:
            print(
                "This is the first time Gatecoin is being used with this address. "
        ...
        known_networks = views.get_token_network_registry_address(views.state_from_gatecoin(self))
        if known_networks and self.default_registry.address not in known_networks:
            ...
            raise RuntimeError(
                ...
                f"Gatecoin is configured to use the smart contract "
    ...
    def handle_state_changes(self, state_changes: List[StateChange]) -> List[Greenlet]:
        ...
        with self.wal.process_state_change_atomically() as dispatcher:
            for state_change in state_changes:
                ...
                gatecoin_events.extend(events)

        return self._trigger_state_change_effects(
            new_state=views.state_from_gatecoin(self),
            state_changes=state_changes,
            events=gatecoin_events,
        )
    ...
    def _trigger_state_change_effects(
        ...
        for canonical_identifier in pfs_capacity_updates:
            send_pfs_update(gatecoin=self, canonical_identifier=canonical_identifier)

        for canonical_identifier in pfs_fee_updates:
            send_pfs_update(
                gatecoin=self, canonical_identifier=canonical_identifier, update_fee_schedule=True
            )

        log.debug(
            "Gatecoin events",
            node=to_checksum_address(self.address),
            gatecoin_events=[redact_secret(DictSerializer.serialize(event)) for event in events],
        )
        ...
        if self.ready_to_process_events:
            return self.async_handle_events(chain_state=new_state, gatecoin_events=events)
        else:
            return []
    ...
    def async_handle_events(
        self, chain_state: ChainState, gatecoin_events: List[GatecoinEvent]
    ) -> List[Greenlet]:
        """Spawn a new thread to handle a Gatecoin event.
        ...
        event_wrapper = EventWrapper(gatecoin_events)
    ...
    def _handle_events(
        ...
        try:
            self.gatecoin_event_handler.on_gatecoin_events(
                gatecoin=self, chain_state=chain_state, events=gatecoin_events
            )
        except GatecoinRecoverableError as e:
            log.info(str(e))
        except InvalidDBData:
            raise
        except (GatecoinUnrecoverableError, BrokenPreconditionError) as e:
            log_unrecoverable = (
    ...
    def _best_effort_synchronize_with_confirmed_head(
        ...
        while guard.should_continue(self.blockchain_events.last_fetched_block):
            ...
            assert self.wal, "gatecoin.wal not set"
            with self.wal.process_state_change_atomically() as dispatcher:
                for event in poll_result.events:
                    ...
                    maybe_state_change = blockchainevent_to_statechange(
                        gatecoin_config=self.config,
                        gatecoin_storage=self.wal.storage,  # FXIME: use more recent
    ...
    def _initialize_transactions_queues(self, chain_state: ChainState) -> None:
        ...
        pending_transactions = cast(
            List[GatecoinEvent], views.get_pending_transactions(chain_state)
        )
        ...
        transaction_greenlets = self.async_handle_events(
            chain_state=chain_state, gatecoin_events=pending_transactions
        )
    ...
    def mediated_transfer_async(
        ...
        # details. This prevents Gatecoin from having concurrently identical
    ...
    def maybe_upgrade_db(self) -> None:
        manager = UpgradeManager(
            db_filename=self.config.database_path, gatecoin=self, web3=self.rpc_client.web3
        )
        manager.run()
