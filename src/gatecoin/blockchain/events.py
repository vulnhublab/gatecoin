...
from gatecoin.blockchain.exceptions import EthGetLogsTimeout, UnknownGatecoinEventType
from gatecoin.blockchain.filters import (
    GatecoinContractFilter,
    decode_event,
    get_filter_args_for_all_events_from_channel,
)
from gatecoin.blockchain.utils import BlockBatchSizeAdjuster
...
from gatecoin.exceptions import InvalidBlockNumberInput
from gatecoin.network.proxies.proxy_manager import ProxyManager
from gatecoin.settings import BlockBatchSizeConfig
...
class DecodedEvent:
    """A confirmed event with the data decoded to conform with Gatecoin's internals.

    Gatecoin prefers bytes for addresses and hashes, and it uses snake_case as a
...
def decode_gatecoin_event_to_internal(
    ...
    if not decoded_event:
        raise UnknownGatecoinEventType()
    ...
    data["args"] = args
    # translate from web3's to gatecoin's name convention
...
def new_filters_from_events(events: List[DecodedEvent]) -> GatecoinContractFilter:
    new_filter = GatecoinContractFilter(
...
class BlockchainEvents:
    def __init__(
        ...
        event_filter: GatecoinContractFilter,
    ...
    def fetch_logs_in_batch(self, target_block_number: BlockNumber) -> Optional[PollResult]:
        ...
        with self._filters_lock:
            ...
            # reducing trafic from Gatecoin to the Ethereum client, this is
    ...
    def _query_and_track(
        ...
        *all* filters will start from 9, thus missing the event for the new
        ...
        event_filter: Optional[GatecoinContractFilter] = self.event_filter
        ...
        while event_filter:
            ...
            if blockchain_events:
                ...
                decoded_events = [
                    decode_gatecoin_event_to_internal(
