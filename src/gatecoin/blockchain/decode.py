...
itself, an utility should be added to `gatecoin.blockchain.state`, and then
...
from gatecoin.blockchain.events import DecodedEvent
from gatecoin.blockchain.state import (
    ChannelSettleState,
    NewChannelDetails,
...
from gatecoin.constants import EMPTY_LOCKSROOT, LOCKSROOT_OF_NO_LOCKS
from gatecoin.settings import MediationFeeConfig, GatecoinConfig
...
def blockchainevent_to_statechange(
    gatecoin_config: GatecoinConfig,
    gatecoin_storage: SerializedSQLiteStorage,
    ...
    if event_name == EVENT_TOKEN_NETWORK_CREATED:
        ...
        if new_channel_details is not None:
            fee_config = gatecoin_config.mediation_fees
            channel_config = ChannelConfig(
                reveal_timeout=gatecoin_config.reveal_timeout,
                fee_schedule=FeeScheduleState(
                    ...
                    proportional=fee_config.get_proportional_fee(
                        new_channel_details.token_address
                    ),
        ...
        return contractreceivechanneldeposit_from_event(event, gatecoin_config.mediation_fees)

    elif event_name == ChannelEvent.WITHDRAW:
        return contractreceivechannelwithdraw_from_event(event, gatecoin_config.mediation_fees)
        ...
        if channel_settle_state:
            ...
            log.debug("Discarding settle event, we're not part of it", gatecoin_event=event)
        ...
        canonical_identifier = get_contractreceivechannelbatchunlock_data_from_event(
            chain_state, gatecoin_storage, event
        )
        ...
        log.error("Unknown event type", gatecoin_event=event)
