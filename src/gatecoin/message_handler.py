...
from gatecoin.constants import ABSENT_SECRET, BLOCK_ID_LATEST
from gatecoin.exceptions import InvalidSecret, ServiceRequestFailed
from gatecoin.messages.abstract import Message
from gatecoin.messages.decode import balanceproof_from_envelope, lockedtransfersigned_from_message
...
from gatecoin.transfer.architecture import StateChange
...
from gatecoin.transfer.state import HopState
...
from gatecoin.transfer.utils.secret import decrypt_secret
from gatecoin.transfer.views import TransferRole
...
class MessageHandler:
    def on_messages(self, gatecoin: "GatecoinService", messages: List[Message]) -> None:
        ...
        for message in unique_messages:
            ...
            if t_message not in handler_map:
                ...
                continue
            pool.apply_async(handler_map[t_message], (gatecoin, message))
        ...
        if all_state_changes:
            ...
            gatecoin.handle_and_track_state_changes(all_state_changes)

    @staticmethod
    def handle_message_withdrawrequest(
        gatecoin: "GatecoinService", message: WithdrawRequest  # pylint: disable=unused-argument
    ) -> List[StateChange]:
    ...
    def handle_message_withdraw_confirmation(
        ...
        message: WithdrawConfirmation,  # pylint: disable=unused-argument
    ) -> List[StateChange]:
        assert message.sender, "message must be signed"
    ...
    def handle_message_withdraw_expired(
        gatecoin: "GatecoinService", message: WithdrawExpired  # pylint: disable=unused-argument
    ) -> List[StateChange]:
        assert message.sender, "message must be signed"
    ...
    def handle_message_secretrequest(
        gatecoin: "GatecoinService", message: SecretRequest  # pylint: disable=unused-argument
    ) -> List[StateChange]:
        assert message.sender, "message must be signed"
    ...
    def handle_message_revealsecret(
        gatecoin: "GatecoinService", message: RevealSecret  # pylint: disable=unused-argument
    ) -> List[StateChange]:
        assert message.sender, "message must be signed"
    ...
    def handle_message_unlock(
        gatecoin: "GatecoinService", message: Unlock  # pylint: disable=unused-argument
    ) -> List[StateChange]:
    ...
    def handle_message_lockexpired(
        gatecoin: "GatecoinService", message: LockExpired  # pylint: disable=unused-argument
    ) -> List[StateChange]:
        balance_proof = balanceproof_from_envelope(message)
    ...
    def handle_message_refundtransfer(
        gatecoin: "GatecoinService", message: RefundTransfer
    ) -> List[StateChange]:
    ...
    def handle_message_lockedtransfer(
        gatecoin: "GatecoinService", message: LockedTransfer  # pylint: disable=unused-argument
    ) -> List[StateChange]:
        ...
        registered = gatecoin.default_secret_registry.is_secret_registered(
        ...
        if message.target == TargetAddress(gatecoin.address):
            ...
            for route_state in from_transfer.route_states:
                next_hop_address = route_state.hop_after(gatecoin.address)
    ...
    def handle_message_processed(
        gatecoin: "GatecoinService", message: Processed  # pylint: disable=unused-argument
    ) -> List[StateChange]:
        assert message.sender, "message must be signed"
    ...
    def handle_message_delivered(
        gatecoin: "GatecoinService", message: Delivered  # pylint: disable=unused-argument
    ) -> List[StateChange]:
        assert message.sender, "message must be signed"
