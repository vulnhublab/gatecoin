...
from gatecoin.utils.typing import Secret
...
def reveal_secret_with_resolver(
    gatecoin: "GatecoinService", chain_state: ChainState, secret_request_event: SendSecretRequest
) -> bool:

    resolver_endpoint = gatecoin.config.resolver_endpoint
    if not resolver_endpoint:
        return False

    log.debug("Using resolver to fetch secret", resolver_endpoint=resolver_endpoint)

    assert isinstance(gatecoin.wal, WriteAheadLog), "GatecoinService has not been started"
    current_state = gatecoin.wal.get_current_state()
    ...
    request = {
        ...
        "payment_recipient": to_hex(gatecoin.address),
        "chain_id": chain_state.chain_id,
    }

    # loop until we get a valid response from the resolver or until timeout
    while True:
        current_state = views.state_from_gatecoin(gatecoin)
    ...
    gatecoin.handle_and_track_state_changes([state_change])
    return True
