...
from gatecoin.network.proxies.metadata import SmartContractMetadata
from gatecoin.network.proxies.utils import raise_on_call_returned_empty
from gatecoin.network.rpc.client import (
    JSONRPCClient,
...
from gatecoin.utils.formatting import format_block_id, to_checksum_address
from gatecoin.utils.packing import pack_balance_proof, pack_signed_balance_proof, pack_withdraw
from gatecoin.utils.signer import recover
from gatecoin.utils.smart_contracts import safe_gas_limit
...
class TokenNetwork:
    ...
    def _new_netting_channel(
        ...
        if estimated_transaction is None:
            ...
            if self.safety_deprecation_switch(block_identifier=failed_at_blockhash):
                raise GatecoinRecoverableError("This token network is deprecated.")

            raise GatecoinRecoverableError(
            ...
            if not was_transaction_successfully_mined(transaction_mined):
                ...
                raise GatecoinRecoverableError("Creating new channel failed.")
    ...
    def get_channel_identifier(
        ...
        Raises:
            GatecoinRecoverableError: If there is not open channel among
        ...
        if channel_identifier == 0:
            ...
            raise GatecoinRecoverableError(msg)
    ...
    def approve_and_set_total_deposit(
        ...
        Raises:
            ...
            GatecoinRecoverableError: If the channel was closed meanwhile the
                deposit was in transit.
            GatecoinUnrecoverableError: If the transaction was successful and the
    ...
    def _approve_and_set_total_deposit(
        ...
        if estimated_transaction is not None:
            ...
            if not was_transaction_successfully_mined(transaction_mined):
                ...
                # This must not crash the client, because it is not a Gatecoin bug,
                # and otherwise this could be an attack vector.
                total_channel_deposit = total_deposit + partner_details.deposit
                if total_channel_deposit > UINT256_MAX:
                    raise GatecoinRecoverableError("Deposit overflow")

                total_deposit_done = our_details.deposit >= total_deposit
                if total_deposit_done:
                    raise GatecoinRecoverableError("Requested total deposit was already performed")
                ...
                if latest_deposit < total_deposit:
                    raise GatecoinRecoverableError("The tokens were not transferred")
                ...
                raise GatecoinRecoverableError("Unlocked failed for an unknown reason")
            else:
            ...
            if is_invalid_channel_id:
                ...
                raise GatecoinUnrecoverableError(msg)  # This error is considered a bug
            ...
            if channel_data.state == ChannelState.NONEXISTENT:
                ...
                raise GatecoinUnrecoverableError(msg)
            ...
            try:
                ...
                try:
                    ...
                    if participant_recovered_address != initiator:
                        raise GatecoinUnrecoverableError("Invalid withdraw initiator signature")
                ...
                try:
                    ...
                    if partner_recovered_address != partner:
                        raise GatecoinUnrecoverableError("Invalid withdraw partner signature")
    ...
    def _set_total_withdraw(
        ...
        if estimated_transaction is not None:
            ...
            if not was_transaction_successfully_mined(transaction_mined):
                ...
                raise GatecoinUnrecoverableError("SetTotalwithdraw failed for an unknown reason")
            else:
                return transaction_mined.transaction_hash
        else:
            ...
            total_withdraw_done = our_details.withdrawn >= total_withdraw
            if total_withdraw_done:
                raise GatecoinRecoverableError("Requested total withdraw was already performed")
    ...
    def coop_settle(
        ...
        if estimated_transaction is None:
            raise GatecoinRecoverableError("CoopSettle: gas-estimation of transaction failed.")
    ...
    def close(
        ...
        Raises:
            GatecoinRecoverableError: If the close call failed but it is not
                critical.
            GatecoinUnrecoverableError: If the operation was illegal at the
        ...
        try:
            ...
            raise GatecoinUnrecoverableError("Couldn't verify the closing signature")
        else:
            if our_recovered_address != self.node_address:
                raise GatecoinUnrecoverableError("Invalid closing signature")
        ...
        if non_closing_signature != EMPTY_SIGNATURE:
            ...
            try:
                ...
                # InvalidSignature is raised by gatecoin.utils.signer.recover if signature
                ...
                raise GatecoinUnrecoverableError("Couldn't verify the non-closing signature")
            else:
                if partner_recovered_address != partner:
                    raise GatecoinUnrecoverableError("Invalid non-closing signature")
        ...
        with self.channel_operations_lock[partner]:
            ...
            if estimated_transaction is not None:
                ...
                if not was_transaction_successfully_mined(transaction_mined):
                    ...
                    raise GatecoinUnrecoverableError("closeChannel call failed")
                else:
                    return transaction_mined.transaction_hash
    ...
    def update_transfer(
        ...
        if balance_hash is EMPTY_BALANCE_HASH:
            raise GatecoinUnrecoverableError("update_transfer called with an empty balance_hash")

        if nonce <= 0 or nonce > UINT256_MAX:
            raise GatecoinUnrecoverableError("update_transfer called with an invalid nonce")
        ...
        try:
            ...
            raise GatecoinUnrecoverableError("Couldn't verify the balance proof signature")
        else:
            if our_recovered_address != self.node_address:
                raise GatecoinUnrecoverableError("Invalid balance proof signature")

            if partner_recovered_address != partner:
                raise GatecoinUnrecoverableError("Invalid update transfer signature")
    ...
    def _update_transfer(
        ...
        if estimated_transaction is not None:
            ...
            if not was_transaction_successfully_mined(transaction_mined):
                ...
                raise GatecoinUnrecoverableError("update transfer failed for an unknown reason")
            else:
                return transaction_mined.transaction_hash
    ...
    def _unlock(
        ...
        if estimated_transaction is not None:
            ...
            if not was_transaction_successfully_mined(transaction_mined):
                ...
                if is_unlock_done:
                    raise GatecoinRecoverableError("The locks are already unlocked")
        ...
        if estimated_transaction is not None:
            ...
            if not was_transaction_successfully_mined(transaction_mined):
                ...
                if channel_onchain_detail.state in (ChannelState.SETTLED, ChannelState.REMOVED):
                    raise GatecoinRecoverableError("Channel is already settled")

                if channel_onchain_detail.state == ChannelState.OPENED:
                    raise GatecoinUnrecoverableError("Channel is still open. It cannot be settled")
                ...
                raise GatecoinRecoverableError("Settle failed for an unknown reason")
            else:
                return transaction_mined.transaction_hash
        else:
            # The latest block can not be used reliably because of reorgs,
