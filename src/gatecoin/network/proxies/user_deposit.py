...
from gatecoin.constants import BLOCK_ID_LATEST, BLOCK_ID_PENDING, EMPTY_ADDRESS, UINT256_MAX
from gatecoin.exceptions import BrokenPreconditionError, GatecoinRecoverableError
...
from gatecoin_contracts.contract_manager import ContractManager, gas_measurements
...
class UserDeposit:
    ...
    def _init(
        ...
        if estimated_transaction is None:
            ...
            raise GatecoinRecoverableError("Deposit failed of unknown reason")
            ...
            if not was_transaction_successfully_mined(transaction_mined):
                ...
                if token.balance_of(self.node_address, failed_at_blocknumber) < amount_to_deposit:
                    msg = (
                        ...
                        "This could be a bug in Gatecoin or a mallicious "
                        "ERC20 Token."
                    )
                    raise GatecoinRecoverableError(msg)
    ...
    def _plan_withdraw_check_result(
        ...
        if not was_transaction_successfully_mined(transaction_mined):
            ...
            raise GatecoinRecoverableError("Plan withdraw failed for an unknown reason.")
    ...
    def _withdraw_check_result(
        ...
        if not was_transaction_successfully_mined(transaction_mined):
            ...
            if current_token_balance != previous_token_balance + amount_to_withdraw:
                raise GatecoinRecoverableError("Token transfer during withdraw failed.")

            raise GatecoinRecoverableError("Withdraw failed for an unknown reason.")
        else:
            return transaction_mined.transaction_hash
