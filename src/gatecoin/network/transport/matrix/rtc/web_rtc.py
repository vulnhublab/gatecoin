...
from gatecoin.network.transport.matrix.client import ReceivedGatecoinMessage
from gatecoin.network.transport.matrix.rtc.aiogevent import wrap_greenlet, yield_future
...
from gatecoin.utils.typing import Address, Any, Callable, Coroutine, Dict, List, Optional, Union
...
class WebRTCManager(Runnable):
    def __init__(
        self,
        node_address: Address,
        process_messages: Callable[[List[ReceivedGatecoinMessage]], None],
        signaling_send: Callable[[Address, str], None],
        stop_event: GEvent,
    ) -> None:
        super().__init__()
    ...
    def _handle_message(self, message_data: str, partner_address: Address) -> None:
        messages: List[ReceivedGatecoinMessage] = []
        for msg in validate_and_parse_message(message_data, partner_address):
            messages.append(ReceivedGatecoinMessage(message=msg, sender=partner_address))
