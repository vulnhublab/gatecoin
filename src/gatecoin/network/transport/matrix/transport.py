...
from gatecoin.exceptions import GatecoinUnrecoverableError, TransportError
from gatecoin.messages.abstract import Message, RetrieableMessage, SignedRetrieableMessage
from gatecoin.messages.healthcheck import Ping, Pong
from gatecoin.messages.synchronization import Delivered, Processed
from gatecoin.network.pathfinding import PFSProxy
from gatecoin.network.proxies.service_registry import ServiceRegistry
from gatecoin.network.transport.matrix.client import (
    GMatrixClient,
    MatrixMessage,
    ReceivedCallMessage,
    ReceivedGatecoinMessage,
)
from gatecoin.network.transport.matrix.rtc.web_rtc import WebRTCManager
from gatecoin.network.transport.matrix.utils import (
    DisplayNameCache,
...
from gatecoin.network.transport.utils import timeout_exponential_backoff
from gatecoin.settings import MatrixTransportConfig
from gatecoin.storage.serialization import DictSerializer
from gatecoin.storage.serialization.serializer import MessageSerializer
from gatecoin.transfer import views
from gatecoin.transfer.identifiers import CANONICAL_IDENTIFIER_UNORDERED_QUEUE, QueueIdentifier
from gatecoin.transfer.state import QueueIdsToQueues
from gatecoin.utils.capabilities import capconfig_to_dict
...
from gatecoin.utils.tracing import matrix_client_enable_requests_tracing
...
class _RetryQueue(Runnable):
    ...
    def _check_and_send(self) -> None:
        ...
        # `GatecoinService._initialize_monitoring_services_queue`
        ...
        def message_is_in_queue(message_data: _RetryQueue._MessageData) -> bool:
            if message_data.queue_identifier not in self.transport._queueids_to_queues:
                # The Gatecoin queue for this queue identifier has been removed
                return False
            return any(
        ...
        for user_id, batch in itertools.groupby(queue_by_user_id, _metadata_key_func):
            ...
            for message_data in message_data_batch:
                ...
                #     removed from the Gatecoin queue
                remove = False
    ...
    def _run(self) -> None:  # type: ignore
        msg = (
            """_RetryQueue started before transport._gatecoin_service is set. """
            """_RetryQueue should not be started before transport.start() is called"""
        )

        assert self.transport._gatecoin_service is not None, msg
        self.greenlet.name = (
            f"RetryQueue "
            f"node:{to_checksum_address(self.transport._gatecoin_service.address)} "
...
class MatrixTransport(Runnable):
    ...
    def __init__(
        ...
        self._gatecoin_service: Optional["GatecoinService"] = None
        ...
        version = get_system_spec()["gatecoin"]
        ...
        # FIXME this will not update the servers while Gatecoin is running,
        #  thus to-device user-id fallback will only try to send to those home-servers that
        #  are fetched during config time
        #  this fixme is about servers joining the federation during runtime of a gatecoin node
    ...
    def __repr__(self) -> str:
        if self._gatecoin_service is not None:
    ...
    def checksummed_address(self) -> Optional[AddressHex]:
        assert self._gatecoin_service is not None, "_gatecoin_service not set"

        address = self._node_address
        if address is None:
            return None
        return to_checksum_address(self._gatecoin_service.address)

    @property
    def _node_address(self) -> Optional[Address]:
        return self._gatecoin_service.address if self._gatecoin_service is not None else None
    ...
    def displayname(self) -> str:
        if self._gatecoin_service is None:
            return ""
        signature_bytes = self._gatecoin_service.signer.sign(str(self.user_id).encode())
    ...
    def start(  # type: ignore
        self, gatecoin_service: "GatecoinService", prev_auth_data: Optional[str]
    ) -> None:
        if not self._stop_event.ready():
            raise RuntimeError(f"{self!r} already started")
        self.log.debug("Matrix starting")
        self._stop_event.clear()
        self._gatecoin_service = gatecoin_service
        assert gatecoin_service.pfs_proxy is not None, "must be set"
        self._web_rtc_manager = WebRTCManager(
            gatecoin_service.address,
            self._process_gatecoin_messages,
        ...
        try:
            ...
            login(
                client=self._client,
                signer=self._gatecoin_service.signer,
        ...
        self.log = log.bind(
            ...
            node=to_checksum_address(self._gatecoin_service.address),
        ...
        chain_state = views.state_from_gatecoin(gatecoin_service)
    ...
    def _queueids_to_queues(self) -> QueueIdsToQueues:
        ...
        chain_state = views.state_from_gatecoin(self._gatecoin_service)
    ...
    def chain_id(self) -> ChainID:
        assert self._gatecoin_service is not None, "_gatecoin_service not set"
        return self._gatecoin_service.rpc_client.chain_id
    ...
    def _validate_matrix_messages(
    ...
    ) -> Tuple[List[ReceivedGatecoinMessage], List[ReceivedCallMessage]]:
        assert self._gatecoin_service is not None, "_gatecoin_service not set"

        gatecoin_messages: List[ReceivedGatecoinMessage] = []
        ...
        for message in messages:
            ...
            if not is_gatecoin_message and not is_call_message:
                continue
            ...
            if self._gatecoin_service.address == address_from_userid(sender_id):
                continue
        ...
        return gatecoin_messages, call_messages

    def _process_gatecoin_messages(self, all_messages: List[ReceivedGatecoinMessage]) -> None:
        assert self._gatecoin_service is not None, "_process_messages must be called after start"
        if self._stop_event.is_set():
            return
        ...
        for received_message in all_messages:
            ...
            if isinstance(message, (Processed, SignedRetrieableMessage)) and message.sender:
                ...
                self._gatecoin_service.sign(delivered_message)
        ...
        self._gatecoin_service.on_messages(incoming_messages)
    ...
    def _handle_messages(self, messages: List[MatrixMessage]) -> bool:
        ...
        gatecoin_messages, call_messages = self._validate_matrix_messages(messages)
        self._process_gatecoin_messages(gatecoin_messages)
        self._process_call_messages(call_messages)
        return len(gatecoin_messages) > 0 or len(call_messages) > 0
    ...
    def _process_call_messages(self, call_messages: List[ReceivedCallMessage]) -> None:
        ...
        In Gatecoin 'm.room.message' events are used as the communication format.
    ...
    def _send_raw(
        ...
        if communication_medium is CommunicationMedium.WEB_RTC:
            ...
            self._send_to_device_raw(
                user_ids=user_ids,
                device_id=DeviceIDs.GATECOIN.value,
    ...
    def _send_signaling_message(self, address: Address, message: str) -> None:
        assert self._gatecoin_service is not None, "must be set"
        metadata = _query_metadata(self._gatecoin_service.pfs_proxy, address)
