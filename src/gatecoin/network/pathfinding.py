...
from gatecoin.messages.metadata import RouteMetadata
from gatecoin.network.proxies.service_registry import ServiceRegistry
from gatecoin.network.utils import get_response_json
from gatecoin.utils.formatting import to_checksum_address
from gatecoin.utils.http import TimeoutHTTPAdapter
...
from gatecoin_contracts.utils.proofs import sign_one_to_n_iou
...
USER_AGENT_STR = (
    (
        "Gatecoin/{gatecoin}/DB:{gatecoin_db_version}/{python_implementation}/"
...
def configure_pfs_or_exit(
    ...
    if pfs_url == MATRIX_AUTO_SELECT_SERVER:
        if service_registry is None:
            raise GatecoinError(
                "Gatecoin was started with routing mode set to PFS, the pathfinding service address "
    ...
    try:
        ...
        raise GatecoinError(
            ...
            f"{pfs_url}. Gatecoin will shut down. Please try a different Pathfinding Service. \n"
    ...
    if pathfinding_service_info.price > 0 and not pathfinding_service_info.payment_address:
        raise GatecoinError(
            ...
            f"Gatecoin will shut down. Please try a different Pathfinding Service."
        )
    ...
    if node_chain_id != pathfinding_service_info.chain_id:
        raise GatecoinError(
            ...
            f"Gatecoin will shut down. Please choose a different Pathfinding Service."
        )
...
def check_pfs_for_production(
    ...
    if service_registry is None:
        raise GatecoinError(
            ...
            "is set. Gatecoin will shut down. Please select a Service Registry."
        )
    ...
    if not (pfs_registered and pfs_url_matches):
        raise GatecoinError(
            ...
            f"{pfs_info.url}. "
            "Gatecoin will shut down. Please select a registered Pathfinding Service."
        )
...
def _query_paths(
    ...
    for retries in reversed(range(MAX_PATHS_QUERY_ATTEMPTS)):
        ...
        # See https://github.com/gatecoin/gatecoin/issues/5647
        with iou_semaphore:
            if offered_fee > 0:
