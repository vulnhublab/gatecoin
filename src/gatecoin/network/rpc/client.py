...
from gatecoin.network.rpc.middleware import block_hash_cache_middleware
...
from gatecoin_contracts.utils.type_aliases import ChainID, T_ChainID
...
def was_transaction_successfully_mined(transaction: "TransactionMined") -> bool:
    ...
    if "status" not in transaction.receipt:
        # This should never happen. Gatecoin checks ethereum client for compatibility at startup
...
def gas_price_for_fast_transaction(web3: Web3) -> int:
    try:
        ...
        # As per https://github.com/gatecoin/gatecoin/issues/3201
...
def make_patched_web3_get_block(
    ...
    See:
      - https://github.com/gatecoin/gatecoin/issues/3201
...
def monkey_patch_web3(web3: Web3, gas_price_strategy: Callable) -> None:
    ...
    # Parity raises a value error. Gatecoin assumes the return of an empty
...
class JSONRPCClient:
    ...
    def can_query_state_for_block(self, block_identifier: BlockIdentifier) -> bool:
        ...
        not be queried.
        More info: https://github.com/gatecoin/gatecoin/issues/3566.
        """
    ...
    def transact(self, transaction: Union[TransactionEstimated, EthTransfer]) -> TransactionSent:
        ...
        try:
            ...
            log.critical(error_msg, **log_details, reason=reason)
            raise GatecoinUnrecoverableError(reason)
    ...
    def deploy_single_contract(
        ...
        # Add a margin of 50%
        # See https://github.com/gatecoin/gatecoin/issues/5994
    ...
    def poll_transaction(self, transaction_sent: TransactionSent) -> TransactionMined:
        ...
        while True:
            ...
            # canonical chain. https://github.com/gatecoin/gatecoin/issues/4529
    ...
    def check_for_insufficient_eth(
        ...
        Note:
            ...
            https://github.com/gatecoin/gatecoin/issues/3890#issuecomment-485857726
        """
