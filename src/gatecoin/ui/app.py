...
from gatecoin.exceptions import ConfigurationError, GatecoinError
...
from gatecoin.gatecoin_event_handler import (
    EventHandler,
    PFSFeedbackEventHandler,
    GatecoinEventHandler,
)
...
from gatecoin.ui.checks import (
...
from gatecoin.ui.prompt import (
    prompt_account,
...
from gatecoin.utils.cli import get_matrix_servers
from gatecoin.utils.formatting import pex, to_checksum_address
from gatecoin.utils.http import split_endpoint
from gatecoin.utils.mediation_fees import prepare_mediation_fee_config
...
from gatecoin_contracts.constants import CONTRACT_TOKEN_NETWORK_REGISTRY, ID_TO_CHAINNAME
from gatecoin_contracts.contract_manager import ContractDevEnvironment, ContractManager
...
def fetch_available_matrix_servers(
    ...
    """
    Fetches the list of known servers from gatecoin/gatecoin-tranport repo
...
def start_api_server(
    ...
    print(
        f"The Gatecoin API RPC server is now running at {url}.\n\n"
        f"See the Gatecoin documentation for all available endpoints at\n"
        f"{DOC_URL}"
    )
...
def setup_gatecoin_config(
    ...
    **kwargs: Any,
    ...
    """
    Initialise the nested gatecoin config objects from "flat" arguments.
...
def run_gatecoin_service(
    ...
    **kwargs: Any,  # FIXME: not used here, but still receives stuff in smoketest
    ...
    if datadir is None:
        datadir = os.path.join(os.path.expanduser("~"), ".gatecoin")
    ...
    # The user has the option to launch Gatecoin with a custom
    ...
    # If a UDC address was not provided, Gatecoin would fall back
    # to using the ones deployed and provided by the gatecoin-contracts package.
    if user_deposit_contract_address is not None:
        if not is_address(user_deposit_contract_address):
            raise GatecoinError("The user deposit address is invalid")
    ...
    check_ethereum_confirmed_block_is_not_pruned(
        ...
        secret_registry=gatecoin_bundle.secret_registry,
    ...
    database_path = Path(
        os.path.join(
            ...
            f"node_{pex(address)}",
            f"netid_{chain_id}",
            f"network_{pex(gatecoin_bundle.token_network_registry.address)}",
            f"v{GATECOIN_DB_VERSION}_log.db",
        )
    )
    config.database_path = database_path

    print(f"Gatecoin is running in {config.environment_type.value.lower()} mode")
    print(
    ...
    event_handler: EventHandler = GatecoinEventHandler()
    ...
    if has_default_fees:
        click.secho(
            ...
            "If you want use Gatecoin with mediation fees - flat, proportional and imbalance fees - "
            "see https://gatecoin.readthedocs.io/en/latest/overview_and_guide.html#run-it",  # noqa: E501
            fg="yellow",
        )
    ...
    gatecoin_service = GatecoinService(
        config=config,
        rpc_client=rpc_client,
        proxy_manager=proxy_manager,
        query_start_block=smart_contracts_start_at,
        gatecoin_bundle=gatecoin_bundle,
        services_bundle=services_bundle,
        transport=matrix_transport,
        gatecoin_event_handler=event_handler,
        message_handler=message_handler,
        routing_mode=routing_mode,
        api_server=api_server,
    )

    gatecoin_service.start()
    return gatecoin_service
