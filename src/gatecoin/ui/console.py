...
from gatecoin.constants import BLOCK_ID_LATEST, UINT256_MAX
...
def print_usage() -> None:
    print(f"\tuse `{HEADER}app{OKBLUE}` to interact with the top level Gatecoin API.")
    print(f"\t{OKBLUE}use `{HEADER}gatecoin{OKBLUE}` to interact with the gatecoin service.")
    print(
...
class Console(gevent.Greenlet):
    ...
    def __init__(self, gatecoin_service: GatecoinService) -> None:
        super().__init__()
        self.gatecoin_service = gatecoin_service
    ...
    def _run(self) -> None:  # pylint: disable=method-hidden
        ...
        tools = ConsoleTools(self.gatecoin_service)

        self.console_locals = {
            "gatecoin": self.gatecoin_service,
            "denoms": denoms,
            "proxy_manager": self.gatecoin_service.proxy_manager,
            "tools": tools,
            "lasterr": lasterr,
            "lastlog": lastlog,
            "usage": print_usage,
...
class ConsoleTools:
    ...
    def __init__(self, gatecoin_service: GatecoinService) -> None:
        self._gatecoin = gatecoin_service
        self._api = GatecoinAPI(gatecoin_service)

    def create_token(
        self,
        registry_address_hex: AddressHex,
        initial_alloc: int = 10**6,
        name: str = "gatecointester",
        symbol: str = "RDT",
        decimals: int = 2,
        timeout: int = 60,
        ...
        Per default it will be registered with 'gatecoin'.
        ...
        Args:
            ...
            auto_register: if True(default), automatically register
                the token with gatecoin.

        Returns:
            token_address_hex: the hex encoded address of the new token/token.
        """
        with gevent.Timeout(timeout):
            contract_proxy, _ = self._gatecoin.rpc_client.deploy_single_contract(
                contract_name=CONTRACT_HUMAN_STANDARD_TOKEN,
                contract=self._gatecoin.contract_manager.get_contract(
    ...
    def register_token(
        ...
        """Register a token with the gatecoin token manager.
        ...
        registry = self._gatecoin.proxy_manager.token_network_registry(
        ...
        waiting.wait_for_token_network(
            self._gatecoin, registry.address, token_address, retry_timeout
        )

        return self._gatecoin.proxy_manager.token_network(token_network_address, BLOCK_ID_LATEST)
    ...
    def wait_for_contract(self, contract_address_hex: AddressHex, timeout: int = None) -> bool:
        ...
        result = self._gatecoin.rpc_client.web3.eth.get_code(
