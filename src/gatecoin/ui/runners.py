...
from gatecoin.tasks import check_chain_id, check_gas_reserve, check_rdn_deposits, check_version
...
def run_services(options: Dict[str, Any]) -> None:
    gatecoin_service = run_gatecoin_service(**options)

    gevent_tasks: List[gevent.Greenlet] = []

    if options["console"]:
        from gatecoin.ui.console import Console

        console = Console(gatecoin_service)
        console.start()
    ...
    gevent_tasks.append(
        spawn_named(
            "check_version", check_version, get_system_spec()["gatecoin"], gatecoin_service
        )
    )
    gevent_tasks.append(spawn_named("check_gas_reserve", check_gas_reserve, gatecoin_service))
    gevent_tasks.append(
        spawn_named(
            "check_chain_id",
            check_chain_id,
            gatecoin_service.rpc_client.chain_id,
            gatecoin_service.rpc_client.web3,
        )
    )

    spawn_user_deposit_task = gatecoin_service.default_user_deposit and (
    ...
    if spawn_user_deposit_task:
        gevent_tasks.append(
            spawn_named(
                ...
                check_rdn_deposits,
                gatecoin_service,
                gatecoin_service.default_user_deposit,
            )
        )
    ...
    gatecoin_service.greenlet.link(stop_event)
    ...
    try:
        ...
        gatecoin_service.stop()

        gevent.joinall(
            set(gevent_tasks + [gatecoin_service]),
            gatecoin_service.config.shutdown_timeout,
            raise_error=True,
        )
