...
from gatecoin.accounts import KeystoreAuthenticationError, KeystoreFileNotFound
...
from gatecoin.log_config import configure_logging
from gatecoin.network.utils import get_free_port
...
from gatecoin.ui.app import setup_gatecoin_config
from gatecoin.ui.runners import run_services
from gatecoin.utils.cli import (
    ADDRESS_TYPE,
...
from gatecoin.utils.debugging import IDLE, enable_monitoring_signal
from gatecoin.utils.formatting import to_checksum_address
from gatecoin.utils.system import get_system_spec
from gatecoin.utils.tracing import enable_pfs_request_tracing
from gatecoin.utils.typing import MYPY_ANNOTATION, ChainID
from gatecoin_contracts.constants import ID_TO_CHAINNAME
from gatecoin_contracts.contract_manager import ContractDevEnvironment
...
class ReturnCode(Enum):
    ...
    ETH_ACCOUNT_ERROR = 6
    GATECOIN_CONFIGURATION_ERROR = 7
...
def write_stack_trace(ex: Exception) -> None:
    file = NamedTemporaryFile(
        "w",
        prefix=f"gatecoin-exception-{datetime.datetime.utcnow():%Y-%m-%dT%H-%M}",
        suffix=".txt",
        delete=False,
    )
...
def get_version(short: bool) -> str:
    if short:
        return get_system_spec()["gatecoin"]
    else:
...
OPTIONS = [
    ...
    option(
        "--datadir",
        help="Directory for storing Gatecoin data.",
        default="~/.gatecoin",
        type=ExpandablePath(
            exists=False,
            dir_okay=True,
    ...
    option(
        "--address",
        help=(
            "The Ethereum address you would like Gatecoin to use and for which "
    ...
    option(
        ...
        "chain_id",
        help=(
            "Specify the chain name/id of the Ethereum network to run Gatecoin on.\n"
            "Available networks:\n"
            '"mainnet" - chain id: 1\n'
            '"ropsten" - chain id: 3\n'
            '"rinkeby" - chain id: 4\n'
    ...
    option(
        ...
        help=(
            ...
            "for running Gatecoin on the mainnet.\n"
        ),
    ...
    option(
        ...
        help=(
            ...
            "Change this only when you are developing Gatecoin itself."
        ),
    ...
    option_group(
        ...
        option(
            ...
            help=(
                f"URL to the Gatecoin path finding service to request paths from.\n "
                f"Example: https://pfs-ropsten.services-dev.gatecoin.network\n "
                f"Can also be given the '{MATRIX_AUTO_SELECT_SERVER}' value "
                f"so that Gatecoin chooses a PFS randomly from the service "
                f"registry contract."
            ),
    ...
    option_group(
        ...
        option(
            ...
            help=(
                ...
                f"A URL pointing to a Gatecoin matrix homeserver"
            ),
            default=MATRIX_AUTO_SELECT_SERVER,
    ...
    option_group(
        ...
        option(
            ...
            help=(
                ...
                " - OSX: ~/Library/Logs/Gatecoin/gatecoin_debug_XXX.log\n"
                " - Windows: ~/Appdata/Roaming/Gatecoin/gatecoin_debug_XXX.log\n"
                " - Linux: ~/.gatecoin/gatecoin_debug_XXX.log\n"
...
if find_spec("IPython"):
    OPTIONS.append(
        option(
            "--console/--no-console", help="Start the interactive Gatecoin console", default=False
        )
    )
else:
...
def _run(ctx: Context, **kwargs: Any) -> None:
    ...
    try:
        ...
        if flamegraph:  # pragma: no cover
            ...
            from gatecoin.utils.profiling.sampler import FlameGraphCollector, TraceSampler
        ...
        if switch_tracing is True:  # pragma: no cover
            ...
            from gatecoin.utils.profiling.greenlets import SwitchMonitoring
        ...
        if log_memory_usage_interval > 0:  # pragma: no cover
            ...
            from gatecoin.utils.profiling.memory import MemoryLogger
        ...
        gatecoin_version = get_version(short=True)
        click.secho(f"Welcome to Gatecoin, version {gatecoin_version}!", fg="green")
        ...
        gatecoin_config = setup_gatecoin_config(**kwargs)
        kwargs["config"] = gatecoin_config
        ...
        click.secho(
            textwrap.dedent(
                ...
                |                                                                        |
                | Privacy warning: Please be aware, that by using the Gatecoin Client,     |
                ...
                | same might also be stored on systems of parties running Gatecoin nodes   |
                ...
                |                                                                        |
                | Also be aware, that data on individual Gatecoin token transfers will be  |
                ...
                | server operators, see Gatecoin Transport Specification.                  |
                +------------------------------------------------------------------------+"""
            ),
            fg="yellow",
        )
        ...
        if enable_tracing:
            tracing_config = Config(
                config={
                    ...
                    "logging": True,
                    "tags": {
                        "gatecoin.client.version": get_version(short=True),
                        "gatecoin.client.type": "Python",
                    },
                },
                service_name="gatecoin_client",
        ...
        click.secho(
            f"{ex}. Please make sure that this Gatecoin node is the "
    ...
    except GatecoinUnrecoverableError as ex:
        ...
        click.secho(str(e), fg="red")
        sys.exit(ReturnCode.GATECOIN_CONFIGURATION_ERROR)
        ...
        click.secho(
            f"FATAL: Another Gatecoin instance already running for account "
...
@run.command()
@option("--short", is_flag=True, help="Only display Gatecoin version")
@click.pass_context
...
def _smoketest(
    ...
    """Test, that the Gatecoin installation is sane."""
    from gatecoin.tests.utils.smoketest import run_smoketest, setup_smoketest, step_printer

    gatecoin_stdout = StringIO()
    ...
    append_report("Gatecoin version", json.dumps(get_system_spec()))
    append_report("Gatecoin log")
    ...
    try:
        with step_printer(step_count=8, stdout=sys.stdout) as print_step:
            with setup_smoketest(
                ...
                @options
                def _setup_gatecoin_config(**kwargs: Any) -> None:
                    gatecoin_config = setup_gatecoin_config(**kwargs)
                    args["config"] = gatecoin_config
                    args.update(kwargs)
                    return
