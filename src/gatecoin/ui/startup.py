...
from gatecoin.constants import BLOCK_ID_LATEST, Environment, RoutingMode
from gatecoin.exceptions import GatecoinError
from gatecoin.network.pathfinding import PFSConfig, check_pfs_for_production, configure_pfs_or_exit
...
from gatecoin.settings import GATECOIN_CONTRACT_VERSION, GatecoinConfig
from gatecoin.ui.checks import (
    DeploymentAddresses,
    check_pfs_configuration,
    check_gatecoin_environment,
)
...
def load_deployed_contracts_data(
    ...
    """
    check_gatecoin_environment(chain_id, config.environment_type)
...
def gatecoin_bundle_from_contracts_deployment(
    proxy_manager: ProxyManager,
    token_network_registry_address: TokenNetworkRegistryAddress,
    secret_registry_address: SecretRegistryAddress,
) -> GatecoinBundle:
    ...
    # By now these should be set or Gatecoin aborted
