...
from gatecoin.exceptions import InsufficientEth, GatecoinUnrecoverableError, ServiceRequestFailed
from gatecoin.messages.abstract import Message
from gatecoin.messages.encode import message_from_sendevent
from gatecoin.network.pathfinding import post_pfs_feedback
from gatecoin.network.proxies.payment_channel import PaymentChannel, WithdrawInput
from gatecoin.network.proxies.token_network import TokenNetwork
from gatecoin.network.resolver.client import reveal_secret_with_resolver
from gatecoin.network.transport.matrix.transport import MessagesQueue
from gatecoin.storage.restore import (
...
from gatecoin.transfer.architecture import Event, SendMessageEvent
...
from gatecoin.transfer.identifiers import CanonicalIdentifier, QueueIdentifier
...
from gatecoin.transfer.state import ChainState, NettingChannelEndState
...
from gatecoin.utils.packing import pack_signed_balance_proof, pack_withdraw
...
from gatecoin_contracts.constants import MessageTypeId
...
class EventHandler(ABC):
    @abstractmethod
    def on_gatecoin_events(
        self, gatecoin: "GatecoinService", chain_state: ChainState, events: List[Event]
    ) -> None:
        pass


class GatecoinEventHandler(EventHandler):
    def on_gatecoin_events(
        self, gatecoin: "GatecoinService", chain_state: ChainState, events: List[Event]
        ...
        gatecoin.transport.send_async(all_messages)

    @staticmethod
    def handle_missing_metadata_events(
        gatecoin: "GatecoinService", event: RequestMetadata, chain_state: ChainState
    ) -> None:
        ...
        for wrapped_event in event.dependant_events:
            ...
            if gatecoin.config.pfs_config is not None:
                try:
        ...
        gatecoin.async_handle_events(chain_state, new_events)
    ...
    def handle_send_lockexpired(
        ...
        gatecoin.sign(lock_expired_message)
    ...
    def handle_send_lockedtransfer(
        ...
        gatecoin.sign(mediated_transfer_message)
    ...
    def handle_send_secretreveal(
        ...
        gatecoin.sign(reveal_secret_message)
    ...
    def handle_send_secretrequest(
        ...
        if reveal_secret_with_resolver(gatecoin, chain_state, secret_request_event):
            return

        secret_request_message = message_from_sendevent(secret_request_event)
        gatecoin.sign(secret_request_message)
    ...
    def handle_send_withdrawrequest(
        ...
        gatecoin.sign(withdraw_request_message)
    ...
    def handle_send_withdraw(
        ...
        gatecoin.sign(withdraw_message)
    ...
    def handle_send_withdrawexpired(
        ...
        gatecoin.sign(withdraw_expired_message)
    ...
    def handle_send_processed(
        ...
        gatecoin.sign(processed_message)
    ...
    def handle_paymentsentsuccess(
        gatecoin: "GatecoinService", payment_sent_success_event: EventPaymentSentSuccess
    ) -> None:  # pragma: no unittest
        target = payment_sent_success_event.target
        payment_identifier = payment_sent_success_event.identifier
        payment_status = gatecoin.targets_to_identifiers_to_statuses[target].pop(
            payment_identifier
        )
        ...
        # see: https://github.com/gatecoin/gatecoin/pull/3191
        payment_status.payment_done.set(payment_sent_success_event)

    @staticmethod
    def handle_paymentsentfailed(
        gatecoin: "GatecoinService", payment_sent_failed_event: EventPaymentSentFailed
    ) -> None:  # pragma: no unittest
    ...
    def handle_unlockfailed(
        gatecoin: "GatecoinService", unlock_failed_event: EventUnlockFailed
    ) -> None:  # pragma: no unittest
    ...
    def handle_invalidsecretrequest(
        gatecoin: "GatecoinService", invalid_secret_request_event: EventInvalidSecretRequest
    ) -> None:  # pragma: no unittest
        # pylint: disable=unused-argument
        log.warning(
    ...
    def handle_contract_send_secretreveal(
        gatecoin: "GatecoinService", channel_reveal_secret_event: ContractSendSecretReveal
    ) -> None:  # pragma: no unittest
        try:
            gatecoin.default_secret_registry.register_secret(
                secret=channel_reveal_secret_event.secret
            )
        except InsufficientEth as e:
            raise GatecoinUnrecoverableError(str(e)) from e

    @staticmethod
    def handle_contract_send_channelwithdraw(
        gatecoin: "GatecoinService", channel_withdraw_event: ContractSendChannelWithdraw
    ) -> None:
        withdraw_confirmation_data = pack_withdraw(
            canonical_identifier=channel_withdraw_event.canonical_identifier,
            participant=gatecoin.address,
            total_withdraw=channel_withdraw_event.total_withdraw,
            expiration_block=channel_withdraw_event.expiration,
        )
        our_signature = gatecoin.signer.sign(data=withdraw_confirmation_data)
        ...
        if channel_state is None:
            raise GatecoinUnrecoverableError(
                "ContractSendChannelWithdraw for non-existing channel."
            )

        channel_proxy = gatecoin.proxy_manager.payment_channel(
    ...
    def handle_contract_send_coopsettle(
        gatecoin: "GatecoinService", channel_coopsettle_event: ContractSendChannelCoopSettle
    ) -> None:
        chain_state = state_from_gatecoin(gatecoin)
        channel_state = get_channelstate_by_canonical_identifier(
            chain_state=chain_state,
        ...
        our_signature_initiator = gatecoin.signer.sign(data=participant_withdraw_data)
        withdraw_initiator = WithdrawInput(
            initiator=gatecoin.address,
            total_withdraw=channel_coopsettle_event.our_total_withdraw,
            expiration_block=channel_coopsettle_event.expiration,
        ...
        our_signature_partner = gatecoin.signer.sign(data=partner_withdraw_data)
    ...
    def handle_contract_send_channelclose(
        ...
        our_signature = gatecoin.signer.sign(data=closing_data)

        confirmed_block_identifier = state_from_gatecoin(gatecoin).block_hash
        channel_state = get_channelstate_by_canonical_identifier(
            chain_state=chain_state, canonical_identifier=channel_close_event.canonical_identifier
        )

        if channel_state is None:
            raise GatecoinUnrecoverableError("ContractSendChannelClose for non-existing channel.")
    ...
    def handle_contract_send_channelupdate(
        gatecoin: "GatecoinService", channel_update_event: ContractSendChannelUpdateTransfer
    ) -> None:
        ...
        if balance_proof:
            ...
            channel = gatecoin.proxy_manager.payment_channel(
                channel_state=channel_state, block_identifier=confirmed_block_identifier
            )
            ...
            our_signature = gatecoin.signer.sign(data=non_closing_data)
    ...
    def handle_contract_send_channelunlock(
        ...
        """
        assert gatecoin.wal, "The Gatecoin Service must be initialize to handle events"
        ...
        channel_state = get_channelstate_by_canonical_identifier(
            chain_state=state_from_gatecoin(gatecoin), canonical_identifier=canonical_identifier
        )
        if channel_state is None:
            raise GatecoinUnrecoverableError(
        ...
        confirmed_block_identifier = state_from_gatecoin(gatecoin).block_hash
        payment_channel: PaymentChannel = gatecoin.proxy_manager.payment_channel(
            channel_state=channel_state, block_identifier=confirmed_block_identifier
        )
        ...
        if not search_events and not search_state_changes:
            ...
            # Check https://github.com/gatecoin/gatecoin/issues/3152
            # for more details
            log.warning(
        ...
        if search_state_changes:
            ...
            if state_change_record is None:
                raise GatecoinUnrecoverableError(
                    ...
                    f"chain_id:{gatecoin.rpc_client.chain_id} "
                    f"token_network:{to_checksum_address(token_network_address)} "
    ...
    def handle_contract_send_channelsettle(
        gatecoin: "GatecoinService", channel_settle_event: ContractSendChannelSettle
    ) -> None:
        assert gatecoin.wal, "The Gatecoin Service must be initialize to handle events"

        canonical_identifier = CanonicalIdentifier(
            chain_identifier=gatecoin.rpc_client.chain_id,
        ...
        if channel_state is None:
            raise GatecoinUnrecoverableError("ContractSendChannelSettle for non-existing channel.")
    ...
    def handle_update_services_addresses(
        gatecoin: "GatecoinService", update_services_addresses: UpdateServicesAddresses
    ) -> None:  # pragma: no unittest
        gatecoin.transport.update_services_addresses(
            {update_services_addresses.service_address: update_services_addresses.validity}
        )


class PFSFeedbackEventHandler(GatecoinEventHandler):
    ...
    def on_gatecoin_events(
        ...
        for event in events:
            if type(event) == EventRouteFailed:
                ...
                self.handle_routefailed(gatecoin, event)
            elif type(event) == EventPaymentSentSuccess:
                assert isinstance(event, EventPaymentSentSuccess), MYPY_ANNOTATION
                self.handle_paymentsentsuccess(gatecoin, event)

        self.wrapped.on_gatecoin_events(gatecoin, chain_state, events)

    @staticmethod
    def handle_routefailed(
        gatecoin: "GatecoinService", route_failed_event: EventRouteFailed
    ) -> None:  # pragma: no unittest
        feedback_token = gatecoin.route_to_feedback_token.get(tuple(route_failed_event.route))
        ...
        if feedback_token and pfs_config:
            ...
            post_pfs_feedback(
                routing_mode=gatecoin.routing_mode,
    ...
    def handle_paymentsentsuccess(
        ...
        feedback_token = gatecoin.route_to_feedback_token.get(
