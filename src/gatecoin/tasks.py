...
from gatecoin.settings import MIN_REI_THRESHOLD
from gatecoin.utils import gas_reserve
...
from gatecoin.utils.typing import Any, BlockNumber, Callable, ChainID, List, Optional
...
def _do_check_version(
    current_version: Union[Version, LegacyVersion], gatecoin: "GatecoinService"
) -> bool:
    ...
    if security_message:
        ...
        gatecoin.add_notification(notification, click_opts={"fg": "red"})
...
def check_version(
    current_version: str, gatecoin: "GatecoinService"
) -> None:  # pragma: no unittest
    """Check periodically for a new release"""
    app_version = parse_version(current_version)
    while True:
        try:
            _do_check_version(app_version, gatecoin)
...
def check_gas_reserve(gatecoin: "GatecoinService") -> None:  # pragma: no unittest
...
def check_rdn_deposits(
    gatecoin: "GatecoinService", user_deposit_proxy: UserDeposit
) -> None:  # pragma: no unittest
    """Check periodically for RDN deposits in the user-deposits contract"""
    while True:
        rei_balance = user_deposit_proxy.effective_balance(gatecoin.address, BLOCK_ID_LATEST)
...
def check_chain_id(chain_id: ChainID, web3: Web3) -> None:  # pragma: no unittest
    ...
    while True:
        ...
        if chain_id != current_id:
            raise RuntimeError(
                f"Gatecoin was running on network with id {chain_id} and it detected "
                f"that the underlying ethereum client network id changed to {current_id}."
                f" Changing the underlying blockchain while the Gatecoin node is running "
                f"is not supported."
            )
