...
from gatecoin.constants import BLOCK_ID_PENDING, NULL_ADDRESS_BYTES, UINT64_MAX, UINT256_MAX
...
from gatecoin.settings import DEFAULT_RETRY_TIMEOUT, PythonApiConfig
from gatecoin.storage.utils import TimestampedEvent
from gatecoin.transfer import channel, views
from gatecoin.transfer.architecture import Event, StateChange, TransferTask
from gatecoin.transfer.events import (
    EventPaymentReceivedSuccess,
    EventPaymentSentFailed,
    EventPaymentSentSuccess,
)
from gatecoin.transfer.mediated_transfer.tasks import InitiatorTask, MediatorTask, TargetTask
from gatecoin.transfer.state import (
    ChainState,
    ChannelState,
    NettingChannelState,
    NetworkState,
...
from gatecoin.transfer.state_change import ActionChannelClose, ActionChannelCoopSettle
from gatecoin.transfer.views import TransferRole, get_token_network_by_address
from gatecoin.utils.formatting import to_checksum_address
from gatecoin.utils.gas_reserve import has_enough_gas_reserve
...
if TYPE_CHECKING:
    from gatecoin.gatecoin_service import PaymentStatus, GatecoinService
...
def get_transfer_from_task(
    ...
    if isinstance(transfer_task, InitiatorTask):
        # Work around for https://github.com/gatecoin/gatecoin/issues/5480,
        # can be removed when
        # https://github.com/gatecoin/gatecoin/issues/5515 is done.
...
class GatecoinAPI:  # pragma: no unittest
    # pylint: disable=too-many-public-methods

    def __init__(self, gatecoin: "GatecoinService"):
        self.gatecoin = gatecoin

    @property
    def address(self) -> Address:
        return self.gatecoin.address

    @property
    def notifications(self) -> Dict:
        return self.gatecoin.notifications

    @property
    def config(self) -> PythonApiConfig:
        return self.gatecoin.config.python_api
    ...
    def token_network_register(
        ...
        Raises:
            ...
            GatecoinRecoverableError: If the register transaction failed, this may
        ...
        # views.state_from_gatecoin() returns the same state again and again
        ...
        chainstate = views.state_from_gatecoin(self.gatecoin)

        registry = self.gatecoin.proxy_manager.token_network_registry(
    ...
    def token_network_leave(
        ...
        token_network_address = views.get_token_network_address_by_token_address(
            chain_state=views.state_from_gatecoin(self.gatecoin),
            token_network_registry_address=registry_address,
            token_address=token_address,
        )
    ...
    def is_already_existing_channel(
        ...
        proxy_manager = self.gatecoin.proxy_manager
        ...
        channel_identifier = proxy.get_channel_identifier_or_none(
            participant1=self.gatecoin.address,
    ...
    def channel_open(
        ...
        if settle_timeout is None:
            settle_timeout = self.gatecoin.config.settle_timeout

        if reveal_timeout is None:
            reveal_timeout = self.gatecoin.config.reveal_timeout
        ...
        confirmed_block_identifier = views.get_confirmed_blockhash(self.gatecoin)
        registry = self.gatecoin.proxy_manager.token_network_registry(
        ...
        token_network = self.gatecoin.proxy_manager.token_network(
        ...
        has_enough_reserve, estimated_required_reserve = has_enough_gas_reserve(
            self.gatecoin, channels_to_open=1
        )
        ...
        except GatecoinRecoverableError:
        ...
        self.gatecoin.set_channel_reveal_timeout(
    ...
    def mint_token_for(
        ...
        """
        confirmed_block_identifier = self.gatecoin.get_block_number()
        token_proxy = self.gatecoin.proxy_manager.custom_token(
    ...
    def set_total_channel_withdraw(
        ...
        Raises:
            InvalidBinaryAddress: If either token_address or partner_address is not
                20 bytes long.
            GatecoinUnrecoverableError: May happen for multiple reasons:
        ...
        pfs_proxy = self.gatecoin.pfs_proxy
    ...
    def set_total_channel_deposit(
        ...
        Raises:
            InvalidBinaryAddress: If either token_address or partner_address is not
                20 bytes long.
            GatecoinRecoverableError: May happen for multiple reasons:
        ...
        token = self.gatecoin.proxy_manager.token(
            token_address, block_identifier=confirmed_block_identifier
        )
        token_network_registry = self.gatecoin.proxy_manager.token_network_registry(
            registry_address, block_identifier=confirmed_block_identifier
        )
        ...
        token_network_proxy = self.gatecoin.proxy_manager.token_network(
            address=token_network_address, block_identifier=confirmed_block_identifier
        )
        channel_proxy = self.gatecoin.proxy_manager.payment_channel(
        ...
        balance = token.balance_of(self.gatecoin.address, block_identifier=blockhash)
        ...
        target_address = self.gatecoin.address
    ...
    def channel_batch_close(
        ...
        greenlets = set(self.gatecoin.handle_state_changes(close_state_changes))
    ...
    def _batch_coop_settle(
        ...
        greenlets = set(self.gatecoin.handle_state_changes(coop_settle_state_changes))
        gevent.joinall(greenlets, raise_error=True)
        ...
        for channel_state in channels_to_settle:
            ...
            expired = waiting.ChannelExpiredCoopSettle(
                self.gatecoin, channel_state.canonical_identifier
            )
        ...
        waiting.wait_for_channels(
            self.gatecoin, channels_to_conditions, retry_timeout=retry_timeout
        )
    ...
    def get_node_network_state(self, node_address: Address) -> NetworkState:
        ...
        return views.get_node_network_status(
            chain_state=views.state_from_gatecoin(self.gatecoin), node_address=node_address
        )
    ...
    def transfer_async(
        ...
        current_state = views.state_from_gatecoin(self.gatecoin)
        token_network_registry_address = self.gatecoin.default_registry.address
        ...
        valid_tokens = views.get_token_identifiers(
            views.state_from_gatecoin(self.gatecoin), registry_address
        )
        if token_address not in valid_tokens:
            raise UnknownTokenAddress("Token address is not known.")
        ...
        log.debug(
            "Initiating transfer",
            initiator=to_checksum_address(self.gatecoin.address),
        ...
        with opentracing.tracer.start_span("mediated_transfer_async"):
            payment_status = self.gatecoin.mediated_transfer_async(
                token_network_address=token_network_address,
                amount=amount,
                target=target,
                identifier=identifier,
    ...
    def get_gatecoin_events_payment_history_with_timestamps(
        ...
        if token_address and not is_binary_address(token_address):
            raise InvalidBinaryAddress(
                "Expected binary address format for token in get_gatecoin_events_payment_history"
            )
        ...
        if token_address:
            ...
            if not token_network_address:
                raise InvalidTokenAddress("Token address does not match a Gatecoin token network")

        if target_address and not is_binary_address(target_address):
            raise InvalidBinaryAddress(
                "Expected binary address format for "
                "target_address in get_gatecoin_events_payment_history"
            )

        assert self.gatecoin.wal, "Gatecoin service has to be started for the API to be usable."

        event_types = [
            "gatecoin.transfer.events.EventPaymentReceivedSuccess",
            "gatecoin.transfer.events.EventPaymentSentFailed",
            "gatecoin.transfer.events.EventPaymentSentSuccess",
        ]

        events = self.gatecoin.wal.storage.get_gatecoin_events_payment_history_with_timestamps(
    ...
    def get_gatecoin_internal_events_with_timestamps(
        ...
        assert self.gatecoin.wal, "Gatecoin service has to be started for the API to be usable."
        return self.gatecoin.wal.storage.get_events_with_timestamps(limit=limit, offset=offset)
    ...
    def get_pending_transfers(
        ...
        confirmed_block_identifier = chain_state.block_hash
        if token_address is not None:
            token_network = self.gatecoin.default_registry.get_token_network(
                token_address=token_address, block_identifier=confirmed_block_identifier
            )
    ...
    def set_total_udc_deposit(self, new_total_deposit: TokenAmount) -> TransactionHash:
        ...
        Raises:
            ...
            GatecoinRecoverableError: The transaction failed for any reason.

        Returns: TransactionHash of the successfully mined transaction.
        """
        user_deposit = self.gatecoin.default_user_deposit
    ...
    def get_new_notifications(self) -> List:
        notifications = list(self.gatecoin.notifications.values())
        self.gatecoin.notifications = {}
