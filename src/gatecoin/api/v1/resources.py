...
from gatecoin.api.rest_utils import if_api_available
from gatecoin.api.v1.encoding import (
    ...
    MintTokenSchema,
    PaymentSchema,
    GatecoinEventsRequestSchema,
    UserDepositPostSchema,
)
from gatecoin.utils.typing import TYPE_CHECKING, Address, Any, TargetAddress, TokenAddress

if TYPE_CHECKING:
    from gatecoin.api.rest import RestAPI
...
class VersionResource(BaseResource):
    @if_api_available
    def get(self) -> Response:
        return self.rest_api.get_gatecoin_version()
...
class ChannelsResource(BaseResource):
    ...
    def get(self) -> Response:
        ...
        return self.rest_api.get_channel_list(
            self.rest_api.gatecoin_api.gatecoin.default_registry.address
        )
    ...
    def put(self, **kwargs: Any) -> Response:
        ...
        return self.rest_api.open(
            registry_address=self.rest_api.gatecoin_api.gatecoin.default_registry.address, **kwargs
        )
...
class PartnersResourceByTokenAddress(BaseResource):
    ...
    def get(self, token_address: TokenAddress) -> Response:
        return self.rest_api.get_partners_by_token(
            self.rest_api.gatecoin_api.gatecoin.default_registry.address, token_address
        )


class GatecoinInternalEventsResource(BaseResource):

    get_schema = GatecoinEventsRequestSchema()

    @if_api_available
    def get(self) -> Response:
        kwargs = validate_query_params(self.get_schema)
        return self.rest_api.get_gatecoin_internal_events_with_timestamps(**kwargs)
...
class ConnectionsResource(BaseResource):
    ...
    def delete(self, token_address: TokenAddress) -> Response:
        return self.rest_api.leave(
            registry_address=self.rest_api.gatecoin_api.gatecoin.default_registry.address,
...
class PaymentEventsResource(BaseResource):
    ...
    def get(self, token_address: TokenAddress = None) -> Response:
        kwargs = validate_query_params(self.get_schema)
        return self.rest_api.get_gatecoin_events_payment_history_with_timestamps(
            registry_address=self.rest_api.gatecoin_api.gatecoin.default_registry.address,
            token_address=token_address,
            **kwargs,
        )
