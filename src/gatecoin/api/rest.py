...
from gatecoin_webui import GATECOIN_WEBUI_PATH
from werkzeug.exceptions import NotFound
from werkzeug.routing import BaseConverter

from gatecoin.api.exceptions import ChannelNotFound, NonexistingChannel
from gatecoin.api.objects import AddressList, PartnersPerTokenList
from gatecoin.api.python import GatecoinAPI
from gatecoin.api.rest_utils import api_error, api_response
from gatecoin.api.v1.encoding import (
    AddressListSchema,
    ChannelStateSchema,
...
from gatecoin.api.v1.resources import (
    AddressResource,
    ChannelsResource,
    ...
    GatecoinInternalEventsResource,
...
from gatecoin.constants import UINT256_MAX, Environment
...
from gatecoin.transfer import channel, views
from gatecoin.transfer.events import (
    EventPaymentReceivedSuccess,
    EventPaymentSentFailed,
    EventPaymentSentSuccess,
)
from gatecoin.transfer.state import ChannelState, NettingChannelState, RouteState
from gatecoin.ui.sync import blocks_to_sync
from gatecoin.utils.formatting import optional_address_to_string, to_checksum_address
from gatecoin.utils.gevent import spawn_named
from gatecoin.utils.http import split_endpoint
from gatecoin.utils.runnable import Runnable
from gatecoin.utils.system import get_system_spec
...
URLS_V1 = [
    ...
    ("/_debug/gatecoin_events", GatecoinInternalEventsResource),
...
class APIServer(Runnable):  # pragma: no unittest
    ...
    The API is wrapped in multiple layers, and the Server should be invoked this way::
        ...
        # instance of the gatecoin-api
        gatecoin_api = GatecoinAPI(...)

        # wrap the gatecoin-api with rest-logic and encoding
    ...
    def __init__(
        ...
        self.flask_app.config["WEBUI_PATH"] = GATECOIN_WEBUI_PATH
    ...
    def _check_shutdown_before_handle_request(self) -> Optional[Response]:
        ...
        if self.stop_event.is_set():
            return api_error("Gatecoin API is shutting down", HTTPStatus.SERVICE_UNAVAILABLE)
    ...
    def _serve_webui(self, file_name: str = "index.html") -> Response:
        try:
            ...
            if "config." in file_name and file_name.endswith(".json"):
                environment_type = (
                    self.rest_api.gatecoin_api.gatecoin.config.environment_type.name.lower()
                )
                config = {
                    "gatecoin": self._api_prefix,
                    "web3": web3,
                    "settle_timeout": self.rest_api.gatecoin_api.gatecoin.config.settle_timeout,
                    "reveal_timeout": self.rest_api.gatecoin_api.gatecoin.config.reveal_timeout,
                    "environment_type": environment_type,
                }

                # if gatecoin sees eth rpc endpoint as localhost, replace it by Host header,
                # which is the hostname by which the client/browser sees/access the gatecoin node
...
class RestAPI:  # pragma: no unittest
    """
    This wraps around the actual GatecoinAPI in api/python.
    It will provide the additional, neccessary RESTful logic and
    the proper JSON-encoding of the Objects provided by the GatecoinAPI
    """

    version = 1

    def __init__(self, gatecoin_api: GatecoinAPI = None, rpc_client: JSONRPCClient = None) -> None:
        self._rpc_client = rpc_client
        self._gatecoin_api = gatecoin_api
    ...
    def checksum_address(self) -> Optional[str]:
        return to_checksum_address(self.gatecoin_api.address) if self.available else None

    @property
    def gatecoin_api(self) -> GatecoinAPI:
        assert self._gatecoin_api is not None, "gatecoin_api accessed but not initialized"
        return self._gatecoin_api

    @gatecoin_api.setter
    def gatecoin_api(self, gatecoin_api: GatecoinAPI) -> None:
        self._gatecoin_api = gatecoin_api

    @property
    def available(self) -> bool:
        return self._gatecoin_api is not None

    def get_our_address(self) -> Response:
        return api_response(result=dict(our_address=self.checksum_address))

    @classmethod
    def get_gatecoin_version(cls) -> Response:
        return api_response(result=dict(version=get_system_spec()["gatecoin"]))

    def get_node_settings(self) -> Response:
        pfs_config = self.gatecoin_api.gatecoin.config.pfs_config
    ...
    def get_contract_versions(self) -> Response:
        gatecoin = self.gatecoin_api.gatecoin
        service_registry_address = gatecoin.default_service_registry and to_checksum_address(
            gatecoin.default_service_registry.address
        )
        user_deposit_address = gatecoin.default_user_deposit and to_checksum_address(
            gatecoin.default_user_deposit.address
        )
        monitoring_service_address = gatecoin.default_msc_address and to_checksum_address(
            gatecoin.default_msc_address
        )
        one_to_n_address = gatecoin.default_one_to_n_address and to_checksum_address(
            gatecoin.default_one_to_n_address
        )
        contracts = dict(
            contracts_version=gatecoin.proxy_manager.contract_manager.contracts_version,
            token_network_registry_address=to_checksum_address(gatecoin.default_registry.address),
            secret_registry_address=to_checksum_address(gatecoin.default_secret_registry.address),
    ...
    def register_token(
        ...
        if self.gatecoin_api.gatecoin.config.environment_type == Environment.PRODUCTION:
            return api_error(
        ...
        try:
            token_network_address = self.gatecoin_api.token_network_register(
    ...
    def mint_token_for(
        ...
        try:
            transaction_hash = self.gatecoin_api.mint_token_for(
    ...
    def open(
        ...
        confirmed_block_identifier = views.get_confirmed_blockhash(self.gatecoin_api.gatecoin)
        try:
            token = self.gatecoin_api.gatecoin.proxy_manager.token(
                token_address, block_identifier=confirmed_block_identifier
            )
        except AddressWithoutCode as e:
            return api_error(errors=str(e), status_code=HTTPStatus.CONFLICT)

        balance = token.balance_of(self.gatecoin_api.gatecoin.address)
        ...
        try:
            self.gatecoin_api.channel_open(
                registry_address=registry_address,
                token_address=token_address,
            ...
            channel_status = channel.get_status(
                self.gatecoin_api.get_channel(registry_address, token_address, partner_address)
            )
        ...
        if total_deposit:
            ...
            try:
                self.gatecoin_api.set_total_channel_deposit(
                    registry_address=registry_address,
                    token_address=token_address,
                    partner_address=partner_address,
                    total_deposit=total_deposit,
    ...
    def leave(
        ...
        closed_channels = self.gatecoin_api.token_network_leave(registry_address, token_address)
    ...
    def get_connection_managers_info(
        ...
        for token in self.gatecoin_api.get_tokens_list(registry_address):
            open_channels = views.get_channelstate_open(
                chain_state=views.state_from_gatecoin(self.gatecoin_api.gatecoin),
                ...
                connection_managers[to_checksum_address(token)] = {
                    "sum_deposits": str(
                        views.get_our_deposits_for_token_network(
                            views.state_from_gatecoin(self.gatecoin_api.gatecoin),
                            registry_address,
                            token,
                        )
                    ),
    ...
    def get_channel_list(
        ...
        gatecoin_service_result = self.gatecoin_api.get_channel_list(
            registry_address, token_address, partner_address
        )
        typecheck(gatecoin_service_result, list)

        channels_result = []
        for channel_state in gatecoin_service_result:
            result = self._updated_channel_state(registry_address, channel_state)
    ...
    def get_tokens_list(self, registry_address: TokenNetworkRegistryAddress) -> Response:
        ...
        gatecoin_service_result = self.gatecoin_api.get_tokens_list(registry_address)
        typecheck(gatecoin_service_result, list)

        tokens_list = AddressList(gatecoin_service_result)
    ...
    def get_token_network_for_token(
        ...
        token_network_address = self.gatecoin_api.get_token_network_address_for_token_address(
    ...
    def get_gatecoin_events_payment_history_with_timestamps(
        self,
        registry_address: TokenNetworkRegistryAddress,
        token_address: TokenAddress = None,
        ...
        try:
            service_result = self.gatecoin_api.get_gatecoin_events_payment_history_with_timestamps(
                registry_address=registry_address,
                token_address=token_address,
    ...
    def get_gatecoin_internal_events_with_timestamps(
        self, limit: Optional[int], offset: Optional[int]
    ) -> Response:
        assert self.gatecoin_api.gatecoin.wal, "Gatecoin Service has to be initialized"
        events = [
            str(e)
            for e in self.gatecoin_api.gatecoin.wal.storage.get_events_with_timestamps(
    ...
    def get_partners_by_token(
        ...
        for result in gatecoin_service_result:
            return_list.append(
                {
    ...
    def initiate_payment(
        ...
        try:
            payment_status = self.gatecoin_api.transfer_and_wait(
                registry_address=registry_address,
                token_address=token_address,
    ...
    def get_new_notifications(self) -> Response:
        ...
        notifications = self.gatecoin_api.get_new_notifications()
    ...
    def _withdraw(
        ...
        try:
            self.gatecoin_api.set_total_channel_withdraw(
                registry_address,
                channel_state.token_address,
                channel_state.partner_state.address,
                total_withdraw,
    ...
    def _set_channel_reveal_timeout(
        ...
        try:
            self.gatecoin_api.set_reveal_timeout(
                registry_address=registry_address,
                token_address=channel_state.token_address,
                partner_address=channel_state.partner_state.address,
                reveal_timeout=reveal_timeout,
    ...
    def _close(
        ...
        try:
            self.gatecoin_api.channel_close(
    ...
    def patch_channel(
        ...
        try:
            channel_state = self.gatecoin_api.get_channel(
                registry_address=registry_address,
                token_address=token_address,
                partner_address=partner_address,
            )
    ...
    def get_pending_transfers(
        ...
        try:
            return api_response(
                self.gatecoin_api.get_pending_transfers(
    ...
    def _deposit_to_udc(self, total_deposit: TokenAmount) -> Response:
        ...
        try:
            transaction_hash = self.gatecoin_api.set_total_udc_deposit(total_deposit)
        except (InsufficientEth, InsufficientFunds) as e:
            return api_error(errors=str(e), status_code=HTTPStatus.PAYMENT_REQUIRED)
    ...
    def _plan_withdraw_from_udc(self, planned_withdraw_amount: TokenAmount) -> Response:
        ...
        try:
            (transaction_hash, planned_withdraw_block_number) = (
                self.gatecoin_api.plan_udc_withdraw(planned_withdraw_amount)
            )
        except InsufficientEth as e:
            return api_error(errors=str(e), status_code=HTTPStatus.PAYMENT_REQUIRED)
    ...
    def _withdraw_from_udc(self, amount: TokenAmount) -> Response:
        ...
        try:
            transaction_hash = self.gatecoin_api.withdraw_from_udc(amount)
        except InsufficientEth as e:
            return api_error(errors=str(e), status_code=HTTPStatus.PAYMENT_REQUIRED)
        except WithdrawMismatch as e:
            return api_error(errors=str(e), status_code=HTTPStatus.CONFLICT)
    ...
    def shutdown(self) -> Response:
        shutdown_greenlet = spawn_named("trigger shutdown", self.gatecoin_api.shutdown)
        shutdown_greenlet.link_exception(self.gatecoin_api.gatecoin.on_error)
