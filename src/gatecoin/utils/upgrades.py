...
from gatecoin.storage.sqlite import SQLiteStorage
from gatecoin.storage.versions import VERSION_RE, filter_db_names, latest_db_file
from gatecoin.utils.typing import Any, Callable, DatabasePath, List, NamedTuple
...
class UpgradeManager:
    ...
    def run(self) -> None:
        ...
        if file_version == GATECOIN_DB_VERSION:
            return

        if file_version > GATECOIN_DB_VERSION:
            raise RuntimeError(
                f"Conflicting database versions detected, latest db version is v{file_version}, "
                f"Gatecoin client version is v{GATECOIN_DB_VERSION}."
                f"\n\n"
                f"Running a downgraded version of Gatecoin after an upgrade is not supported, "
                f"because the transfers done with the new client are not understandable by the "
                f"older."
            )

        if GATECOIN_DB_VERSION >= 27 and file_version <= 26 and file_version > 1:
            msg = (
                f"Your Gatecoin database is version {file_version} and there is no compatible "
                f"migration to version {GATECOIN_DB_VERSION} available.\n"
                "You need to either start a new Gatecoin node with a different account, or "
                "close and settle all channels, and start over with a fresh database.\n\n"
                "More information on this topic at "
                "https://gatecoin.readthedocs.io/en/latest/other/known-issues.html"
                "#database-upgrades\n\n"
                "If you are on **mainnet** and affected by this, please create an issue at "
                "https://github.com/gatecoin/gatecoin/issues/new?title=Mainnet%20Migration%20"
                f"{file_version}%20{GATECOIN_DB_VERSION}"
            )
            log.warning(msg)
            sys.exit(msg)
    ...
    def _upgrade(self, target_file: Path, from_file: Path, from_version: int) -> None:
        with get_file_lock(from_file), get_file_lock(target_file):
            ...
            with SQLiteStorage(target_file) as storage:
                log.debug(f"Upgrading database from v{from_version} to v{GATECOIN_DB_VERSION}")
                    ...
                    with storage.transaction():
                        for upgrade_record in UPGRADES_LIST:
                            ...
                            version_iteration = upgrade_record.function(
                                ...
                                current_version=GATECOIN_DB_VERSION,
                                **self._kwargs,
                            )

                        update_version(storage, GATECOIN_DB_VERSION)
