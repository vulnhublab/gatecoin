...
from gatecoin.constants import UNLOCK_TX_GAS_LIMIT
from gatecoin.network.rpc.client import gas_price_for_fast_transaction
from gatecoin.transfer import views
from gatecoin_contracts.contract_manager import gas_measurements
...
def get_required_gas_estimate(gatecoin: "GatecoinService", channels_to_open: int = 0) -> int:
    ...
    # iterating over it.
    with gatecoin.proxy_manager.token_network_creation_lock:
        num_opening_channels = sum(
            token_network.opening_channels_count
            for token_network in gatecoin.proxy_manager.address_to_token_network.values()
        )

    chain_state = views.state_from_gatecoin(gatecoin)
    registry_address = gatecoin.default_registry.address
    ...
    return _get_required_gas_estimate(
        gas_measurements=gas_measurements(gatecoin.contract_manager.contracts_version),
...
def get_reserve_estimate(gatecoin: "GatecoinService", channels_to_open: int = 0) -> int:
    gas_estimate = get_required_gas_estimate(gatecoin, channels_to_open)
    gas_price = gas_price_for_fast_transaction(gatecoin.rpc_client.web3)
...
def has_enough_gas_reserve(
    gatecoin: "GatecoinService", channels_to_open: int = 0
) -> Tuple[bool, int]:
    ...
    Args:
        gatecoin: A gatecoin service instance
    ...
    Returns:
        ...
        lifecycle cost
    """
    secure_reserve_estimate = get_reserve_estimate(gatecoin, channels_to_open)
    current_account_balance = gatecoin.rpc_client.balance(gatecoin.rpc_client.address)
