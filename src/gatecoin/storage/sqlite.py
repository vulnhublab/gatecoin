...
from gatecoin.constants import GATECOIN_DB_VERSION, SQLITE_MIN_REQUIRED_VERSION
from gatecoin.exceptions import InvalidDBData, InvalidNumberInput
from gatecoin.storage.serialization import SerializationBase
from gatecoin.storage.utils import DB_SCRIPT_CREATE_TABLES, TimestampedEvent
from gatecoin.transfer.architecture import Event, State, StateChange
...
class SQLiteStorage:
    def __init__(self, database_path: DatabasePath):
        ...
        try:
            ...
            raise InvalidDBData(
                f"Existing DB {database_path} was found to be corrupt at Gatecoin startup. "
    ...
    def update_version(self) -> None:
        ...
        cursor.execute(
            ...
            (str(GATECOIN_DB_VERSION),),
        )
        self.maybe_commit()

    def log_run(self) -> None:
        """Log timestamp and gatecoin version to help with debugging"""
        version = get_system_spec()["gatecoin"]
        cursor = self.conn.cursor()
        cursor.execute("INSERT INTO runs(gatecoin_version) VALUES (?)", [version])
        self.maybe_commit()

    def get_version(self) -> GatecoinDBVersion:
        cursor = self.conn.cursor()
        query = cursor.execute('SELECT value FROM settings WHERE name="version";')
        result = query.fetchall()
        # If setting is not set, it's the latest version
        if len(result) == 0:
            return GATECOIN_DB_VERSION

        return GatecoinDBVersion(int(result[0][0]))
...
class SerializedSQLiteStorage:
    ...
    def get_gatecoin_events_payment_history_with_timestamps(
        ...
        events = self.database.get_gatecoin_events_payment_history_with_timestamps(
