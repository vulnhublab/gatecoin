...
_FIRST_PARTY_PACKAGES = frozenset(["gatecoin", "gatecoin_contracts"])
...
class GatecoinFilter(logging.Filter):
...
def configure_debug_logfile_path(debug_log_file_path: Optional[str]) -> str:
    ...
    if os.name == "nt":
        ...
        time = time.replace(":", "_")
    debug_log_file_name = f"gatecoin-debug_{time}.log"

    home = os.path.expanduser("~")
    if home == "~":  # Could not expand user path, just use /tmp
        datadir = "/tmp"
    if sys.platform == "darwin":
        datadir = os.path.join(home, "Library", "Logs", "Gatecoin")
    elif sys.platform == "win32" or sys.platform == "cygwin":
        datadir = os.path.join(home, "AppData", "Roaming", "Gatecoin")
    elif os.name == "posix":
        datadir = os.path.join(home, ".gatecoin")
    else:
...
def configure_logging(
    ...
    if not disable_debug_logfile:
        ...
        handlers["debug-info"] = {
            ...
            "filters": ["gatecoin_debug_file_filter"],
        }
    ...
    logging.config.dictConfig(
        {
            ...
            "filters": {
                "user_filter": {"()": GatecoinFilter, "log_level_config": logger_level_config},
                "gatecoin_debug_file_filter": {
                    "()": GatecoinFilter,
                    "log_level_config": {
                        "": DEFAULT_LOG_LEVEL,
                        "gatecoin": "DEBUG",
    ...
    # all messages, which are then be filtered by the `GatecoinFilter`
