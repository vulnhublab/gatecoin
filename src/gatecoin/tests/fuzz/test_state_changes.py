...
from gatecoin.constants import GENESIS_BLOCK_NUMBER, LOCKSROOT_OF_NO_LOCKS, UINT64_MAX
from gatecoin.settings import (
    DEFAULT_NUMBER_OF_BLOCK_CONFIRMATIONS,
    DEFAULT_WAIT_BEFORE_LOCK_REMOVAL,
)
from gatecoin.tests.fuzz import utils
from gatecoin.tests.utils import factories
from gatecoin.tests.utils.factories import make_block_hash
from gatecoin.transfer import channel, node
...
class Client:
    ...
    def assert_monotonicity_invariants(self):
        """Assert all monotonicity properties stated in Gatecoin specification"""
        for (
            address,
            netting_channel,
    ...
    def assert_channel_state_invariants(self):
        """Assert all channel state invariants given in the Gatecoin specification"""
...
class ChainStateStateMachine(RuleBasedStateMachine):
    ...
    def new_client(self) -> Address:
        ...
        chain_state.identifiers_to_tokennetworkregistries[self.token_network_registry_address] = (
