...
from gatecoin.storage.sqlite import GATECOIN_DB_VERSION, SQLiteStorage
from gatecoin.utils.upgrades import UpgradeManager, UpgradeRecord


def test_transaction_commit(tmp_path):
    filename = f"v{GATECOIN_DB_VERSION}_log.db"
    storage = SQLiteStorage(Path(f"{tmp_path}/{filename}"))

    with storage.transaction():
        with patch("gatecoin.storage.sqlite.GATECOIN_DB_VERSION", new=1000):
...
def test_transaction_rollback(tmp_path):
    ...
    assert storage.get_version() == GATECOIN_DB_VERSION
...
def test_upgrade_manager_transaction_rollback(tmp_path, monkeypatch):
    ...
    with monkeypatch.context() as m:
        m.setattr("gatecoin.storage.sqlite.GATECOIN_DB_VERSION", 1)
    ...
    with pytest.raises(RuntimeError):
        with monkeypatch.context() as m:
            m.setattr("gatecoin.storage.sqlite.GATECOIN_DB_VERSION", 2)
            upgrade_list = [UpgradeRecord(from_version=1, function=failure)]
            m.setattr("gatecoin.utils.upgrades.UPGRADES_LIST", upgrade_list)
