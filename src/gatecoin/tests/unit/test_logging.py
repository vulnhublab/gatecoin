...
from gatecoin.exceptions import ConfigurationError
from gatecoin.log_config import LogFilter, configure_logging
...
def test_log_filter():
    ...
    assert filter_.should_log("gatecoin", "DEBUG") is False
    assert filter_.should_log("gatecoin", "INFO") is True
    assert filter_.should_log("gatecoin.cli", "DEBUG") is False
    assert filter_.should_log("gatecoin.cli", "INFO") is True
    ...
    assert filter_.should_log("gatecoin", "INFO") is False
    assert filter_.should_log("gatecoin", "WARN") is True
    assert filter_.should_log("gatecoin.cli", "INFO") is False
    assert filter_.should_log("gatecoin.cli", "WARN") is True
    ...
    rules = {"gatecoin": "DEBUG"}
    filter_ = LogFilter(rules, default_level="INFO")

    assert filter_.should_log("test", "DEBUG") is False
    assert filter_.should_log("test", "INFO") is True
    assert filter_.should_log("gatecoin", "DEBUG") is True
    assert filter_.should_log("gatecoin.cli", "DEBUG") is True

    rules = {"gatecoin.network": "DEBUG"}
    filter_ = LogFilter(rules, default_level="INFO")
    ...
    assert filter_.should_log("gatecoin", "DEBUG") is False
    assert filter_.should_log("gatecoin", "INFO") is True
    assert filter_.should_log("gatecoin.network", "DEBUG") is True

    rules = {
        "": "WARN",
        "gatecoin": "DEBUG",
        "gatecoin.network": "INFO",
        "gatecoin.network.transport": "DEBUG",
    }
    filter_ = LogFilter(rules, default_level="INFO")

    assert filter_.should_log("gatecoin.network.transport.matrix", "DEBUG") is True
    assert filter_.should_log("gatecoin.network.transport", "DEBUG") is True
    assert filter_.should_log("gatecoin.network", "DEBUG") is False
    assert filter_.should_log("gatecoin.network", "INFO") is True
    assert filter_.should_log("gatecoin.network", "INFO") is True
    assert filter_.should_log("gatecoin", "DEBUG") is True
...
@pytest.mark.parametrize("module", ["", "gatecoin", "gatecoin.network"])
@pytest.mark.parametrize("level", ["DEBUG", "WARNING"])
@pytest.mark.parametrize("logger", ["test", "gatecoin", "gatecoin.network"])
...
def test_basic_logging(capsys, module, level, logger, disabled_debug, tmpdir):
    configure_logging(
        ...
        debug_log_file_path=str(tmpdir / "gatecoin-debug.log"),
        colorize=False,
    )
...
def test_debug_logfile_invalid_dir():
    ...
    with pytest.raises(ConfigurationError):
        configure_logging(
            {"": "DEBUG"}, debug_log_file_path=os.path.join("notarealdir", "gatecoin-debug.log")
        )


def test_redacted_request(capsys, tmpdir):
    configure_logging({"": "DEBUG"}, debug_log_file_path=str(tmpdir / "gatecoin-debug.log"))
...
def test_that_secret_is_redacted(capsys, tmpdir):
    ...
    log = structlog.get_logger("gatecoin.network.transport.matrix.transport")
