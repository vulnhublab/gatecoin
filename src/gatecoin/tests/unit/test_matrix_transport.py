...
import gatecoin.network.transport.matrix.client
import gatecoin.network.transport.matrix.utils
from gatecoin.constants import DeviceIDs, ServerListType
from gatecoin.exceptions import TransportError
from gatecoin.messages.synchronization import Processed
from gatecoin.messages.transfers import RevealSecret
...
from gatecoin.tests.utils.factories import make_secret, make_signature, make_signer
...
def test_login_for_the_first_time_must_set_display_name_and_avatar_url():
    ...
    user = login(
        client=client, signer=signer, device_id=DeviceIDs.GATECOIN, capabilities={"test": 1}
    )

    # avatar_url must have been set
    user.set_avatar_url.assert_called_once_with("mxc://gatecoin.network/cap?test=1")
...
@pytest.mark.skip(reason="flaky, see https://github.com/gatecoin/gatecoin/issues/5678")
...
def test_make_client(monkeypatch):
    ...
    with pytest.raises(TransportError), monkeypatch.context() as m:
        mock_get_http_rtt = Mock(
            spec=gatecoin.network.transport.matrix.utils.get_average_http_response_time,
    ...
    with monkeypatch.context() as m:
        m.setattr(gatecoin.network.transport.matrix.client.GMatrixHttpApi, "_send", mock_send)

        url = "https://server1.xyz"
        client = make_client(ignore_messages, [url])
        assert isinstance(client, gatecoin.network.transport.matrix.client.GMatrixClient)
