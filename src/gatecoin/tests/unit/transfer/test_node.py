...
from gatecoin.settings import GAS_LIMIT
from gatecoin.tests.unit.channel_state.utils import create_channel_from_models, create_model
...
from gatecoin.transfer.identifiers import (
    CANONICAL_IDENTIFIER_UNORDERED_QUEUE,
    CanonicalIdentifier,
    QueueIdentifier,
)
from gatecoin.transfer.mediated_transfer.state import TargetTransferState
from gatecoin.transfer.mediated_transfer.state_change import ReceiveLockExpired
from gatecoin.transfer.mediated_transfer.tasks import TargetTask
from gatecoin.transfer.node import (
...
from gatecoin.transfer.views import get_networks
...
def test_subdispatch_by_canonical_id(chain_state):
    ...
    token_network.partneraddresses_to_channelidentifiers[partner_model.participant_address] = (
        canonical_identifier.channel_identifier
    )
    token_network.channelidentifiers_to_channels[canonical_identifier.channel_identifier] = (
        channel_state
    )
    token_network_registry = TokenNetworkRegistryState(
        address=factories.make_address(), token_network_list=[token_network]
    )
    chain_state.identifiers_to_tokennetworkregistries[token_network_registry.address] = (
