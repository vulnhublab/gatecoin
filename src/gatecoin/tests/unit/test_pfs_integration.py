...
from gatecoin.api.v1.encoding import CapabilitiesSchema
from gatecoin.constants import RoutingMode
from gatecoin.exceptions import ServiceRequestFailed, ServiceRequestIOURejected
from gatecoin.network import pathfinding
from gatecoin.network.pathfinding import (
    IOU,
...
from gatecoin.network.transport.matrix.utils import make_user_id
from gatecoin.routing import get_best_routes, make_route_state
from gatecoin.settings import CapabilitiesConfig
from gatecoin.tests.utils import factories
from gatecoin.tests.utils.mocks import mocked_failed_response, mocked_json_response
...
def assert_failed_pfs_request(
    ...
    pfs_proxy = PFSProxy(PFS_CONFIG)
    with patch("gatecoin.network.pathfinding.get_pfs_info") as mocked_pfs_info:
...
def test_routing_in_direct_channel(happy_path_fixture, our_signer, one_to_n_address):
    ...
    with patch("gatecoin.routing.get_best_routes_pfs") as pfs_route_request, patch.object(
    ...
    with patch("gatecoin.routing.get_best_routes_pfs") as pfs_request:
...
def test_insufficient_payment(query_paths_args, valid_response_json):
    ...
    with patch("gatecoin.network.pathfinding.get_pfs_info", Mock(return_value=new_pfs_info)):
        assert_failed_pfs_request(
            query_paths_args,
    ...
    with patch("gatecoin.network.pathfinding.get_pfs_info") as mocked_pfs_info:
        ...
        with patch.object(pathfinding, "create_current_iou"):
            ...
            with patch.object(
                ...
                # the other. If semaphore in gatecoin.network.pathfinding is bound to 2,
                # the test fails
                assert duration >= 0.4
...
def test_make_route_state_address_to_metadata_serialization_regression():
    ...
    See: https://github.com/gatecoin/gatecoin/issues/6943"""
    ...
    with patch(
        "gatecoin.transfer.views.get_channelstate_by_token_network_and_partner"
    ) as mocked_get_channelstate, patch("gatecoin.transfer.channel.get_status") as get_status:
