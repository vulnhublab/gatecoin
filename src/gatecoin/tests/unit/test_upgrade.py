...
import gatecoin.utils.upgrades
from gatecoin.storage.serialization import JSONSerializer
from gatecoin.storage.sqlite import FilteredDBQuery, Operator, SQLiteStorage
from gatecoin.tests.utils import factories
from gatecoin.tests.utils.migrations import create_fake_web3_for_block_hash
from gatecoin.transfer.state_change import Block
from gatecoin.utils.typing import BlockGasLimit
from gatecoin.utils.upgrades import VERSION_RE, UpgradeManager, UpgradeRecord, get_db_version
...
def test_no_upgrade_executes_if_already_upgraded(tmp_path):
    ...
    for version in [16, 17, 18, 19]:
        ...
        with patch("gatecoin.storage.sqlite.GATECOIN_DB_VERSION", new=version), SQLiteStorage(
            str(old_db_filename)
        ) as storage:
    ...
    with patch("gatecoin.utils.upgrades.UpgradeManager._upgrade") as upgrade_mock:
        with patch("gatecoin.utils.upgrades.GATECOIN_DB_VERSION", new=version):
...
def test_upgrade_executes_necessary_migration_functions(tmp_path, monkeypatch):
    ...
    with patch("gatecoin.storage.sqlite.GATECOIN_DB_VERSION", new=18), SQLiteStorage(
        old_db_filename
    ) as storage:
        storage.update_version()
    ...
    with monkeypatch.context() as m:
        m.setattr(gatecoin.utils.upgrades, "UPGRADES_LIST", upgrade_functions)
        m.setattr(gatecoin.utils.upgrades, "GATECOIN_DB_VERSION", 19)
...
def test_upgrade_manager_restores_backup(tmp_path, monkeypatch):
    ...
    with patch("gatecoin.storage.sqlite.GATECOIN_DB_VERSION", new=16), SQLiteStorage(
        str(old_db_filename)
    ) as storage:
        state_change = Block(
    ...
    with SQLiteStorage(str(db_path)) as storage:
        state_change_record = storage.get_latest_state_change_by_data_field(
            FilteredDBQuery(
                filters=[{"_type": "gatecoin.transfer.state_change.Block"}],
...
def test_sequential_version_numbers(tmp_path, monkeypatch):
    ...
    with monkeypatch.context() as m:
        ...
        m.setattr(gatecoin.utils.upgrades, "UPGRADES_LIST", upgrade_functions)
        m.setattr(gatecoin.utils.upgrades, "GATECOIN_DB_VERSION", 19)
        m.setattr(gatecoin.utils.upgrades, "latest_db_file", latest_db_file)
