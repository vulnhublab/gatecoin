...
from gatecoin.storage.versions import VERSION_RE
from gatecoin.utils.upgrades import delete_dbs_with_failed_migrations
...
def test_delete_dbs_with_failed_migrations(monkeypatch):
    ...
    monkeypatch.setattr("gatecoin.utils.upgrades.get_file_lock", GetLockMock)

    with monkeypatch.context() as m:
        remove_mock = MagicMock()

        m.setattr("gatecoin.utils.upgrades.get_db_version", _return_valid_db_version)
        m.setattr("gatecoin.utils.upgrades.os.path.exists", exists_mock)
        m.setattr("gatecoin.utils.upgrades.os.remove", remove_mock)
    ...
    with monkeypatch.context() as m:
        ...
        m.setattr("gatecoin.utils.upgrades.get_db_version", _return_higher_db_version)
        m.setattr("gatecoin.utils.upgrades.os.path.exists", exists_mock)
        m.setattr("gatecoin.utils.upgrades.os.remove", remove_mock)
    ...
    with monkeypatch.context() as m:
        ...
        m.setattr("gatecoin.utils.upgrades.get_db_version", _return_smaller_db_version)
        m.setattr("gatecoin.utils.upgrades.os.path.exists", exists_mock)
        m.setattr("gatecoin.utils.upgrades.os.remove", remove_mock)
