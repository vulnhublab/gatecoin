...
from gatecoin.constants import Environment, EthClient, RoutingMode
...
from gatecoin.network.rpc.middleware import faster_gas_price_strategy
from gatecoin.tests.utils.cli import assert_invoked_kwargs, get_cli_result, get_invoked_kwargs
from gatecoin.ui import cli
from gatecoin.ui.cli import ReturnCode
from gatecoin.utils.ethereum_clients import VersionSupport, is_supported_client
...
def test_cli_version_short(cli_runner, arg_list):
    ...
    expected_short_version = get_system_spec()["gatecoin"]
    assert version == expected_short_version
    assert result.exit_code == 0


def test_gatecoin_read_config(tmp_path, cli_runner):
    ...
    config = """
        ...
        [log-config]
        "" = "DEBUG"
        "gatecoin.network" = "INFO"
        "gatecoin.transfer" = "WARNING"
        """

    # Config file should exist at the default location (~/.gatecoin/config.toml)
    datadir = tmp_path / ".gatecoin"
    datadir.mkdir(parents=True, exist_ok=True)
    config_file = datadir / "config.toml"

    config_file.write_text(config)

    cli_command = "gatecoin --log-config gatecoin.transfer:INFO"
    ...
    expected_args = {
        ...
        "log_config": (
            ...
            {"": "DEBUG", "gatecoin.network": "INFO", "gatecoin.transfer": "INFO"},
        ),
    ...
    _, kwargs = get_invoked_kwargs(cli_command, cli_runner, "gatecoin.ui.cli._run")
    assert_invoked_kwargs(kwargs, expected_args)


def test_gatecoin_defaults(cli_runner, tmp_path):
...
def test_gatecoin_disable_on_no_rpc(cli_runner):

    cli_command = "gatecoin --no-rpc --web-ui --accept-disclaimer"
    ...
    result = get_cli_result(cli_command, cli_runner, "gatecoin.ui.cli.run_services")
    expected_output = (
    ...
    # config)
    args, _ = get_invoked_kwargs(cli_command, cli_runner, "gatecoin.ui.cli.run_services")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "gatecoin --no-enable-monitoring --network-id 1 --accept-disclaimer",
            {
...
def test_no_monitoring_mainnet_warning(cli_runner, cli_command, expected_kwargs):
    ...
    cli_command = "gatecoin --accept-disclaimer --network-id 42"
...
def test_smoketest_defaults(cli_runner):
    ...
    cli_command = "gatecoin smoketest"
    ...
    _, kwargs = get_invoked_kwargs(cli_command, cli_runner, "gatecoin.ui.cli._smoketest")
    assert_invoked_kwargs(kwargs, expected_args)
...
def test_smoketest(cli_runner):
    ...
    # check debug flag
    cli_command = "gatecoin smoketest --debug"
...
def test_parent_for_subcommand(cli_runner):
    ...
    cli_command = "gatecoin --environment-type development smoketest"
    _, kwargs = get_invoked_kwargs(cli_command, cli_runner, "gatecoin.ui.cli._smoketest")
...
def test_run_error_reporting(cli_runner, monkeypatch):
    caught_exceptions = {
        ...
        ConfigurationError(): ReturnCode.GATECOIN_CONFIGURATION_ERROR,
        ...
        GatecoinUnrecoverableError(): ReturnCode.FATAL,
...
def test_check_json_rpc_geth():
    ...
    with patch(
        "gatecoin.utils.ethereum_clients.HIGHEST_SUPPORTED_GETH_VERSION", new="1.9.2"
    ), patch("gatecoin.utils.ethereum_clients.LOWEST_SUPPORTED_GETH_VERSION", new="1.7.2"):
...
def test_check_json_rpc_parity():
    # Pin the highest supported version for the test purposes
    with patch("gatecoin.utils.ethereum_clients.HIGHEST_SUPPORTED_PARITY_VERSION", new="2.5.5"):
