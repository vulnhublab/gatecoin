...
from gatecoin.messages.transfers import Lock
...
from gatecoin.storage.sqlite import (
...
from gatecoin.transfer.state import BalanceProofUnsignedState, HopState, RouteState
from gatecoin.transfer.state_change import Block, ReceiveUnlock
...
def test_get_event_with_balance_proof():
    ...
    for event, balance_proof in events_balanceproofs:
        ...
        # Issue https://github.com/gatecoin/gatecoin/issues/3179
...
def test_log_run():
    with patch("gatecoin.storage.sqlite.get_system_spec") as get_speck_mock:
        get_speck_mock.return_value = dict(gatecoin="1.2.3")
        serializer = JSONSerializer()
        store = SerializedSQLiteStorage(":memory:", serializer)
        store.log_run()
    cursor = store.database.conn.cursor()
    cursor.execute("SELECT started_at, gatecoin_version FROM runs")
    run = cursor.fetchone()
    now = datetime.utcnow()
...
def test_batch_query_state_changes():
    ...
    state_changes_batch_query = storage.batch_query_state_changes(
        batch_size=10, filters=[("_type", "gatecoin.transfer.state_change.Block")]
    )
    ...
    state_changes_batch_query = storage.batch_query_state_changes(
        ...
        filters=[
            # Should be 5 of them
            ("_type", "gatecoin.transfer.state_change.ContractReceiveChannel%"),
            # Should be only 1
            ("_type", "gatecoin.transfer.state_change.ContractReceiveNewTokenNetworkRegistry"),
        ],
        logical_and=False,
    )
    for state_changes_batch in state_changes_batch_query:
...
def test_batch_query_event_records():
    ...
    events_batch_query = storage.batch_query_event_records(
        batch_size=1, filters=[("_type", "gatecoin.transfer.events.EventPaymentReceivedSuccess")]
    )
    for events_batch in events_batch_query:
        events.extend(events_batch)
    assert len(events) == 1
    event_type = json.loads(events[0].data)["_type"]
    assert event_type == "gatecoin.transfer.events.EventPaymentReceivedSuccess"
    ...
    events_batch_query = storage.batch_query_event_records(
        batch_size=1,
        filters=[
            ("_type", "gatecoin.transfer.events.EventPaymentReceivedSuccess"),
            ("_type", "gatecoin.transfer.events.ContractSendChannelSettle"),
        ],
        logical_and=False,
    )
    for events_batch in events_batch_query:
