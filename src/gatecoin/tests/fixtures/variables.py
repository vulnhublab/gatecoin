...
from gatecoin.constants import Environment, EthClient
from gatecoin.network.utils import get_free_port
from gatecoin.settings import DEFAULT_NUMBER_OF_BLOCK_CONFIRMATIONS, DEFAULT_RETRY_TIMEOUT
from gatecoin.tests.fixtures.constants import DEFAULT_BALANCE
from gatecoin.tests.utils.ci import shortened_artifacts_storage
from gatecoin.tests.utils.eth_node import EthNodeDescription
from gatecoin.tests.utils.factories import UNIT_CHAIN_ID
from gatecoin.tests.utils.tests import unique_path
from gatecoin.utils.typing import Iterator, Port, TokenAmount
from gatecoin_contracts.constants import TEST_SETTLE_TIMEOUT_MAX, TEST_SETTLE_TIMEOUT_MIN
...
def deposit() -> TokenAmount:
    """Gatecoin chain default deposit."""
...
def register_tokens() -> bool:
    """Should fixture generated tokens be registered with gatecoin."""
    return True


@pytest.fixture
def number_of_nodes() -> int:
    """Number of gatecoin nodes in the test network."""
    return 3


@pytest.fixture
def channels_per_node() -> int:
    """Number of pre-created channels per test gatecoin node."""
    return 1
...
def private_keys(number_of_nodes, privatekey_seed):
    """Private keys for each gatecoin node."""
...
def blockchain_number_of_nodes() -> int:
    """Number of nodes in the cluster, not the same as the number of gatecoin
...
@pytest.fixture
def start_gatecoin_apps() -> bool:
    """Determines if the gatecoin apps created at test setup should also be started"""
    return True
