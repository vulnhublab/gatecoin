...
from gatecoin.network.rpc.client import JSONRPCClient, make_sane_poa_middleware
from gatecoin.settings import DEFAULT_NUMBER_OF_BLOCK_CONFIRMATIONS, GATECOIN_CONTRACT_VERSION
from gatecoin.tests.fixtures.constants import DEFAULT_BALANCE, DEFAULT_PASSPHRASE
from gatecoin.tests.utils.eth_node import (
    AccountDescription,
    EthNodeDescription,
    GenesisDescription,
    eth_node_to_datadir,
...
from gatecoin.tests.utils.smartcontracts import deploy_token, is_tx_hash_bytes
from gatecoin.tests.utils.transport import make_requests_insecure
from gatecoin.transfer import channel, views
from gatecoin.transfer.state import ChannelState
from gatecoin.ui.app import run_gatecoin_service
from gatecoin.utils.formatting import to_checksum_address
from gatecoin.utils.http import HTTPExecutor, split_endpoint
...
from gatecoin_contracts.deploy.contract_deployer import ContractDeployer


if TYPE_CHECKING:
    # pylint: disable=unused-import
    from gatecoin.tests.utils.transport import ParsedURL  # noqa: F401
...
def setup_matrix_for_smoketest(
    ...
    from gatecoin.tests.utils.transport import matrix_server_starter
...
class GatecoinTestSetup(NamedTuple):
    args: Dict[str, Any]
    token: Contract
...
def setup_gatecoin(
    ...
    print_step("Deploying Gatecoin contracts")
    ...
    print_step("Setting up Gatecoin")
    ...
    # Wait until the secret registry is confirmed, otherwise the GatecoinService
...
def run_smoketest(print_step: StepPrinter, setup: GatecoinTestSetup) -> None:
    print_step("Starting Gatecoin")

    app = None
    try:
        app = run_gatecoin_service(**setup.args)
        gatecoin_api = app.gatecoin_api
        assert gatecoin_api is not None  # for mypy
        ...
        wait_for_block(gatecoin=app, block_number=block, retry_timeout=1.0)

        gatecoin_api.channel_open(
        ...
        gatecoin_api.set_total_channel_deposit(
            registry_address=TokenNetworkRegistryAddress(
                setup.contract_addresses[CONTRACT_TOKEN_NETWORK_REGISTRY]
            ),
            token_address=TokenAddress(to_canonical_address(setup.token.address)),
        ...
        gatecoin_service = app
        token_network_added_events = gatecoin_service.default_registry.filter_token_added_events()
        ...
        token_networks = views.get_token_identifiers(
            views.state_from_gatecoin(gatecoin_service), gatecoin_service.default_registry.address
        )
        assert len(token_networks) == 1

        channel_state = views.get_channelstate_for(
            chain_state=views.state_from_gatecoin(gatecoin_service),
            token_network_registry_address=gatecoin_service.default_registry.address,
        ...
        port_number = gatecoin_service.config.rest_api.port
...
def setup_smoketest(
    *,
    ...
    with stdout_manager, testchain_manager as testchain, matrix_manager as server_urls:
        ...
        ethereum_nodes = None
        try:
            gatecoin_setup = setup_gatecoin(
                matrix_server=server_urls[0][0],
                print_step=print_step,
                contracts_version=GATECOIN_CONTRACT_VERSION,
