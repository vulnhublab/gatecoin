"""Utilities to set-up a Gatecoin network."""
...
from gatecoin.constants import BLOCK_ID_LATEST, GENESIS_BLOCK_NUMBER, Environment, RoutingMode
from gatecoin.exceptions import PFSReturnedError
from gatecoin.network.pathfinding import PFSProxy
from gatecoin.network.proxies.proxy_manager import ProxyManager, ProxyManagerMetadata
...
from gatecoin.tests.utils.app import database_from_privatekey
from gatecoin.tests.utils.factories import UNIT_CHAIN_ID
from gatecoin.tests.utils.protocol import HoldGatecoinEventHandler, WaitForMessage
from gatecoin.tests.utils.transport import ParsedURL, TestMatrixTransport
...
from gatecoin.ui.app import start_api_server
from gatecoin.ui.startup import GatecoinBundle, ServicesBundle
from gatecoin.utils.formatting import to_checksum_address, to_hex_address
...
from gatecoin.waiting import wait_for_token_network
from gatecoin_contracts.contract_manager import ContractManager

AppChannels = Iterable[Tuple[GatecoinService, GatecoinService]]
...
def check_channel(
    ...
    channel_state2 = get_channelstate_by_token_network_and_partner(
        chain_state=state_from_gatecoin(app2),
        token_network_address=token_network_address,
...
def payment_channel_open_and_deposit(
    ...
    if deposit != 0:
        for app, partner in [(app0, app1), (app1, app0)]:
            ...
            chain_state = state_from_gatecoin(app)
...
def network_with_minimum_channels(
    apps: List[GatecoinService], channels_per_node: int
) -> AppChannels:
...
def create_network_channels(
    gatecoin_apps: List[GatecoinService], channels_per_node: int
) -> AppChannels:
    app_channels: AppChannels

    num_nodes = len(gatecoin_apps)
    ...
    if channels_per_node == 0:
        ...
        app_channels = list(zip(gatecoin_apps[:-1], gatecoin_apps[1:]))
    else:
        app_channels = list(network_with_minimum_channels(gatecoin_apps, channels_per_node))
...
def create_sequential_channels(
    ...
    if channels_per_node == 1:
        assert len(gatecoin_apps) % 2 == 0, "needs an even number of nodes"
        every_two = iter(gatecoin_apps)
        app_channels = list(zip(every_two, every_two))

    if channels_per_node == 2:
        app_channels = list(zip(gatecoin_apps, gatecoin_apps[1:] + [gatecoin_apps[0]]))
...
def create_apps(
    ...
    for idx, proxy_manager in enumerate(blockchain_services):
        ...
        app = GatecoinService(
            ...
            gatecoin_bundle=GatecoinBundle(registry, secret_registry),
...
def parallel_start_apps(gatecoin_apps: List[GatecoinService]) -> None:
    """Start all the gatecoin apps in parallel."""
    start_tasks = set()

    for app in gatecoin_apps:
        greenlet = gevent.spawn(app.start)
        greenlet.name = f"Fixture:gatecoin_network node:{to_checksum_address(app.address)}"
        start_tasks.add(greenlet)

    gevent.joinall(start_tasks, raise_error=True)

    addresses_in_order = {
        pos: to_hex_address(app.address) for pos, app in enumerate(gatecoin_apps)
    }
    log.info("Gatecoin Apps started", addresses_in_order=addresses_in_order)
...
def wait_for_alarm_start(
    gatecoin_apps: List[GatecoinService], retry_timeout: float = DEFAULT_RETRY_TIMEOUT
) -> None:
    """Wait until all Alarm tasks start & set up the last_block"""
    apps = list(gatecoin_apps)
...
def wait_for_usable_channel(
    ...
    waiting.wait_for_participant_deposit(
        ...
        token_address=token_address,
        partner_address=partner_address,
        target_address=gatecoin.address,
...
def wait_for_token_networks(
    gatecoin_apps: List[GatecoinService],
    token_network_registry_address: TokenNetworkRegistryAddress,
    token_addresses: List[TokenAddress],
    retry_timeout: float = DEFAULT_RETRY_TIMEOUT,
) -> None:
