...
from gatecoin.messages.abstract import SignedMessage
from gatecoin.messages.decode import balanceproof_from_envelope
from gatecoin.messages.metadata import Metadata, RouteMetadata
from gatecoin.messages.transfers import Lock, LockedTransfer, LockExpired, Unlock
...
from gatecoin.storage.wal import SavedState, WriteAheadLog
from gatecoin.tests.utils.events import has_unlock_failure, gatecoin_state_changes_search_for_item
...
from gatecoin.transfer.architecture import TransitionResult
...
from gatecoin.transfer.mediated_transfer.state import LockedTransferSignedState
...
from gatecoin.transfer.state_change import ContractReceiveChannelDeposit, ReceiveUnlock
from gatecoin.utils.formatting import to_checksum_address
from gatecoin.utils.signer import LocalSigner, Signer
from gatecoin.utils.timeout import BlockTimeout
...
def sign_and_inject(message: SignedMessage, signer: Signer, app: GatecoinService) -> None:
...
def get_channelstate(
    app0: GatecoinService, app1: GatecoinService, token_network_address: TokenNetworkAddress
) -> NettingChannelState:
    channel_state = views.get_channelstate_by_token_network_and_partner(
        views.state_from_gatecoin(app0), token_network_address, app1.address
    )
...
def create_route_state_for_route(
    apps: List[GatecoinService], token_address: TokenAddress, fee_estimate: FeeAmount = None
) -> RouteState:
    ...
    token_network = views.get_token_network_by_token_address(
        views.state_from_gatecoin(apps[0]),
...
@contextmanager
def patch_transfer_routes(routes: List[List[GatecoinService]], token_address: TokenAddress):
    """
...
def watch_for_unlock_failures(*apps):
    ...
    for app in apps:
        app.gatecoin_event_handler.pre_hooks.add(check)

    try:
        yield
    finally:
        for app in apps:
            app.gatecoin_event_handler.pre_hooks.remove(check)
        assert failed_event is None, f"Unexpected unlock failure: {str(failed_event)}"


def transfer(
    initiator_app: GatecoinService,
    target_app: GatecoinService,
    ...
    routes: List[List[GatecoinService]] = None,
) -> SecretHash:
...
def _transfer_unlocked(
    ...
    token_network_address = views.get_token_network_address_by_token_address(
        chain_state=views.state_from_gatecoin(initiator_app),
...
def _transfer_secret_not_requested(
    ...
    assert isinstance(target_app.gatecoin_event_handler, HoldGatecoinEventHandler)
    hold_secret_request = target_app.gatecoin_event_handler.hold(
...
def transfer_and_assert_path(
    ...
    token_network_address = views.get_token_network_address_by_token_address(
        chain_state=views.state_from_gatecoin(first_app),
        token_network_registry_address=token_network_registry_address,
        token_address=token_address,
    )
    assert token_network_address
    ...
    for app in path:
        ...
        app_token_network_address = views.get_token_network_address_by_token_address(
            chain_state=views.state_from_gatecoin(app),
            token_network_registry_address=token_network_registry_address,
            token_address=token_address,
        )
...
def assert_deposit(
    ...
    app0: GatecoinService,
    app1: GatecoinService,
    ...
    if channel0.our_state.contract_balance != channel1.partner_state.contract_balance:
        ...
        node0_deposit_event = gatecoin_state_changes_search_for_item(
            app0, ContractReceiveChannelDeposit, deposit_description
        )
        node1_deposit_event = gatecoin_state_changes_search_for_item(
        ...
        if received_balance_proof is not None:
            ...
            if not is_valid:
                msg = (
                    ...
                    f"is likely a Gatecoin bug. reason={innermsg} "
...
def block_offset_timeout(
    ...
    expiration = BlockNumber(
        gatecoin.get_block_number() + (offset or gatecoin.config.settle_timeout) + safety_margin
    )
...
def block_timeout_for_transfer_by_secrethash(
    gatecoin: GatecoinService, secrethash: SecretHash, error_message: str = None
) -> BlockTimeout:
    """
    ...
    assert isinstance(gatecoin.wal, WriteAheadLog)
    state_change = get_state_change_with_transfer_by_secrethash(gatecoin.wal.storage, secrethash)
