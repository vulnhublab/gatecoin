...
from gatecoin.tests.utils.events import check_nested_attrs
from gatecoin.transfer.architecture import Event as GatecoinEvent
...
from gatecoin.utils.typing import Callable, Dict, List, NamedTuple, SecretHash, Set
...
class WaitForMessage(MessageHandler):
    ...
    def on_messages(self, gatecoin: GatecoinService, messages: List[Message]) -> None:
        ...
        super().on_messages(gatecoin, messages)
...
class HoldGatecoinEventHandler(EventHandler):
    ...
    def on_gatecoin_events(
        self, gatecoin: GatecoinService, chain_state: ChainState, events: List[GatecoinEvent]
    ):
        events_to_dispatch = []
        ...
        if events_to_dispatch:
            self.wrapped.on_gatecoin_events(gatecoin, chain_state, events_to_dispatch)
    ...
    def release(self, gatecoin: GatecoinService, event: GatecoinEvent):
        ...
        self.wrapped.on_gatecoin_events(gatecoin, hold.chain_state, [event])
        log.debug(f"{event} released.", node=to_checksum_address(gatecoin.address))
    ...
    def release_secretreveal_for(self, gatecoin: GatecoinService, secrethash: SecretHash):
        for hold in self.eventtype_to_holdings[SendSecretReveal]:
            if hold.attributes["secrethash"] == secrethash:
                self.release(gatecoin, hold.event)

    def release_secretrequest_for(self, gatecoin: GatecoinService, secrethash: SecretHash):
        for hold in self.eventtype_to_holdings[SendSecretRequest]:
            if hold.attributes["secrethash"] == secrethash:
                self.release(gatecoin, hold.event)

    def release_unlock_for(self, gatecoin: GatecoinService, secrethash: SecretHash):
...
def dont_handle_lock_expired_mock(app):
    """Takes in a gatecoin app and returns a mock context where lock_expired is not processed"""

    def do_nothing(gatecoin, message):  # pylint: disable=unused-argument
        return []
