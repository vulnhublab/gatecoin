...
from gatecoin.transfer.architecture import Event, StateChange
from gatecoin.transfer.mediated_transfer.events import EventUnlockClaimFailed, EventUnlockFailed
...
def gatecoin_events_search_for_item(
    gatecoin: GatecoinService, item_type: Type[TE], attributes: Mapping
) -> Optional[TE]:
    ...
    `attributes` are compared using the utility `check_nested_attrs`.
    """
    assert gatecoin.wal, "GatecoinService must be started"
    event = search_for_item(gatecoin.wal.storage.get_events(), item_type, attributes)
    return cast(TE, event)


def gatecoin_state_changes_search_for_item(
    gatecoin: GatecoinService, item_type: Type[SC], attributes: Mapping
) -> Optional[SC]:
    """Search for the first event of type `item_type` with `attributes` in the
    `gatecoin` database.
    ...
    for item in gatecoin.wal.storage.get_statechanges_by_range(RANGE_ALL_STATE_CHANGES):
        if isinstance(item, item_type) and check_nested_attrs(item, attributes):
            return item
...
def has_unlock_failure(gatecoin: GatecoinService, offset: int = 0) -> bool:
    return count_unlock_failures(gatecoin.wal.storage.get_events()) > offset  # type: ignore


def wait_for_gatecoin_event(
    gatecoin: GatecoinService, item_type: Type[Event], attributes: Mapping, retry_timeout: float
) -> Optional[Event]:
    ...
    found = None
    while found is None:
        found = gatecoin_events_search_for_item(gatecoin, item_type, attributes)
        gevent.sleep(retry_timeout)
    return found


def wait_for_state_change(
    gatecoin: GatecoinService, item_type: Type[SC], attributes: Mapping, retry_timeout: float
) -> SC:
    ...
    found = None
    while found is None:
        found = gatecoin_state_changes_search_for_item(gatecoin, item_type, attributes)
        gevent.sleep(retry_timeout)
