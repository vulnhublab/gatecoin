...
from gatecoin.constants import DeviceIDs, Environment
from gatecoin.messages.abstract import Message
from gatecoin.network.transport.matrix.client import GMatrixClient, MatrixMessage
...
from gatecoin.tests.utils.factories import make_signer
from gatecoin.transfer.identifiers import QueueIdentifier
from gatecoin.utils.http import EXECUTOR_IO, HTTPExecutor
from gatecoin.utils.signer import recover
from gatecoin.utils.typing import Iterable, Port
from gatecoin_contracts.utils.type_aliases import Signature

log = get_logger(__name__)

_SYNAPSE_BASE_DIR_VAR_NAME = "GATECOIN_TESTS_SYNAPSE_BASE_DIR"
_SYNAPSE_LOGS_PATH = os.environ.get("GATECOIN_TESTS_SYNAPSE_LOGS_DIR")
...
class EthAuthProvider:
    ...
    async def check_password(self, user_id: str, password: str) -> bool:
        ...
        if not (await self.account_handler.check_user_exists(user_id)):
            ...
            await self.account_handler.register_device(registered_user_id, device_id="GATECOIN")
