...
from gatecoin.constants import Environment, MatrixMessageType
from gatecoin.messages.transfers import SecretRequest
from gatecoin.network.transport import MatrixTransport
from gatecoin.network.transport.matrix.client import GMatrixHttpApi
from gatecoin.network.transport.matrix.rtc.web_rtc import WebRTCManager
from gatecoin.network.transport.matrix.transport import RETRY_QUEUE_IDLE_AFTER, _RetryQueue
...
from gatecoin.tests.utils.factories import make_message_identifier, make_signer
...
from gatecoin.utils.formatting import to_hex_address
...
def skip_userid_validation(monkeypatch):
    import gatecoin.network.transport.matrix
    import gatecoin.network.transport.matrix.utils

    def mock_validate_userid_signature(user):  # pylint: disable=unused-argument
        return factories.HOP1

    monkeypatch.setattr(
        gatecoin.network.transport.matrix,
        "validate_userid_signature",
        mock_validate_userid_signature,
    )
    monkeypatch.setattr(
        gatecoin.network.transport.matrix.transport,
        "validate_userid_signature",
        mock_validate_userid_signature,
    )
    monkeypatch.setattr(
        gatecoin.network.transport.matrix.utils,
        "validate_userid_signature",
        mock_validate_userid_signature,
    )
...
@pytest.fixture()
def mock_gatecoin_service():
    return MockGatecoinService()


@pytest.fixture()
def mock_matrix(
    monkeypatch,
    mock_gatecoin_service,
    retry_interval_initial,
    retry_interval_max,
    retries_before_backoff,
):

    from gatecoin.network.transport.matrix import transport as transport_module
    from gatecoin.network.transport.matrix.client import GMatrixClient
    from gatecoin.network.transport.matrix.utils import UserPresence
    ...
    transport._gatecoin_service = mock_gatecoin_service
    transport._web_rtc_manager = WebRTCManager(
        mock_gatecoin_service.address,
        transport._process_gatecoin_messages,
        transport._send_raw,
    ...
    monkeypatch.setattr(transport._gatecoin_service, "on_messages", mock_on_messages)
...
def test_retry_queue_does_not_resend_removed_messages(
    ...
    Messages should only be retried while they are present in the respective Gatecoin queue.
    Once they have been removed they should not be sent again.

    In the past they could have been sent twice.
    See: https://github.com/gatecoin/gatecoin/issue/4111
    """
    # Pretend the Transport greenlet is running
    mock_matrix.greenlet = True
    ...
    # The message has been removed from the gatecoin queue and should therefore not be sent again
    retry_queue._check_and_send()
