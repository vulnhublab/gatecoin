...
from gatecoin.constants import BLOCK_ID_LATEST, EMPTY_SIGNATURE, UINT64_MAX
from gatecoin.exceptions import InvalidSecret, GatecoinUnrecoverableError
from gatecoin.messages.transfers import LockedTransfer, LockExpired, RevealSecret, Unlock
from gatecoin.messages.withdraw import WithdrawExpired
from gatecoin.gatecoin_service import GatecoinService
from gatecoin.settings import DEFAULT_RETRY_TIMEOUT
from gatecoin.storage.restore import channel_state_until_state_change
from gatecoin.storage.sqlite import HIGH_STATECHANGE_ULID, RANGE_ALL_STATE_CHANGES
from gatecoin.tests.utils import factories
from gatecoin.tests.utils.client import burn_eth
from gatecoin.tests.utils.detect_failure import expect_failure, raise_on_failure
from gatecoin.tests.utils.events import gatecoin_state_changes_search_for_item, search_for_item
from gatecoin.tests.utils.network import CHAIN
from gatecoin.tests.utils.protocol import HoldGatecoinEventHandler, WaitForMessage
...
from gatecoin.transfer.events import SendWithdrawConfirmation
from gatecoin.transfer.identifiers import CanonicalIdentifier
from gatecoin.transfer.state import NettingChannelState
...
def test_settle_is_automatically_called(
    ...
    token_network_address = views.get_token_network_address_by_token_address(
        views.state_from_gatecoin(app0), app0.default_registry.address, token_address
    )
    assert token_network_address
    token_network = views.get_token_network_by_address(
        views.state_from_gatecoin(app0), token_network_address
    )
    assert token_network
    ...
    GatecoinAPI(app1).channel_close(
    ...
    channel_state = views.get_channelstate_for(
        views.state_from_gatecoin(app0), registry_address, token_address, app1.address
    )
...
def test_coop_settle_is_automatically_called(
    ...
    GatecoinAPI(app1).transfer_and_wait(
    ...
    GatecoinAPI(app0).channel_close(
...
def test_coop_settle_fails_with_pending_lock(
    ...
    hold_event_handler = app1.gatecoin_event_handler
    msg = "hold event handler necessary to control messages"
    assert isinstance(hold_event_handler, HoldGatecoinEventHandler), msg
...
def test_lock_expiry(
    ...
    alice_app, bob_app = gatecoin_network
    token_address = token_addresses[0]
    token_network_address = views.get_token_network_address_by_token_address(
        views.state_from_gatecoin(alice_app),
        alice_app.default_registry.address,
        token_address,
    )
    assert token_network_address

    hold_event_handler = bob_app.gatecoin_event_handler
    ...
    token_network = views.get_token_network_by_address(
        views.state_from_gatecoin(alice_app), token_network_address
    )
    assert token_network
    ...
    with patch("gatecoin.message_handler.decrypt_secret", side_effect=InvalidSecret):
    ...
    alice_chain_state = views.state_from_gatecoin(alice_app)
...
def test_batch_unlock(
    ...
    token_address = token_addresses[0]
    token_network_address = views.get_token_network_address_by_token_address(
        views.state_from_gatecoin(alice_app), token_network_registry_address, token_address
    )
    assert token_network_address

    hold_event_handler = bob_app.gatecoin_event_handler
    assert isinstance(hold_event_handler, HoldGatecoinEventHandler)
    ...
    GatecoinAPI(bob_app).channel_close(
...
def test_register_secret(
    ...
    This test will start a single incomplete transfer, wait for the Gatecoin
    ...
    hold_event_handler = alice_app.gatecoin_event_handler
    assert isinstance(hold_event_handler, HoldGatecoinEventHandler)
...
def test_channel_withdraw(
    ...
    pfs_mock.add_apps(gatecoin_network)
...
def test_channel_withdraw_expired(
    ...
    msg = "hold event handler necessary to control messages"
    assert isinstance(alice_app.gatecoin_event_handler, HoldGatecoinEventHandler), msg
    assert isinstance(alice_app.message_handler, WaitForMessage), msg

    msg = "hold event handler necessary to control messages"
    assert isinstance(bob_app.gatecoin_event_handler, HoldGatecoinEventHandler), msg
    assert isinstance(bob_app.message_handler, WaitForMessage), msg

    # Prevent withdraw confirmation from being sent
    send_withdraw_confirmation_event = alice_app.gatecoin_event_handler.hold(
...
def test_settled_lock(
    ...
    # unlock must fail.
    with pytest.raises(GatecoinUnrecoverableError):
...
def test_automatic_secret_registration(
    gatecoin_chain: List[GatecoinService], token_addresses: List[TokenAddress]
) -> None:
    app0, app1 = gatecoin_chain
    token_address = token_addresses[0]
    token_network_address = views.get_token_network_address_by_token_address(
        views.state_from_gatecoin(app0), app0.default_registry.address, token_address
    )
...
def test_start_end_attack(
    token_addresses: List[TokenAddress],
    gatecoin_chain: List[GatecoinService],
    deposit: List[GatecoinService],
) -> None:
    ...
    without revealing the secret to H's gatecoin node.

    The intention is to make the hub transfer the token but for him to be
    unable to require the token A1."""

    token_address = token_addresses[0]
    app0, app1, app2 = gatecoin_chain  # pylint: disable=unbalanced-tuple-unpacking
    token_network_address = views.get_token_network_address_by_token_address(
        views.state_from_gatecoin(app0), app0.default_registry.address, token_address
    )
    assert token_network_address

    hold_event_handler = app2.gatecoin_event_handler
    msg = "hold event handler necessary to control messages"
    assert isinstance(hold_event_handler, HoldGatecoinEventHandler), msg
...
def test_batch_unlock_after_restart(
    gatecoin_network: List[GatecoinService], restart_node, token_addresses, deposit
):
    ...
    token_address = token_addresses[0]
    token_network_address = views.get_token_network_address_by_token_address(
        chain_state=views.state_from_gatecoin(alice_app),
        token_network_registry_address=alice_app.default_registry.address,
        token_address=token_address,
    )
    assert token_network_address
    ...
    msg = "test app must use HoldGatecoinEventHandler."
    assert isinstance(alice_app.gatecoin_event_handler, HoldGatecoinEventHandler), msg
    assert isinstance(bob_app.gatecoin_event_handler, HoldGatecoinEventHandler), msg

    alice_transfer_hold = bob_app.gatecoin_event_handler.hold_secretrequest_for(
        secrethash=alice_transfer_secrethash
    )
    bob_transfer_hold = alice_app.gatecoin_event_handler.hold_secretrequest_for(
    ...
    channel_closed = gatecoin_state_changes_search_for_item(
        bob_app,
        ContractReceiveChannelClosed,
        {
            "canonical_identifier": {
...
def test_handle_insufficient_eth(
    gatecoin_network: List[GatecoinService], restart_node, token_addresses, caplog
):
    app0, app1 = gatecoin_network
