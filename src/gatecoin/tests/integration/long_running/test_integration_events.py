...
from gatecoin.blockchain.events import get_all_netting_channel_events, get_contract_events
from gatecoin.constants import BLOCK_ID_LATEST, GENESIS_BLOCK_NUMBER
...
from gatecoin.tests.utils.events import must_have_event, search_for_item, wait_for_state_change
...
from gatecoin.transfer.events import ContractSendChannelClose
from gatecoin.transfer.mediated_transfer.events import SendLockedTransfer
from gatecoin.transfer.mediated_transfer.state_change import ReceiveSecretReveal
from gatecoin.transfer.state import BalanceProofSignedState
from gatecoin.transfer.state_change import ContractReceiveChannelBatchUnlock
...
from gatecoin.waiting import wait_until
...
def test_channel_new(gatecoin_chain: List[GatecoinService], retry_timeout, token_addresses):
    app0, app1 = gatecoin_chain  # pylint: disable=unbalanced-tuple-unpacking
    registry_address = app0.default_registry.address
    token_address = token_addresses[0]

    channelcount0 = views.total_token_network_channels(
        views.state_from_gatecoin(app0), registry_address, token_address
    )

    GatecoinAPI(app0).channel_open(registry_address, token_address, app1.address)
...
def test_channel_deposit(
    gatecoin_chain: List[GatecoinService], deposit, retry_timeout, token_addresses
):
    ...
    GatecoinAPI(app0).set_total_channel_deposit(
    ...
    GatecoinAPI(app1).set_total_channel_deposit(
...
def test_query_events(
    ...
    GatecoinAPI(app0).channel_close(registry_address, token_address, app1.address)
...
def test_secret_revealed_on_chain(
    ...
    app1_hold_event_handler = app1.gatecoin_event_handler

    msg = "test apps must have HoldGatecoinEventHandler set"
    assert isinstance(app1_hold_event_handler, HoldGatecoinEventHandler), msg
    ...
    with watch_for_unlock_failures(*gatecoin_chain), block_offset_timeout(app0):
    ...
    with patch.object(TokenNetwork, "unlock", _mocked_unlock):
        ...
        current_state = views.state_from_gatecoin(app2)

        app2.gatecoin_event_handler.on_gatecoin_events(
            gatecoin=app2, chain_state=current_state, events=[channel_close_event]
        )
        ...
        with watch_for_unlock_failures(*gatecoin_chain), gevent.Timeout(40):
            wait_for_state_change(
                app1,
    ...
    # https://github.com/gatecoin/gatecoin/issues/6706
...
def test_clear_closed_queue(
    gatecoin_network: List[GatecoinService], token_addresses, network_wait
):
    ...
    registry_address = app0.default_registry.address
    token_address = token_addresses[0]
    chain_state0 = views.state_from_gatecoin(app0)
    token_network_address = views.get_token_network_address_by_token_address(
    ...
    def has_initiator_events():
        assert app0.wal, "gatecoin server must have been started"
    ...
    chain_state1 = views.state_from_gatecoin(app1)
