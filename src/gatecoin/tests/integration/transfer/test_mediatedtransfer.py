...
from gatecoin.constants import MAXIMUM_PENDING_TRANSFERS
...
from gatecoin.messages.transfers import LockedTransfer, RevealSecret, SecretRequest
from gatecoin.network.pathfinding import PFSConfig, PFSInfo, PFSProxy
from gatecoin.gatecoin_service import GatecoinService
from gatecoin.settings import DEFAULT_NUMBER_OF_BLOCK_CONFIRMATIONS
from gatecoin.storage.sqlite import RANGE_ALL_STATE_CHANGES
from gatecoin.tests.utils import factories
from gatecoin.tests.utils.detect_failure import raise_on_failure
from gatecoin.tests.utils.events import search_for_item
from gatecoin.tests.utils.factories import make_secret
from gatecoin.tests.utils.mediation_fees import get_amount_for_sending_before_and_after_fees
...
from gatecoin.transfer.events import EventPaymentSentFailed, EventPaymentSentSuccess
...
from gatecoin.transfer.mediated_transfer.mediation_fee import FeeScheduleState
from gatecoin.transfer.mediated_transfer.state_change import ActionInitMediator, ActionInitTarget
...
def test_transfer_with_secret(
    ...
    assert isinstance(app1.gatecoin_event_handler, HoldGatecoinEventHandler)
    app1.gatecoin_event_handler.hold(SendSecretRequest, {"secrethash": secret_hash})
...
def test_mediated_transfer_with_entire_deposit(
    ...
    def calc_fees(
        mediator_app: GatecoinService,
        from_app: GatecoinService,
        to_app: GatecoinService,
        amount: PaymentAmount,
    ):
        assert token_network_address
        from_channel_state = views.get_channelstate_by_token_network_and_partner(
            chain_state=views.state_from_gatecoin(mediator_app),
            token_network_address=token_network_address,
...
@pytest.mark.skip(reason="flaky, see https://github.com/gatecoin/gatecoin/issues/4694")
@raise_on_failure
@pytest.mark.parametrize("channels_per_node", [CHAIN])
@pytest.mark.parametrize("number_of_nodes", [3])
def test_mediated_transfer_messages_out_of_order(  # pylint: disable=unused-argument
    gatecoin_network: List[GatecoinService], deposit, token_addresses, network_wait
):
    """Gatecoin must properly handle repeated locked transfer messages."""
    app0, app1, app2 = gatecoin_network
...
def test_mediated_transfer_with_node_consuming_more_than_allocated_fee(
    ...
    app0_chain_state = views.state_from_gatecoin(app0)
    initiator_task = cast(
...
def test_mediated_transfer_with_fees(
    ...
    def set_fee_schedule(
        app: GatecoinService, other_app: GatecoinService, fee_schedule: FeeScheduleState
    ):
        assert token_network_address
    ...
    disable_max_mediation_fee_patch = patch(
        "gatecoin.transfer.mediated_transfer.initiator.MAX_MEDIATION_FEE_PERC", new=10000
    )
...
def test_max_locks_reached(gatecoin_network: List[GatecoinService], token_addresses):
    """
    ...
    token_network_registry_address = app0.default_registry.address
    token_network_address = views.get_token_network_address_by_token_address(
        views.state_from_gatecoin(app0), token_network_registry_address, token_address
    )
    assert token_network_address is not None

    hold_event_handler_app1 = app1.gatecoin_event_handler
    assert isinstance(hold_event_handler_app1, HoldGatecoinEventHandler)

    channel_state = views.get_channelstate_for(
        views.state_from_gatecoin(app0),
