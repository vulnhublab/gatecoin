...
from gatecoin.transfer.events import ContractSendChannelWithdraw
...
from gatecoin.utils.typing import BlockNumber, List, PaymentAmount, PaymentID, WithdrawAmount


@pytest.mark.skip(reason="flaky, see https://github.com/gatecoin/gatecoin/issues/5821")
@raise_on_failure
...
def test_recovery_unhappy_case(
    ...
    GatecoinAPI(app1).channel_close(app1.default_registry.address, token_address, app0.address)

    channel01 = views.get_channelstate_for(
        views.state_from_gatecoin(app1),
...
def test_recovery_blockchain_events(
    gatecoin_network: List[GatecoinService], restart_node, token_addresses
):
    """Close one of the two gatecoin apps that have a channel between them,
...
def test_node_clears_pending_withdraw_transaction_after_channel_is_closed(
    ...
    """A test case related to https://github.com/gatecoin/gatecoin/issues/4639
    ...
    assert isinstance(app0.gatecoin_event_handler, HoldGatecoinEventHandler)
    send_channel_withdraw_event = app0.gatecoin_event_handler.hold(ContractSendChannelWithdraw, {})
