...
from gatecoin.settings import DEFAULT_NUMBER_OF_BLOCK_CONFIRMATIONS
from gatecoin.tests.utils.client import burn_eth
from gatecoin.tests.utils.detect_failure import raise_on_failure
from gatecoin.tests.utils.events import wait_for_state_change
...
from gatecoin.tests.utils.transfer import get_channelstate
from gatecoin.transfer import channel, views
from gatecoin.transfer.events import EventPaymentSentFailed
from gatecoin.transfer.mediated_transfer.events import SendSecretRequest
from gatecoin.transfer.state import ChannelState, NetworkState
from gatecoin.transfer.state_change import (
    ContractReceiveChannelSettled,
    ContractReceiveNewTokenNetwork,
)
from gatecoin.utils.gas_reserve import (
...
from gatecoin_contracts.constants import CONTRACT_HUMAN_STANDARD_TOKEN
...
def test_register_token(
    gatecoin_network: List[GatecoinService], retry_timeout, unregistered_token
):
    app1 = gatecoin_network[0]
    registry_address = app1.default_registry.address
    token_address = unregistered_token

    # Wait until Gatecoin can start using the token contract.
    # Here, the block at which the contract was deployed should be confirmed by Gatecoin.
...
def test_token_registered_race(
    gatecoin_chain: List[GatecoinService], retry_timeout, unregistered_token
):
    ...
    with pytest.raises(GatecoinRecoverableError):
...
def test_deposit_updates_balance_immediately(
    gatecoin_chain: List[GatecoinService], token_addresses
):
...
def test_transfer_with_invalid_address_type(
    gatecoin_network: List[GatecoinService], token_addresses
):
    app0, _ = gatecoin_network
    token_address = token_addresses[0]
    ...
    with pytest.raises(InvalidBinaryAddress):
        GatecoinAPI(app0).transfer_and_wait(
...
@pytest.mark.parametrize("number_of_nodes", [2])
@pytest.mark.parametrize("channels_per_node", [1])
def test_insufficient_funds(gatecoin_network: List[GatecoinService], token_addresses, deposit):
    app0, app1 = gatecoin_network
    token_address = token_addresses[0]

    result = GatecoinAPI(app0).transfer_and_wait(
        app0.default_registry.address,
        token_address,
        deposit + 1,
...
@pytest.mark.skip(
    reason=(
        ...
        "https://github.com/gatecoin/gatecoin/issues/4625#issuecomment-585672612"
    )
)
@raise_on_failure
@pytest.mark.parametrize("number_of_nodes", [3])
@pytest.mark.parametrize("channels_per_node", [0])
def test_funds_check_for_openchannel(gatecoin_network: List[GatecoinService], token_addresses):
    """Reproduces issue 2923 -- two open channel racing through the gas reserve"""
    app0, app1, app2 = gatecoin_network
    token_address = token_addresses[0]

    gas = get_required_gas_estimate(gatecoin=app0, channels_to_open=1)
...
def test_payment_timing_out_if_partner_does_not_respond(  # pylint: disable=unused-argument
    gatecoin_network: List[GatecoinService], token_addresses, reveal_timeout, retry_timeout
):
    ...
    Issue: https://github.com/gatecoin/gatecoin/issues/3094"""

    app0, app1 = gatecoin_network
    token_address = token_addresses[0]

    msg = "test app must use HoldGatecoinEventHandler."
    assert isinstance(app1.gatecoin_event_handler, HoldGatecoinEventHandler), msg
    app1.gatecoin_event_handler.hold(SendSecretRequest, {})
...
def test_participant_deposit_amount_must_be_smaller_than_the_limit(
    ...
    https://github.com/gatecoin/gatecoin/pull/276/ , the limit is
    ...
    contract_proxy, _ = app1.rpc_client.deploy_single_contract(
        ...
        constructor_parameters=(token_supply, 2, "gatecoin", "Rd"),
    )
...
@pytest.mark.parametrize("number_of_nodes", [2])
@pytest.mark.parametrize("number_of_tokens", [1])
def test_token_addresses(gatecoin_network: List[GatecoinService], token_addresses):
    """
    ...
    views.state_from_gatecoin(app0).block_hash = app0.rpc_client.get_block(  # type: ignore
...
def run_test_token_addresses(gatecoin_network: List[GatecoinService], token_addresses):
    app = gatecoin_network[0]
    api = GatecoinAPI(app)
...
@pytest.mark.parametrize("number_of_nodes", [2])
@pytest.mark.parametrize("channels_per_node", [0])
def test_gatecoinapi_channel_lifecycle(
    gatecoin_network: List[GatecoinService],
    token_addresses,
    deposit,
    retry_timeout,
    ...
    """Uses GatecoinAPI to go through a complete channel lifecycle."""
    app1, app2 = gatecoin_network
    token_address = token_addresses[0]
    token_network_address = views.get_token_network_address_by_token_address(
        views.state_from_gatecoin(app1), app1.default_registry.address, token_address
    )
    assert token_network_address

    api1 = GatecoinAPI(app1)
    api2 = GatecoinAPI(app2)
    ...
    registry_address = api1.gatecoin.default_registry.address
    ...
    channel_state = views.get_channelstate_by_token_network_and_partner(
        views.state_from_gatecoin(app1), token_network_address, app2.address
    )
...
@pytest.mark.parametrize("number_of_nodes", [2])
@pytest.mark.parametrize("channels_per_node", [1])
def test_same_addresses_for_payment(gatecoin_network: List[GatecoinService], token_addresses):
    app0, _ = gatecoin_network
    api0 = GatecoinAPI(app0)
    registry_address = app0.default_registry.address
    token_address = token_addresses[0]
