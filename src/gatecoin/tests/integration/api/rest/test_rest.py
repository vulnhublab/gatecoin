...
from gatecoin.constants import BLOCK_ID_LATEST, Environment
from gatecoin.exceptions import InvalidSecret
from gatecoin.messages.transfers import LockedTransfer, Unlock
...
from gatecoin.tests.integration.api.rest.utils import (
    api_url_for,
...
from gatecoin.tests.integration.fixtures.smartcontracts import (
...
from gatecoin.tests.utils.events import must_have_event, must_have_events
...
from gatecoin.transfer.mediated_transfer.initiator import calculate_fee_margin
from gatecoin.transfer.state import ChannelState
...
from gatecoin.waiting import (
    TransferWaitResult,
    wait_for_block,
    wait_for_received_transfer_result,
    wait_for_token_network,
)
from gatecoin_contracts.constants import CONTRACT_CUSTOM_TOKEN, CONTRACTS_VERSION
...
def test_api_query_our_address(api_server_test_instance: APIServer):
    ...
    our_address = api_server_test_instance.rest_api.gatecoin_api.address
    assert get_json_response(response) == {"our_address": to_checksum_address(our_address)}


@raise_on_failure
@pytest.mark.parametrize("enable_rest_api", [True])
def test_api_get_gatecoin_version(api_server_test_instance: APIServer):
    request = grequests.get(api_url_for(api_server_test_instance, "versionresource"))
    response = request.send().response
    assert_proper_response(response)

    gatecoin_version = get_system_spec()["gatecoin"]

    assert get_json_response(response) == {"version": gatecoin_version}
...
def test_api_get_node_settings(api_server_test_instance: APIServer):
    ...
    pfs_config = api_server_test_instance.rest_api.gatecoin_api.gatecoin.config.pfs_config
...
@pytest.mark.parametrize("number_of_nodes", [2])
@pytest.mark.parametrize("enable_rest_api", [True])
def test_channel_events_gatecoin(
    api_server_test_instance: APIServer,
    gatecoin_network: List[GatecoinService],
    token_addresses,
    pfs_mock,
...
def test_pending_transfers_endpoint(
    decrypt_patch: Mock, gatecoin_network: List[GatecoinService], token_addresses
):
    initiator, mediator, target = gatecoin_network
    token_address = token_addresses[0]
    token_network_address = views.get_token_network_address_by_token_address(
        views.state_from_gatecoin(mediator), mediator.default_registry.address, token_address
    )
    assert token_network_address

    amount_to_send = PaymentAmount(150)
    # Remove when https://github.com/gatecoin/gatecoin/issues/4982 is tackled
    ...
    target_hold = target.gatecoin_event_handler
    assert isinstance(
        target_hold, HoldGatecoinEventHandler
    ), "test app must use HoldGatecoinEventHandler"
...
def test_api_testnet_token_mint(api_server_test_instance: APIServer, token_addresses):
    ...
    burn_eth(api_server_test_instance.rest_api.gatecoin_api.gatecoin.rpc_client)
    request = grequests.post(url, json=dict(to=user_address, value=1))
    response = request.send().response
...
def test_udc_api(api_server_test_instance: APIServer, retry_timeout):
    ...
    gatecoin_address = api_server_test_instance.rest_api.gatecoin_api.address
    gatecoin_service = api_server_test_instance.rest_api.gatecoin_api.gatecoin

    user_deposit = gatecoin_service.default_user_deposit
    assert user_deposit
    initial_deposit = user_deposit.get_total_deposit(gatecoin_address, BLOCK_ID_LATEST)
    ...
    withdraw_plan = user_deposit.get_withdraw_plan(gatecoin_address, BLOCK_ID_LATEST)
    ...
    wait_for_block(
        gatecoin=gatecoin_service,
        block_number=BlockNumber(
            gatecoin_service.get_block_number() + DEFAULT_NUMBER_OF_BLOCK_CONFIRMATIONS + 1
        ),
        retry_timeout=retry_timeout,
    )

    new_balance = user_deposit.get_balance(gatecoin_address, BLOCK_ID_LATEST)
    ...
    updated_total_deposit = user_deposit.get_total_deposit(gatecoin_address, BLOCK_ID_LATEST)
...
def test_shutdown(api_server_test_instance: APIServer):
    ...
    assert finished, "The Gatecoin node did not shut down!"
