...
from gatecoin.constants import UINT64_MAX
...
from gatecoin.tests.utils.transfer import watch_for_unlock_failures
from gatecoin.utils.secrethash import sha256_secrethash
from gatecoin.utils.typing import List, Secret
...
def test_api_payments(
    ...
    request = grequests.get(
        api_url_for(
            api_server_test_instance, "gatecoininternaleventsresource", limit=limit, offset=0
        )
    )
    response = request.send().response
    assert_proper_response(response)
...
def test_api_payments_without_pfs(
    ...
    our_address = api_server_test_instance.rest_api.gatecoin_api.address
    our_metadata = (
        api_server_test_instance.rest_api.gatecoin_api.gatecoin.transport.address_metadata
    )
...
def test_api_payments_with_hash_no_secret(
    api_server_test_instance, gatecoin_network: List[GatecoinService], token_addresses, pfs_mock
):
    _, app1 = gatecoin_network
    token_address = token_addresses[0]
    target_address = app1.address
