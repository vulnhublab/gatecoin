...
from gatecoin.constants import MATRIX_AUTO_SELECT_SERVER, Environment, EthClient
from gatecoin.settings import GATECOIN_CONTRACT_VERSION
from gatecoin.tests.utils.ci import get_artifacts_storage
from gatecoin.tests.utils.smoketest import setup_gatecoin, setup_testchain
from gatecoin.tests.utils.transport import ParsedURL
from gatecoin.utils.typing import Any, ContextManager, Dict, List, Optional


@pytest.fixture(scope="module")
def cli_tests_contracts_version() -> str:
    return GATECOIN_CONTRACT_VERSION


@pytest.fixture(scope="module")
def gatecoin_testchain(
    ...
    with testchain_manager as testchain:
        ...
        args = result.args
        print("setup_gatecoin took", time.monotonic() - start_time)
        yield args
...
def cli_args(
    ...
    # This assumes that there is only one Gatecoin instance per CLI test
    base_logfile = os.path.join(logs_storage, "gatecoin_nodes", "cli_test.log")
...
@pytest.fixture
def gatecoin_spawner(tmp_path, request):
    def spawn_gatecoin(args: List[str]):
        # Remove any possibly defined `GATECOIN_*` environment variables from outer scope
        new_env = {k: copy(v) for k, v in os.environ.items() if not k.startswith("GATECOIN")}
