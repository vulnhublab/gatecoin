...
def test_cli_full_init(cli_args, gatecoin_spawner):
...
def test_cli_wrong_keystore_path(cli_args, gatecoin_spawner):
    child = gatecoin_spawner(cli_args)
...
def test_cli_missing_password_file_enter_password(gatecoin_testchain, cli_args, gatecoin_spawner):
    child = gatecoin_spawner(cli_args)

    expect_cli_until_acknowledgment(child)
    child.expect("Enter the password to unlock")
    with open(gatecoin_testchain["password_file"], "r") as password_file:
...
def test_cli_missing_data_dir(cli_args, gatecoin_spawner):
    child = gatecoin_spawner(cli_args)
    expect_cli_normal_startup(child, EXPECTED_DEFAULT_ENVIRONMENT.value)


@pytest.mark.parametrize("changed_args", [{"eth_rpc_endpoint": "http://8.8.8.8:2020"}])
def test_cli_wrong_rpc_endpoint(cli_args, gatecoin_spawner):
    child = gatecoin_spawner(cli_args)
...
def test_cli_wrong_chain_id_try_kovan(cli_args, gatecoin_spawner):
    child = gatecoin_spawner(cli_args)
    expect_cli_until_acknowledgment(child)
...
def test_cli_registry_address_without_deployed_contract(cli_args, gatecoin_spawner):
    child = gatecoin_spawner(cli_args)
