...
from gatecoin.constants import BLOCK_ID_LATEST, DeviceIDs, RoutingMode
from gatecoin.exceptions import GatecoinUnrecoverableError
...
from gatecoin.tests.integration.test_integration_pfs import wait_all_apps
...
from gatecoin.tests.utils.transfer import transfer
...
def test_regression_filters_must_be_installed_from_confirmed_block(gatecoin_network):
    """On restarts Gatecoin must install the filters from the last run's
    confirmed block instead of the latest known block.

    Regression test for: https://github.com/gatecoin/gatecoin/issues/2894.
    """
...
def test_broadcast_messages_must_be_sent_before_protocol_messages_on_restarts(
    ...
    """Gatecoin must broadcast the latest known balance proof on restarts.

    Regression test for: https://github.com/gatecoin/gatecoin/issues/3656.
    """
    ...
    app0_restart = GatecoinService(
        ...
        gatecoin_bundle=GatecoinBundle(app0.default_registry, app0.default_secret_registry),
...
@pytest.mark.parametrize("start_gatecoin_apps", [False])
@pytest.mark.parametrize("deposit", [0])
@pytest.mark.parametrize("channels_per_node", [CHAIN])
@pytest.mark.parametrize("number_of_nodes", [2])
def test_alarm_task_first_run_syncs_blockchain_events(
    gatecoin_network: List[GatecoinService], blockchain_services
):
    """Gatecoin must synchronize with the blockchain events during
    initialization.

    Test for https://github.com/gatecoin/gatecoin/issues/4498
    """
    ...
    channels = GatecoinAPI(app0).get_channel_list(registry_address=app0.default_registry.address)
...
@pytest.mark.parametrize("channels_per_node", [CHAIN])
@pytest.mark.parametrize("number_of_nodes", [2])
def test_initialize_wal_throws_when_lock_is_taken(gatecoin_network: List[GatecoinService]):
    """Gatecoin must throw a proper exception when the filelock of the DB is already taken.

    Test for https://github.com/gatecoin/gatecoin/issues/6079
    """
    app0, _ = gatecoin_network

    # Start a second app, that should throw an expection, as the lock is already taken
    app0_2 = GatecoinService(
        config=app0.config,
        rpc_client=app0.rpc_client,
        proxy_manager=app0.proxy_manager,
        query_start_block=BlockNumber(0),
...
def test_fees_are_updated_during_startup(
    gatecoin_network: List[GatecoinService], restart_node, token_addresses, deposit, retry_timeout
) -> None:
    """
    ...
    app0.stop()
    app0.config = deepcopy(original_config)
    app0.config.mediation_fees.token_to_proportional_imbalance_fee[token_address] = (
...
def test_blockchain_event_processed_interleaved(
    ...
    Regression test for: https://github.com/gatecoin/gatecoin/issues/6444
    """
    app0, app1 = gatecoin_network
    ...
    # Restart node 1
    restart_node(app1)
    wait_all_apps(gatecoin_network)
