...
from gatecoin.constants import DeviceIDs, RoutingMode
...
from gatecoin.messages.transfers import Unlock
from gatecoin.gatecoin_service import GatecoinService
from gatecoin.settings import MediationFeeConfig
from gatecoin.tests.utils.detect_failure import raise_on_failure
from gatecoin.tests.utils.factories import make_transaction_hash
...
from gatecoin.tests.utils.transport import TestMatrixTransport
from gatecoin.transfer import views
from gatecoin.transfer.state import TransactionChannelDeposit
...
def get_messages(app: GatecoinService) -> List[Message]:
    assert isinstance(
...
def reset_messages(app: GatecoinService) -> None:
    assert isinstance(
        app.transport, TestMatrixTransport
    ), "Transport is not a `TestMatrixTransport`"
...
def wait_all_apps(gatecoin_network: List[GatecoinService]) -> None:
    last_known_block = max(app.rpc_client.block_number() for app in gatecoin_network)
...
def test_pfs_send_capacity_updates_on_deposit_and_withdraw(
    gatecoin_network: List[GatecoinService], token_addresses: List[TokenAddress], pfs_mock
) -> None:
    """
...
@pytest.mark.parametrize("channels_per_node", [CHAIN])
@pytest.mark.parametrize("routing_mode", [RoutingMode.PFS])
def test_pfs_send_unique_capacity_and_fee_updates_during_mediated_transfer(gatecoin_network):
    """
    Tests that PFSCapacityUpdates and PFSFeeUpdates are being
