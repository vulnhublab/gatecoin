...
from gatecoin.blockchain.decode import update_service_addresses_from_event
from gatecoin.blockchain.events import decode_gatecoin_event_to_internal
from gatecoin.blockchain.filters import GatecoinContractFilter
from gatecoin.constants import BLOCK_ID_LATEST
from gatecoin.exceptions import BrokenPreconditionError
from gatecoin.network.pathfinding import get_random_pfs, get_valid_pfs_url
from gatecoin.settings import GATECOIN_CONTRACT_VERSION
from gatecoin.tests.utils.factories import HOP1
from gatecoin.tests.utils.smartcontracts import deploy_service_registry_and_set_urls
from gatecoin.transfer.events import UpdateServicesAddresses
from gatecoin.utils.keys import privatekey_to_address
from gatecoin.utils.typing import FeeAmount, TokenNetworkRegistryAddress
...
def test_service_registry_set_url(service_registry_address, private_keys, web3, contract_manager):
    ...
    with pytest.raises(BrokenPreconditionError):
        c1_service_proxy.set_url("gatecoin.com")
...
def test_service_registry_random_pfs(
    ...
    with patch("gatecoin.network.pathfinding.get_pfs_info", mock_get_pfs_info):
...
def test_service_registry_events(service_registry_address, private_keys, web3, contract_manager):
    """
    - Test that `GatecoinContractFilter` successfully matches on `RegisteredService`.
    - Test that blockchain-event, state_change and gatecoin-event decoding methods work.
    """
    c1_service_proxy, _ = deploy_service_registry_and_set_urls(
        private_keys=private_keys,
        web3=web3,
    ...
    # register filter
    event_filter = GatecoinContractFilter(service_registry=c1_service_proxy)

    manager = ContractManager(contracts_precompiled_path(GATECOIN_CONTRACT_VERSION))
    ...
    decoded_events = [decode_gatecoin_event_to_internal(ABI, 0, ev) for ev in events]
    assert len(decoded_events) == 3
    state_changes = [update_service_addresses_from_event(ev) for ev in decoded_events]
    assert len(state_changes) == 3
    gatecoin_events = [UpdateServicesAddresses.from_state_change(sc) for sc in state_changes]
    assert len(gatecoin_events) == 3
