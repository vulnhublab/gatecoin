...
from gatecoin.utils.typing import Set, T_ChannelID
...
def test_token_network_proxy(
    ...
    with pytest.raises(GatecoinUnrecoverableError, match=match):
    ...
    match = "closed"
    with pytest.raises(GatecoinRecoverableError, match=match):
...
def test_token_network_proxy_update_transfer(
    ...
    with pytest.raises(GatecoinUnrecoverableError) as exc:
    ...
    with pytest.raises(GatecoinUnrecoverableError) as excinfo:
        c2_token_network_proxy.update_transfer(
            channel_identifier=channel_identifier,
            partner=c1_client.address,
            balance_hash=balance_proof_c1.balance_hash,
...
def test_query_pruned_state(token_network_proxy, private_keys, web3, contract_manager):
    """A test for https://github.com/gatecoin/gatecoin/issues/3566
