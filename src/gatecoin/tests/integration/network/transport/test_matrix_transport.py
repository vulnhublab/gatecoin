...
from gatecoin.network.transport.matrix.transport import (
    MatrixTransport,
    MessagesQueue,
    _RetryQueue,
    populate_services_addresses,
)
from gatecoin.network.transport.matrix.utils import AddressReachability
...
from gatecoin.tests.utils.mocks import MockGatecoinService, make_pfs_config
from gatecoin.tests.utils.smartcontracts import deploy_service_registry_and_set_urls
...
from gatecoin.utils.keys import privatekey_to_address
from gatecoin.utils.typing import Address, MessageID
...
def ping_pong_message_success(transport0, transport1):
    queueid0 = QueueIdentifier(
        recipient=transport0._gatecoin_service.address,
    ...
    transport0_gatecoin_queues = views.get_all_messagequeues(
        views.state_from_gatecoin(transport0._gatecoin_service)
    )
    transport1_gatecoin_queues = views.get_all_messagequeues(
        views.state_from_gatecoin(transport1._gatecoin_service)
    )

    transport0_gatecoin_queues[queueid1] = []
    transport1_gatecoin_queues[queueid0] = []

    received_messages0 = transport0._gatecoin_service.message_handler.bag
    received_messages1 = transport1._gatecoin_service.message_handler.bag
    ...
    transport0_gatecoin_queues[queueid1].append(ping_message)

    transport0._gatecoin_service.sign(ping_message)
    transport1._gatecoin_service.sign(pong_message)
    ...
    transport0_gatecoin_queues[queueid1].clear()
    transport1_gatecoin_queues[queueid0].append(ping_message)

    transport0._gatecoin_service.sign(pong_message)
    transport1._gatecoin_service.sign(ping_message)
    ...
    transport1_gatecoin_queues[queueid0].clear()
...
def test_matrix_message_sync(matrix_transports):
    ...
    gatecoin_service0 = MockGatecoinService(transport0_message_handler)
    gatecoin_service1 = MockGatecoinService(transport1_message_handler)

    gatecoin_service1.handle_and_track_state_changes = MagicMock()
    ...
    transport1.start(transport1._gatecoin_service, None)
...
def test_matrix_message_retry(
    ...
    gatecoin_service = MockGatecoinService(None)

    transport.start(gatecoin_service, None)
    transport.log = MagicMock()

    queueid = QueueIdentifier(
        recipient=partner_address, canonical_identifier=CANONICAL_IDENTIFIER_UNORDERED_QUEUE
    )
    chain_state = gatecoin_service.wal.get_current_state()
    ...
    transport._gatecoin_service.sign(message)
...
def test_matrix_cross_server_with_load_balance(matrix_transports):
    ...
    gatecoin_service0 = MockGatecoinService(message_handler0)
    gatecoin_service1 = MockGatecoinService(message_handler1)
    gatecoin_service2 = MockGatecoinService(message_handler2)

    transport0.start(gatecoin_service0, "")
    transport1.start(gatecoin_service1, "")
    transport2.start(gatecoin_service2, "")
...
def test_matrix_broadcast(matrix_transports, services, device_id):
    ...
    transport.start(MockGatecoinService(None), "")
    gevent.idle()
...
def test_monitoring_broadcast_messages(
    ...
    """
    Test that GatecoinService broadcast RequestMonitoring messages to
    ...
    gatecoin_service = MockGatecoinService(None)
    gatecoin_service.config = GatecoinConfig(
    ...
    gatecoin_service.transport = transport
    transport.log = MagicMock()
    ...
    monkeypatch.setattr(
        gatecoin.transfer.views,
        "get_channelstate_by_canonical_identifier",
        lambda *a, **kw: channel_state,
    )
    monkeypatch.setattr(gatecoin.transfer.channel, "get_balance", lambda *a, **kw: 123)
    gatecoin_service.default_user_deposit.effective_balance.return_value = MONITORING_REWARD

    update_monitoring_service_from_balance_proof(
        gatecoin=gatecoin_service,
        chain_state=None,
...
def test_monitoring_broadcast_messages_in_production_if_bigger_than_threshold(
    ...
    """
    Test that in PRODUCTION on DAI and WETH GatecoinService broadcast RequestMonitoring messages
...
def test_pfs_broadcast_messages(
    ...
    """
    Test that GatecoinService broadcasts PFSCapacityUpdate messages to
    ...
    gatecoin_service = MockGatecoinService(None)
    gatecoin_service.config.services.monitoring_enabled = True
    gatecoin_service.routing_mode = RoutingMode.PFS
    ...
    send_pfs_update(
        gatecoin=gatecoin_service, canonical_identifier=balance_proof.canonical_identifier
    )
    ...
    fee_update.sign(gatecoin_service.signer)
    gatecoin_service.transport.broadcast(fee_update, device_id=DeviceIDs.PFS)
...
def test_matrix_user_roaming(matrix_transports, roaming_peer):
    ...
    gatecoin_service0 = MockGatecoinService(message_handler0, private_key=privkey0)
    gatecoin_service1 = MockGatecoinService(message_handler1, private_key=privkey1)
    ...
    transport0.stop()
    transport2.start(gatecoin_service0, "")
...
def test_matrix_multi_user_roaming(matrix_transports, roaming_peer):
    ...
    transport_rs0_1.start(gatecoin_service0, "")
    transport_rs1_0.start(gatecoin_service1, "")
    ...
    transport_rs0_2.start(gatecoin_service0, "")
    transport_rs1_0.start(gatecoin_service1, "")
