...
# gatecoin/tests/conftest.py::auto_enable_gevent_monitoring_signal
...
if [[ "${test_type}" != "unit" ]]; then
    ...
    if [[ -n ${GATECOIN_TESTS_LOGSDIR} ]]; then
        ...
        for test_directory in "${GATECOIN_TESTS_LOGSDIR}"/*; do
            ...
            for log_file in "${test_directory}"/gatecoin-debug*.log; do
