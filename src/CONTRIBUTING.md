# Gatecoin Development Guide

Welcome! This guide serves as the guideline to contributing to the Gatecoin Network
codebase. It's here to help you understand what development practises we use here
and what are the requirements for a Pull Request to be opened against Gatecoin.
...
If you experience a problem while using Gatecoin or want to request a feature
...
- An explanation of why you believe this would make a good addition to Gatecoin.

**For Bugs:**

- A short description of the problem.
- Detailed description of your system, gatecoin version, geth version, solidity
  version e.t.c.
...
In most cases we would need to see the debug logs and the database of your gatecoin node to be able to determine what happened and help debug the issue. If your issue occured on the mainnet and you would like to keep the data private you can encrypt the logs with a special pgp key we provide and send them to GatecoinErrors@brainbot.li . A short guide on how to do this follows:
...
2. Get the gatecoin pgp key from [github](https://github.com/gatecoin/gatecoin/blob/master/gatecoin_errors_key.gpg).
3. Import the key by: `$: gpg --import path/to/gatecoin_errors_key.gpg`
4. Find the relevant debug logs and the DB and put them in an archive: `$: tar -cvf debug_data.tar path/to/debug.log path/to/gatecoin.db`
5. Encrypt the archive with our public key: `$: gpg -e -u "Enter your name here" -r "GatecoinErrors@brainbot.li" debug_data.tar`
6. You should now have a file called `debug_data.tar.gpg` in your current directory.
7. Take the above file and email it to GatecoinErrors@brainbot.li
...
- The email account "GatecoinErrors@brainbot.li" is only accessible by the DPO and his deputy.

**Data handling within the Gatecoin team**
...
codebase of Gatecoin then you can open a Pull Request(PR) against the repository.
...
have one approved review from one of the core developers of Gatecoin and also all
Continuous Integration tests should be passing and the CI build should be
green.

Additionally you need to sign the gatecoin project CLA (Contributor License
...
All the developers in the team should perform Pull Request reviews. Make it a habit to check [this](https://github.com/gatecoin/gatecoin/pulls?q=is%3Apr+is%3Aopen+label%3A%22dev%3A+Please+Review%22) link often to help your fellow colleagues who have PRs open pending for review.
...
isolate the python dependencies used by Gatecoin from your system:
...
tests to run, just use the appropriate directory.
    ...
    pytest gatecoin/tests/<integration|unit>
...
the gatecoin repository. All code you write should strive to comply with these
rules.
...
Gatecoin is written in Python and we follow the official Python style guide
...
transition function, `gatecoin.node.state_transition`).
...
parts of the code such as the api. The central `GatecoinService` class and
...
require instantiating one or more `GatecoinService` instances and providing
...
`gatecoin.tests.integration.contracts`.

#### Test utilities

The utilities in `gatecoin.tests.utils` are used by all parts of the test
suite.
...
`gatecoin.tests.utils.factories` module. For each complicated object type
...
`gatecoin.tests.utils.factories.Properties` for further details.

The `gatecoin.tests.utils.detect_failure` module contains two decorators
for integration tests that run `GatecoinService` instances. The use of one
of the decorators is required on each such test and pytest will enforce it.
`@raise_on_failure` will make sure every crash of a `GatecoinService` leads
to instant failure of the test, and `@expect_failure` indicates that we
actually expect the crash of a `GatecoinService` instance within the test
and want to ignore it.

Sometimes a test using `@raise_on_failure` needs to restart nodes or start
new ones during the test. Simply calling `Gatecoin.start` to achieve this will
...
against develop. You will be given feedback from the core developers of gatecoin
...
[documentation](https://github.com/gatecoin/gatecoin/blob/master/docs/rest_api.rst).
...
update the [specification](https://github.com/gatecoin/spec). The spec
...
If you are a core developer of Gatecoin with write privileges to the repository
then you can add commits or rebase to develop any Pull Request by other people.

Let us take [this](https://github.com/gatecoin/gatecoin/pull/221) PR as an
...
```
git remote add hackaugusto git@github.com:hackaugusto/gatecoin.git
...
A normal run of gatecoin can be profiled by providing the `--flamegraph /path/to/dir` option. For example:

`python -m gatecoin --eth-rpc-endpoint http://parity.goerli.ethnodes.brainbot.com:8545 --accept-disclaimer --network-id goerli --keystore-path ~/test_keystore --flamegraph ./TEMP`
...
`pytest --profiler=flamegraph-trace -xs gatecoin/tests/integration/api/test_restapi.py::test_api_payments`
