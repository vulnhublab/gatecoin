...
RUN pyinstaller --noconfirm --clean gatecoin.spec
...
RUN export FILE_TAG=${ARCHIVE_TAG:-v$(python setup.py --version)} && \
    cd dist && \
    tar -cvf ./gatecoin-${FILE_TAG}-linux-${ARCHITECTURE_TAG}.tar gatecoin-${FILE_TAG}-linux-${ARCHITECTURE_TAG} && \
    mv gatecoin-${FILE_TAG}-linux-${ARCHITECTURE_TAG}.tar ..
