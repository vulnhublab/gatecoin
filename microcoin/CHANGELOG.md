...
* Added more documentation and updated metadata in the µGateCoin web client. #360
...
* Updated documentation of a µGateCoin quickstart in README.md. #342
...
* Added a hint on how to install µGateCoin using `pip install -e`. #335
...
* Made µGateCoin available on pypi. Enjoy your `pip install microcoin`! #296
* Merged `HTTPClient` and `DefaultHTTPClient` into a `requests`-like `Session` class. This class directly inherits `requests.Session` and can easily be used in its stead to create µGateCoin-enabled client applications. Global `microcoin.requests` methods create temporary `Session` objects. #295
...
* Added a lot of documentation to the µGateCoin JS/TS client library. #288
...
* Major refactoring of `contract_proxy.py` methods. #275
    ...
    The `ContractProxy` class was replaced by stateless utility functions exposed in `microcoin.utils`. All required context objects have to be passed on each call.
...
* The HTTP client now supports all HTTP methods that are also supported by the `requests` module. The new `microcoin.requests` module allows for `requests`-like syntax of µGateCoin-enabled HTTP requests. #258
