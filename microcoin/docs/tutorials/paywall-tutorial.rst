...
in ``microcoin/examples/echo_server.py``.
...
.. code:: python
    ...
    from microcoin.make_helpers import make_paywalled_proxy
    app = make_paywalled_proxy(private_key, state_file_name)

:py:meth:`microcoin.make_helpers.make_paywalled_proxy` is a helper that handles the setup of the channel manager
and returns a :py:class:`microcoin.proxy.paywalled_proxy.PaywalledProxy` instance.
Microcoin also includes other helpers that parse common commandline options. We are not using them in this example - for a quick overview how to use them, 
refer to i.e. :py:meth:`microcoin.examples.demo_proxy.__main__`
...
The workflow is the same as with the Flask-restful: Subclass :py:class:`microcoin.proxy.resources.Expensive` and
...
.. code:: python
    ...
    from microcoin.proxy.resources import Expensive
...
We add one static resource to our :py:class:`~microcoin.proxy.paywalled_proxy.PaywalledProxy` instance.
...
.. code:: python
    ...
    from microcoin.proxy.content import PaywalledProxyUrl
...
Note, that the kwargs for the constructor of the resource-class (here our :py:class:`~microcoin.proxy.content.PaywalledProxyUrl`)
