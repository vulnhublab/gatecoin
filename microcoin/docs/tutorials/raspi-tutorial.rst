===========================================
Setup µGateCoin Raspberry Pi in local network
===========================================
...
``mkvirtualenv -p /usr/bin/python3.5 uGatecoin``

- Clone and setup microcoin

``git clone https://github.com/gatecoin/microcoin.git``
...
Running µGateCoin Client and Server
====================================

.. figure:: /diagrams/SetupRasPi.png


    Networking topology of a machine-to-machine application with RaspberryPi µGateCoin-nodes [1]_ [2]_


As the next step, you are going to setup the raspberry pi (`Sender`) as the µGateCoin client or the  and our PC as as the µGateCoin proxy server (`Receiver`) as well as the web3 provider running a geth node synced to Ropsten testnet. Next, you will run the ``echo_server`` and the ``echo_client``  examples from the ``microcoin/examples`` folder. The ``echo_client`` on the raspberry pi, and the ``echo_server`` on your PC.
...
The `rpcaddr` as **0.0.0.0** means that a given socket is listening on all the available IP addresses the computer has. This is important so that µGateCoin client on the raspberry can use it as a **web3** provider.
...
Running the µGateCoin Proxy Server
----------------------------------------


In the ``~/microcoin/microcoin/examples`` folder go to the ``echo_server.py`` and go to the part where we start the server.(These set of actions are performed on **your PC**)

``cd ~/microcoin/microcoin/examples``
...
.. code ::
 ...
 $ python -m  echo_server --private-key ~/.ethereum/testnet/keystore/UTC--2016-07-27T07-40-38.092883212Z--9d80d905bc1e106d5bd0637c12b893c5ab60cb41
  ...
  INFO:filelock:Lock 139916998263696 acquired on ~/.config/microcoin/echo_server.db.lock
  INFO:blockchain:starting blockchain polling (interval 2s)


Running the µGateCoin client on the raspberry pi
-----------------------------------------------

Navigate to the cloned microcoin repository and modify the following files on the **raspberry pi**.

``cd ~/microcoin/microcoin``

1. In the ``microcoin/constants.py`` file change the  **WEB3_PROVIDER_DEFAULT** value to ``"http://192.168.1.104:8545"``  where  ``192.168.1.104``  is the IP address of the PC where we started the geth node and the µGateCoin ``echo_server``.

``sudo nano microcoin/constants.py``

2. In the ``microcoin/examples/echo_client.py``  change ``endpoint_url`` parameter of the `run` function
...
``sudo nano microcoin/examples/echo_client.py``

Now we run the `echo_client.py` like this

.. code ::

   (uGatecoin) pi@raspberrypi:~/microcoin/microcoin $ python -m  microcoin.examples.echo_client --private-key              ~/.ethereum/testnet/keystore/UTC--2018-02-12T08-35-34.437506909Z--9a7d8c3116258c1f50f3c8ac67d120af58a46ceb --resource        /echofix/hello
   Enter the private key password:
   INFO:microcoin.client.client:Creating channel to 0x9d80D905bc1E106d5bd0637c12B893c5Ab60CB41 with an initial deposit of    50 @2684938
   WARNING:microcoin.client.session:Newly created channel does not have enough confirmations yet. Retrying in 5 seconds.
...
Through this example we hope developers can develop their own machine to machine clients and their respective server to use microcoin for micropayments according to their respective use cases, using these resources.

1. microcoin **Session** Library (source microcoin/microcoin/client/session.py)
2. microcoin **Requests** Library (source microcoin/microcoin/requests/__init__.py)
3. microcoin **Client** Library (microcoin/microcoin/client/client.py)
...
.. code ::
  ...
  (uGatecoin) pi@raspberrypi:~ $ apt list --installed *secp256k1*
  Listing... Done
  (uGatecoin) pi@raspberrypi:~ $ apt list  *secp256k1*
  Listing... Done
