Comparison: Gatecoin Network
================================

µGateCoin is not part of the `Gatecoin
Network <https://github.com/gatecoin/gatecoin>`__. However, it was
...
- **µGateCoin** is a many-to-one unidirectional payment channel protocol.

A payment channel in the **Gatecoin Network** is based on the same principles as µGateCoin, but is laid out bidirectionally,
...
This allows participants of the Gatecoin Network to efficiently send transfers without being forced to pay for
opening new channels with people who are already in the network - it is a many-to-many payment solution.

- **µGateCoin**'s off-chain transactions do not cost anything as they are only exchanged between sender and receiver, because they don't use intermediary channels.

Apart from the initial cost of opening up a channel, a µGateCoin transaction doesn't cost anything, because to deliver the
...
the Gatecoin Network requires an additional, sophisticated application transport layer.
...
Since µGateCoin enables easy micropayments from one party to another, the application is
...
The `Sender` is the one who initially deposits Ether in the µGateCoin payment channel.
...
The µGateCoin application has different implementations for different scenarios for the `Sender` side:
...
paywalled content. The content provider is the `Receiver` in this scenario and he would integrate µGateCoin's `Proxy-Server`
for example in his flask or Django backend.
At the same time, the content provider would serve an implementation of µGateCoin's JavaScript Client from his webpage.
...
.. figure:: /diagrams/uGatecoinOverview.png

   Schematic overview of an exemplaric µGateCoin application [1]_ [3]_
...
To be exact, there is a third party involved in µGateCoin:
...
µGateCoin uses its own token for payments which is both
