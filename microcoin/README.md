# µGateCoin [![Build Status](https://api.travis-ci.org/gatecoin/microcoin.svg)](https://travis-ci.org/gatecoin/microcoin)

[![Join the chat at https://gitter.im/gatecoin/microcoin](https://badges.gitter.im/gatecoin/microcoin.svg)](https://gitter.im/gatecoin/microcoin?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge&utm_content=badge)


µGateCoin is an off-chain, cheap, scalable and low-latency micropayment solution.

[µGateCoin documentation](https://microcoin.readthedocs.io/)


## Smart Contract

Current version: `0.2.0` (second Bug Bounty release). Verifiable with `GetMicroTransferChannels.call().version()`.
Note that a new µGateCoin release might include changing the Ethereum address used for the smart contract, in case we need to deploy an improved contract version.

The `GetMicroTransferChannels` contract has been deployed on the main net: [0x1440317CB15499083dEE3dDf49C2bD51D0d92e33](https://etherscan.io/address/0x1440317CB15499083dEE3dDf49C2bD51D0d92e33)
...
- GetMicroTransferChannels address is: [0xed94e711e9de1ff1e7dd34c39f0d4338a6a6ef92](https://kovan.etherscan.io/address/0xed94e711e9de1ff1e7dd34c39f0d4338a6a6ef92#code)
...
- GetMicroTransferChannels address is: [0x74434527b8e6c8296506d61d0faf3d18c9e4649a](https://ropsten.etherscan.io/address/0x74434527b8e6c8296506d61d0faf3d18c9e4649a#code)
...
- GetMicroTransferChannels address is: [0xbec8fb898e6da01152576d1a1acdd2c957e56fb1](https://rinkeby.etherscan.io/address/0xbec8fb898e6da01152576d1a1acdd2c957e56fb1#code)


## Brief Overview

µGateCoin is not part of the [Gatecoin Network](https://github.com/gatecoin/gatecoin). However, it was built using the same state channel idea and implements it in a less general fashion focusing on the concrete application of micropayments for paywalled content.

The main differences between the Gatecoin Network and µGateCoin are:
 * µGateCoin is a many-to-one unidirectional state channel protocol, while the Gatecoin Network is a many-to-many bidirectional solution and implies a more complex design of channel networks. This allows the Gatecoin Network to efficiently send transfers without being forced to pay for opening new channels with people who are already in the network.
 * µGateCoin off-chain transactions do not cost anything, as they are only exchanged between sender and receiver. The Gatecoin Network has a more complicated incentive-based off-chain transport of transaction information, from one user to another (following the channel network path used to connect the sender and the receiver).


### Tokens and Channel Manager Contract

µGateCoin uses its own token for payments which is both [ERC20](https://github.com/ethereum/EIPs/issues/20) and [ERC223](https://github.com/ethereum/EIPs/issues/223) compliant.

In a nutshell, clients (subsequently called "senders") wanting to access a provider's payable resources, will [open a micropayment channel](http://microcoin.readthedocs.io/en/latest/contract/index.html#opening-a-transfer-channel) with the provider ("receiver") and fund the channel with a number of tokens. These escrowed tokens will be kept by a third party contract that manages opening and closing of channels.
...
A visual description of the process can be found [here](http://microcoin.readthedocs.io/en/latest/contract/index.html#closing-a-channel).
...
Try out the µGateCoin demo and build your own customized version, following our instructions below!


## Quick Start

 * install the Proxy component (more details [here](http://microcoin.readthedocs.io/en/latest/proxy-tutorial.html)):

```bash
virtualenv -p python3 env
. env/bin/activate
pip install microcoin
```

* install the WebUI component for the paywall examples

Note that while the `GetMicroTransferChannels` contract supports multiple open channels between a sender and a receiver, the WebUI component only supports one.

```
cd microcoin/microcoin/webui/microcoin
npm i && npm run build
```

* run the Proxy component:

For an overview of parameters and default options check https://github.com/gatecoin/microcoin/blob/master/microcoin/click_helpers.py

For chain and contract settings change: https://github.com/gatecoin/microcoin/blob/master/microcoin/config.py

```
cd microcoin
python -m microcoin.examples.demo_proxy --private-key <private_key_file> start
```
...
virtualenv -p python3 env
. env/bin/activate
pip install -e microcoin
```

#### Using microcoin in pip's _editable_ mode
Because of `gevent` you will need to install microcoin's requirements first.
```bash
virtualenv -p python3 env
. env/bin/activate
git clone git@github.com:gatecoin/microcoin.git
cd microcoin/microcoin
...
```bash
sudo pip3 install -e microcoin
```

## Execution

### HTTP Proxy
There are several examples that demonstrate how to serve custom content. To try them, run one of the following commands from the `microcoin` directory:
```bash
python3 -m microcoin.examples.demo_proxy --private-key <private_key_file> start
```
or
```bash
python3 -m microcoin.examples.wikipaydia --private-key <private_key_file> --private-key-password-file <password_file> start
```
...
### M2M Client
```bash
python3 -m microcoin.examples.m2m_client --key-path <path to private key file> --key-password-path <password file>
```

## Library usage

### Client
The µGateCoin client backend used by the M2M sample client can be used as a standalone library. After installation, import the following class:
```python
from microcoin import Client
...
This client object allows interaction with the blockchain and offline-signing of transactions and Gatecoin balance proofs.
