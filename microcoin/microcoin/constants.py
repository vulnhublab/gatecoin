...
API_PATH = "/api/1"
"""str: api path prefix"""
MICROCOIN_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
"""str: absolute path to module directory. Used to find path to the webUI sources"""
HTML_DIR = os.path.join(MICROCOIN_DIR, 'microcoin', 'webui')
...
CHANNEL_MANAGER_ABI_NAME = 'GetMicroTransferChannels'
...
with open(os.path.join(MICROCOIN_DIR, 'microcoin', CONTRACTS_ABI_JSON)) as metadata_file:
    CONTRACT_METADATA = json.load(metadata_file)

MICROCOIN_VERSION = read_version(os.path.join(MICROCOIN_DIR, 'microcoin', 'VERSION'))
"""str: version of Microcoin library"""
