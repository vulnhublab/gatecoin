...
from microcoin.config import NETWORK_CFG
from microcoin.constants import WEB3_PROVIDER_DEFAULT
from microcoin.utils import (
    addr_from_sig,
    keccak256,
)
from microcoin.test.config import (
    FAUCET_ALLOWANCE,
    INITIAL_TOKEN_SUPPLY
)
from microcoin.constants import (
    get_network_id
)
from microcoin.utils.contract import DEFAULT_TIMEOUT, DEFAULT_RETRY_INTERVAL
import microcoin.utils.contract
...
def deploy_token_contract(web3, deployer_address, faucet_address, token_abi, token_bytecode):
    ...
    txhash = Token.deploy(
        {'from': deployer_address}, args=[INITIAL_TOKEN_SUPPLY, "Gatecoin Network Token", "RDN", 18]
    )
...
def patched_contract(use_tester: bool, monkeypatch: MonkeyPatch, web3: Web3):
    if use_tester:
        ...
        monkeypatch.setattr(microcoin.utils.contract, '_get_logs_raw', patched_get_logs_raw)
        monkeypatch.setattr(microcoin.utils.contract, '_wait', patched_wait_for_event)
        monkeypatch.setattr(
            microcoin.utils.contract,
