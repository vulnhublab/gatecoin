...
def test_extract_balance_proof_signature(get_accounts, token_instance, ugatecoin_instance):
    (A, B) = get_accounts(2)
    ...
    message_hash = balance_proof_hash(receiver, block, balance, ugatecoin.address)
    balance_msg_sig, signer = sign.check(message_hash, tester.k2)
    assert is_same_address(signer, A)

    signature_address = ugatecoin.call().extractBalanceProofSignature(
        receiver,
        block,
        balance,
        balance_msg_sig
...
def test_extract_closing_signature(get_accounts, token_instance, ugatecoin_instance):
    (A, B) = get_accounts(2)
    ugatecoin = ugatecoin_instance
    ...
    message_hash = closing_message_hash(sender, block, balance, ugatecoin.address)
    balance_msg_sig, signer = sign.check(message_hash, tester.k2)
    assert is_same_address(signer, A)

    signature_address = ugatecoin.call().extractClosingSignature(
        sender,
        block,
        balance,
        balance_msg_sig
