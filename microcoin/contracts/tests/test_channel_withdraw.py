...
def test_withdraw_call(channel_params, ugatecoin_instance, get_channel):
    ...
    with pytest.raises(TypeError):
        ugatecoin_instance.transact({"from": receiver}).withdraw(
            -2,
            balance,
            balance_msg_sig
        )
...
def test_withdraw_fail_no_channel(
    ...
    with pytest.raises(tester.TransactionFailed):
        ugatecoin_instance.transact({"from": sender}).withdraw(
            open_block_number,
            balance,
            balance_msg_sig
        )
    with pytest.raises(tester.TransactionFailed):
        ugatecoin_instance.transact({"from": A}).withdraw(
            open_block_number,
            balance,
            balance_msg_sig
        )
...
def test_withdraw_state(
    ...
    ugatecoin_pre_balance = token_instance.call().balanceOf(ugatecoin_instance.address)
    sender_pre_balance = token_instance.call().balanceOf(sender)
    receiver_pre_balance = token_instance.call().balanceOf(receiver)

    txn_hash = ugatecoin_instance.transact({"from": receiver}).withdraw(
        open_block_number,
        balance1,
        balance_msg_sig
    )
    ...
    ugatecoin_balance = ugatecoin_pre_balance - balance1
    assert token_instance.call().balanceOf(ugatecoin_instance.address) == ugatecoin_balance
...
def test_close_after_withdraw(
    ...
    ugatecoin_post_balance = ugatecoin_pre_balance - (balance2 - balance1)
    ...
    assert token_instance.call().balanceOf(ugatecoin_instance.address) == ugatecoin_post_balance
...
def test_withdraw_event(
    ...
    ev_handler.add(txn_hash, UGATECOIN_EVENTS['withdraw'], checkWithdrawEvent(
        sender,
        receiver,
        open_block_number,
        balance)
