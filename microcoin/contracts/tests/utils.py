...
def channel_settle_tests(ugatecoin_instance, token, channel):
    (sender, receiver, open_block_number) = channel

    # Approve token allowance
    # TODO: why this fails?
    # token.transact({"from": sender}).approve(ugatecoin_instance.address, 33)

    with pytest.raises(tester.TransactionFailed):
        ugatecoin_instance.transact({'from': sender}).topUp(receiver, open_block_number, 33)


def channel_pre_close_tests(ugatecoin_instance, token, channel, top_up_deposit=0):
    (sender, receiver, open_block_number) = channel
