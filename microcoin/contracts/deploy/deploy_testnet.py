...
def main(**kwargs):
    ...
    with project.get_chain(chain_name) as chain:
        ...
        microcoin_contract = chain.provider.get_contract_factory('GetMicroTransferChannels')
        txhash = microcoin_contract.deploy(
            args=[token_address, challenge_period, []],
            transaction={'from': owner}
        )
        receipt = check_succesful_tx(chain.web3, txhash, txn_wait)
        microcoin_address = receipt['contractAddress']

        print('GetMicroTransferChannels address is', microcoin_address)
