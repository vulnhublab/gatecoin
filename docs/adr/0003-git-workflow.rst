...
The current workflow team members in the Gatecoin team currently use is as simple as pick issues, create a respective branch for that issue and present a PR once the bug/feature is fixed/implemented.
...
2. Features that move Gatecoin forward into integrating with the PFS / MS services (which are under heavy development)
