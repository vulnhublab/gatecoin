...
has a channel for both WETH and RDN. He can use Gatecoin token swaps to do
...
Gatecoin does not provide upgradable contracts. When deploying new
...
the Gatecoin Network has many users.
...
of different blockchains. A Gatecoin payment can be used to swap between
...
have to coordinate with non-Gatecoin clients. As a consequence, this should
...
Gatecoin currently expects payments to stay within one token network. To
...
The modular approach aims to keep the Gatecoin core protocol lean and
simple. This should keep the Gatecoin protocol robust. Less features
...
the Gatecoin core functionalities to a more powerful feature. These
...
If the Gatecoin protocol were only be able to handle single token payments
