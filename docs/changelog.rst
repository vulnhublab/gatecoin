...
* :support:`7493` Update ``gatecoin-contracts`` package.
...
* :feature:`7195` Enable Gatecoin Node communication over WebRTC. This will allow payments to complete faster due to direct communication between peers.
...
* :feature:`6986` Increment DB version - this will make this release incompatible with previous releases. Please refer to `the known issues section in the docs <https://gatecoin.readthedocs.io/en/latest/other/known-issues.html#database-upgrades>`_ for details.
* :feature:`6851` Gatecoin does not use Matrix rooms anymore. This greatly reduces the load on the matrix servers and increases the reliability of the transport layer.
...
* :feature:`6763` Ensure that pending messages are sent before Gatecoin is stopped.
...
* :feature:`-` Update WebUI to `version 1.2.1 <https://github.com/gatecoin/webui/releases/tag/v1.2.1>`_. Introduces a new "Quick Connect", that is simpler and more transparent than the old "join token network" feature, and adds a dialog for interacting with the UserDeposit contract (UDC).
...
* :feature:`6482` Add support for client capabilities. Gatecoin clients can now negotiate supported features via their matrix profile.
* :bug:`6444 major` Fix a race condition when processing blockchain events.

* :release:`1.1.1 <2020-07-20>`
* :feature:`-` Update WebUI to version 1.0.2 https://github.com/gatecoin/webui/releases/tag/v1.0.2
...
* :feature:`-` Update WebUI to version 1.0.1 https://github.com/gatecoin/webui/releases/tag/v1.0.1
* :bug:`6310` Fixed dependencies used in ``setup.py``.

* :release:`1.0.1 <2020-06-15>`
* :feature:`-` Update WebUI to version 1.0.0 https://github.com/gatecoin/webui/releases/tag/v1.0.0
...
* :feature:`-` Update WebUI to version 0.11.1 https://github.com/gatecoin/webui/releases/tag/v0.11.1
...
* :feature:`-` Update WebUI to version 0.11.0 https://github.com/gatecoin/webui/releases/tag/v0.11.0

* :release:`0.200.0-rc2 <2019-11-25>`
* :feature:`5050` Gatecoin's argument --debug-logfile-name has been renamed to --debug-logfile-path to better reflect the argument's function.
* :bug:`5050` Gatecoin now works on OSX Catalina. Debug logfile is no longer written in the current directory.
...
* :feature:`-` Update WebUI to version 0.10.1 https://github.com/gatecoin/webui/releases/tag/v0.10.1
...
* :feature:`4917` Added documentation on using Gatecoin for atomic swaps.
...
* :feature:`-` Update WebUI to version 0.10.0 https://github.com/gatecoin/webui/releases/tag/v0.10.0
* :feature:`-` Added a new api endpoint ``/api/v1/version`` to query the gatecoin version via the Rest API.

* :release:`0.100.5-a0 <2019-08-12>`
* :feature:`-` Update WebUI to version 0.9.2 https://github.com/gatecoin/webui/releases/tag/v0.9.2
* :bug:`4498` Gatecoin now waits for synchronization with the blockchain events before finishing its startup and opening the API.
* :bug:`4348` Fix wrong calculation of ``balance`` field of channel information when tokens are locked in payments
* :bug:`4502` Fix a Gatecoin crash related to routing feedback

* :release:`0.100.5-dev0 <2019-07-30>`
* :feature:`4457` Gatecoin now checks the version of the ethereum node at startup and if it is not supported quits with an appropriate message.
* :feature:`-` Update WebUI to version 0.9.1 https://github.com/gatecoin/webui/releases/tag/v0.9.1
...
* :feature:`4095` Prevent Gatecoin from sending IOU to PFS when fee is 0
...
* :feature:`4043` Update gatecoin-contracts to 0.19.0 with Görli Testnet support
...
* :bug:`3832` Fix Gatecoin startup when a previous run aborted during migrations
...
* :bug:`3687` Fix startup initialization issue which caused Gatecoin to crash on private chains
...
* :bug:`3566` Handle cases where Gatecoin tries to query blocks which are old and pruned by the blockchain client (Geth & Parity)
* :feature:`3464` Gatecoin will warn users about insufficient user deposit funds (If monitoring service or path-finding service are enabled and used).
* :feature:`3462` Static Monitoring service reward through user deposits contract. Only usable if Gatecoin is run in development environment
* :feature:`3461` Static PFS payment for provided routes through the user deposits contract. Only usable if Gatecoin is run in development environment
* :release:`0.100.3-rc4 <2019-04-17>`
* :release:`0.100.3-rc3 <2019-04-15>`
* :feature:`-` Add support for Görli testnet in Gatecoin.
* :release:`0.100.3-rc2 <2019-04-11>`
* :release:`0.100.3-rc1 <2019-03-29>`
* :feature:`3467` Gatecoin now chooses a PFS from a provided registry. Also added a new argument ``--routing-mode`` to configure the routing mode to either be PFS or BASIC.
* :bug:`3558` Gatecoin will no longer crash if starting with a fresh DB due to the ethereum node queries timing out.
...
* :feature:`3425` Update gatecoin-contracts package to version 0.9.0
...
* :bug:`3528` Do not crash gatecoin if a LockExpired message with invalid channel identifier is received.
* :bug:`3529` Do not crash gatecoin if a SecretRequest message with invalid channel identifier is received.
...
* :bug:`3369` Fix high CPU usage when the gatecoin node is idle.
* :feature:`-` Set python 3.7 as a minimum python version requirement to run Gatecoin.
...
* :feature:`-` Update WebUI to version 0.8.0 https://github.com/gatecoin/webui/releases/tag/v0.8.0
...
* :feature:`-` Update WebUI to version 0.7.1 https://github.com/gatecoin/webui/releases/tag/v0.7.1
...
* :bug:`3216` If coming online after partner closed channel don't try to send updateNonClosingBalanceProof twice and crash Gatecoin.
* :bug:`3211` If using parity and getting the already imported error, attempt to handle it and not crash the client.
* :bug:`3121` If the same payment identifier is reused avoid a specific race condition that can crash Gatecoin.
* :bug:`3201` Workaround for gas price strategy crashing Gatecoin with an Infura ethereum node.
* :bug:`3190` Prevents removal of initiator task when one of the transfers is expired.

* :release:`0.100.1 <2018-12-21>`
* :bug:`3171` Do not crash gatecoin if the Matrix server is offline when joining a discovery room.
...
* :bug:`3175` If Github checking of latest version returns unexpected response do not let Gatecoin crash.
...
* :bug:`3152` If the onchain unlock has already been mined when we try to send the transaction don't crash Gatecoin.
* :feature:`3157` Change REST api version prefix from 1 to v1.
* :bug:`3135` In development mode if more than 100 * (10^18) tokens are deposited then gatecoin no longer crashes.
...
* :bug:`3094` Gatecoin will now properly return payment failure and no longer hang if a payment times out due to a lock expiration.
* :bug:`3093` Getting gatecoin payment history will no longer crash gatecoin for failed sent payment events.
...
* :bug:`2921` Properly estimate gas cost of transactions so that we have a more reasonable minimal amount of ETH required to run Gatecoin.
* :feature:`2962` Check that the ethereum node has all required json rpc interfaces enabled when Gatecoin starts. If not fail with a proper error.
...
* :bug:`2933` Gatecoin can now recover from crashes/restarts when there are pending onchain transactions.
...
* :bug:`2894` Gatecoin will no longer miss confirmation blocks at restart and will emit the block state change only for confirmed blocks.
...
* :bug:`2664` Gatecoin node will now wait for 5 block confirmations before processing a given transaction.

* :release:`0.13.1 <2018-10-15>`
* :bug:`2784` Gatecoin node is no longer left with a partial update if it crashes during polling.
...
* :feature:`2708` Add ``--showconfig`` CLI flag which dumps all configuration values that will control Gatecoin behavior.
...
* :bug:`2655` Gatecoin node will now properly crash if communication with the ethereum node is lost.
...
* :bug:`2414` If partner uses our old balance proof on-chain, the gatecoin client will now recover it from the WAL and properly use it on-chain.
...
* :feature:`1894` We now start having nightly releases found here: https://gatecoin-nightlies.ams3.digitaloceanspaces.com/index.html
...
* :bug:`2027` Gatecoin should now be able to connect to Infura.
* :feature:`2084` Rename the ``/transfers/`` endpoint to ``/payments/``.
* :feature:`1998` Add a strategy to make sure that the account Gatecoin runs on always has enough balance to settle all channels. No new channels can be openend when no sufficient balance for the whole channel lifecycle is available.
...
* :bug:`1996` Providing contracts addresses via the CLI that either have no code or contain unexpected code will now result in an error and not crash Gatecoin.
* :bug:`1994` Starting Gatecoin with a corrupt database will now throw a proper error instead of crashing with an exception.
...
* :bug:`1897` Limit number of concurrent matrix connections so that gatecoin client does not crash.
...
* :bug:`1902` Check for ethnode connection at start and print proper error if Gatecoin can not connect
...
* :feature:`-` Restartability in case of a proper shutdown of the Gatecoin node.
...
* :bug:`1237` Inform the user if geth binary is missing during gatecoin smoketest.
...
* :feature:`87` Update gatecoin to use Python 3 and the latest version of pyethereum.
* :feature:`1015` Added macOS compatibility and binary releases.
* :feature:`1093` Reconnect gatecoin to ethereum node after disconnect.
...
* :bug:`1049` Make gatecoin byzantium compatible by no longer relying on ``estimateGas``.
...
* :feature:`670` Block gatecoin startup until ethereum node is fully synchronized.
...
* :feature:`-`  This is the `Gatecoin Developer Preview <https://github.com/gatecoin/gatecoin/releases/tag/v0.1.0>`_ release. Introduces a gatecoin test network on ropsten, the API and all the basic functionality required to use Gatecoin in Dapps. For more information read the `blog post <https://medium.com/@gatecoin_network/gatecoin-developer-preview-dad83ec3fc23>`_ or the `documentation of v0.1.0 <http://gatecoin.readthedocs.io/en/v0.1.0/>`_.
