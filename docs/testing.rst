How is Gatecoin tested
####################

Gatecoin tests are split into these categories. The following categories are
...
- Unit tests: A collection of fast tests that do not require external services
  ...
  located under ``gatecoin/tests/unit``.
- Property tests: These tests use randomly generated events. These tests are
  ...
  located under ``gatecoin/tests/fuzz``.
- Smoketest: This is a quick self contained test, that can be ran by a user or
  ...
  layer, deploy the smart contracts, start a Gatecoin instance and interact with
  the blockchain. This test is available as a command line command, and it's
  entry point is under ``gatecoin/ui/cli.py``.
- Integration tests: These are long running tests, that require external
  ...
  local private blockchain, a local transport infrastructure, multiple Gatecoin
  nodes, and the REST API for these nodes. A fresh set of smart contracts are
  deployed into this private chain, including Tokens and Gatecoin's smart
  contracts. These tests are important to make sure Gatecoin will work as
  ...
  ``gatecoin/tests/integration``.
- Scenario tests: These tests run multiple times against the available
  testnets. They exercise the Gatecoin nodes through the available REST API, in
...
preferred type of test to add to Gatecoin's test suite.

Integration Tests
=================

These tests exist to make sure that Gatecoin works properly with the other
...
system. This is necessary because these tests will run multiple Gatecoin nodes in
a single process, exchange messages, and assert on the expected state of each
individual node.

Here is an overview of the fixture system:

- Configuration variables are defined in ``gatecoin/tests/fixtures/variables``.
  ...
  many Gatecoin nodes should be started for the test, ``number_of_channels``
...
- The fixtures responsible to start a private chain are defined in
  ``gatecoin/tests/integration/fixtures/blockchain.py``. These will start a new
  private chain, either ``geth`` or ``openethereum``, with
  ``blockchain_number_of_nodes`` nodes. This private chain will have one
  prefunded account for each Gatecoin node, which allows these nodes to send
  on-chain transactions.
- The fixtures responsible to deploy the smart contracts are defined in
  ``gatecoin/tests/integration/fixtures/smart_contracts.py``. These fixtures
  depend on the private chain, and will deploy the necessary Gatecoin and Token
  Smart Contracts for the test.
- The fixtures responsible to start the transport are defined in
  ``gatecoin/tests/integration/fixtures/transport.py``. These fixtures will
  configure a local cluster of Matrix servers to run the tests.
- The fixtures responsible to start the gatecoin nodes and open the initial
  channels are defined in
  ``gatecoin/tests/integration/fixtures/gatecoin_network.py`` . These fixtures
  depend on both the previous fixtures, the private blockchain, the smart
  contract, and the transport fixtures. The gatecoin network fixtures will then
  create the Gatecoin apps and open the necessary channels for the test.
...
gatecoin_network fixture will have a fresh set of matrix servers, a new private
chain with newly deployed smart contracts, and running Gatecoin nodes with open
...
- If a failure is not expected, it should make the test fail. In order to
  ...
  monitors all the Gatecoin nodes, and if any unexpected exception is raised on
