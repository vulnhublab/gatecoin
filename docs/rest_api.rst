...
Introduction
*************
The Gatecoin API is organized around REST and has resource-oriented URL endpoints that accept and return JSON-encoded responses.
The Gatecoin API uses standard HTTP response codes and verbs.
The Gatecoin RESTful API endpoints correspond to the interactions allowed by a Gatecoin node.
...
However, when Gatecoin fails to process the incoming request and raises an exception, the returned http status code will be :http:statuscode:`500`. The caveat of this is that the response body will be just a string message which says "Internal server error".
...
In any way, we consider :http:statuscode:`500` errors as bugs in the Gatecoin client. If you encounter such errors, please report the bug `here <https://github.com/gatecoin/gatecoin/issues/new?template=bug_report.md>`_.
...
All objects that are sent and received from the Gatecoin API are JSON encoded.
The following outlines each of the Gatecoin API endpoints.
...
.. http:get:: /api/(version)/address
   ...
   Queries the Ethereum address you choose when starting Gatecoin.
   A Gatecoin node is up and running correctly if the response returns that same address.
...
.. http:get:: /api/(version)/version
   ...
   You can query the version endpoint to see which version of Gatecoin you're currently running.
...
.. http:get:: /api/(version)/status
   ...
   **Example Response**:
   ...
   :statuscode 500: Internal Gatecoin error
...
.. http:get:: /api/(version)/settings
   ...
   Queries the settings of your Gatecoin node.
   ...
   **Example Response**:
   ...
   .. sourcecode:: http
      ...
      {
          "pathfinding_service_address": "https://pfs.transport04.gatecoin.network"
      }
...
.. http:get:: /api/(version)/tokens
   ...
   **Example Response**:
   ...
   :statuscode 200: Successful query
   :statuscode 500: Internal Gatecoin node error
   :statuscode 503: The API is currently unavailable, e. g. because the Gatecoin node is still in the initial sync or shutting down.
...
The channels endpoints allow you to open channels with other Gatecoin nodes as well as closing channels, querying them for information and making deposits or withdrawals.
          ...
          "paths": [
              {
                  ...
                  "address_metadata": {
                      "0xEA674fdDe714fd979de3EdF0F56AA9716B898ec8": {
                          ...
                          "capabilities": "mxc://gatecoin.network/cap?Receive=1&Mediate=1&Delivery=1&webRTC=1&toDevice=1&immutableMetadata=1",
...
For paying the :doc:`Gatecoin Services <gatecoin_services>` it is necessary to have RDN (Gatecoin Network Tokens) in the User Deposit Contract (UDC).
...
.. http:post:: /api/(version)/user_deposit
   ...
   **Example Response**:
   ...
   :statuscode 400: The provided JSON is in some way malformed
   :statuscode 402: Insufficient balance to do a deposit or insufficient ETH to pay for the gas of the on-chain transaction
   :statuscode 404: No UDC is configured on the Gatecoin node
...
.. http:delete:: /api/(version)/connections/(token_address)
   ...
   **Example Response**:
   ...
   .. note::
      Currently, the API calls are blocking. This means that in the case of long running calls like leave a token network, if an API call is currently being processed by Gatecoin, all pending calls will be queued and processed with their passed API call argument.
...
.. http:get:: /api/(version)/notifications
   ...
   Gatecoin can inform the user about conditions that may require an action by the user or their attention.
   ...
   * ``version_outdated``: Running an outdated version of Gatecoin.
   ...
   **Example Response**:
   ...
   .. sourcecode:: http
      ...
      [
         ...
         {
            "body": "You're running version 1.1.1.dev630+g43ae6e32b. The latest version is 1.2.0It's time to update! Releases: https://github.com/gatecoin/gatecoin/releases",
            "id": "version_outdated",
