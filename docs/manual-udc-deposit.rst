...
To pay the services, you have to lock some of your Gatecoin tokens in the ``UserDeposit`` contract.
In this section, we will briefly explain how it can be done manually so you are able to use Gatecoin.
...
You can obtain the address of the contract from
    ...
    ``https://github.com/gatecoin/gatecoin/blob/master/gatecoin_contracts/data/deployment_services_<network>.json``

where ``network`` is one of ``mainnet``, ``ropsten``, ``rinkeby`` or ``goerli``.

As an example, suppose we want to run a Gatecoin node with address ``0x3040435D7F1012e861f0B0989422a47D1825F120`` on the mainnet
and deposit 10 Gatecoin tokens (10**19 REI) for it to use. Here we use `MetaMask <https://metamask.io>`_ to access our wallet
...
We log into MetaMask with our account that holds the Gatecoin tokens (which should be a different account than the one we use with
Gatecoin, as the latter is supposed to be used with Gatecoin only.) To find the ``UserDeposit`` contract, we take a look at ``deployment_services_mainnet.json``:
...
The Gatecoin token (RDN) can be searched by name on Etherscan, or we can look up its address in the ``UserDeposit`` contract's
