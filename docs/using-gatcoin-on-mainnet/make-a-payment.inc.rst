...
Payments in Gatecoin can be done either as:
...
.. note::
    ...
    It is possible to receive tokens through the Gatecoin Network without having any initial Ether or tokens. The receiving peer just needs to have a Gatecoin full node running and the sender must be willing to pay the fees for the on-chain transactions. 
...
payment, Gatecoin will try to find a path from you to the recipient node
through the network of channels. This is called mediated payments.

Mediated payments are a powerful feature in Gatecoin since it lets you pay
