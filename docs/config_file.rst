Gatecoin Config File
##################

Gatecoin supports reading configuration parameters from a configuration file.
Gatecoin configuration parameter can also be given as environment variables - the variable has to be prefixed
with ``GATECOIN_`` - otherwise the options are the same as the commandline arguments.


Location
--------

The default location is ``<datadir>/config.toml`` (where datadir defaults to ``~/.gatecoin``).
...
#. Option read from the prefixed ``GATECOIN_<config option>`` environment variables
#. Option read from the config file
#. Option default value (as seen in the output of ``gatecoin --help``)
...
Gatecoin allows configuration of the logging system using a concise syntax on the command line.
...
Example::
    ...
    # CLI:
    --log-config ':debug,gatecoin.network:info'
