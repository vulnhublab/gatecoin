...
A custom token can be used for any use-case but we will be adding and using it with the Gatecoin Network.
...
The Gatecoin Network supports transactions of ERC20 compliant tokens. Any smart contract that implements the ERC20 interface can be used with Gatecoin.
...
All you need to do to start using your token is to :doc:`download and install Gatecoin <../overview_and_guide>` and register your token in Gatecoin, either in the :ref:`Web Interface <webui_register_token>` or by using the :ref:`API <api_tut_register_token>`.
