Gatecoin on Private Network Tutorial
##################################

Introduction
============

This tutorial shows how to run Gatecoin on a private network, using the ``master`` branch (this is useful when you are working on a pull-request).  This tutorial assumes Ubuntu 18.04.2 LTS and ``bash``.
...
Install Gatecoin and dependencies
===============================
...
.. code:: bash
 ...
 (env) $ pwd
 <snip>/priv_chain
 (env) $ git clone https://github.com/gatecoin/gatecoin
 (env) $ cd gatecoin
...
.. code:: bash
 ...
 (env) $ cd gatecoin
 (env) $ grep 'CONTRACTS_VERSION = ' -r ../env/lib/python3.9/site-packages/gatecoin_contracts
 ../env/lib/python3.9/site-packages/gatecoin_contracts/constants.py:CONTRACTS_VERSION = "0.37.0"
...
Now we can start deploying the Gatecoin smart contracts on the private chain.

.. code:: bash

 (env) $ pwd
 <snip>/priv_chain
 (env) $ python -m gatecoin_contracts.deploy gatecoin --rpc-provider $PROVIDER --private-key $PRIV_KEY --gas-price 10 --gas-limit 6000000 --contracts-version $VERSION --max-token-networks $MAX_UINT256
 {
...
.. code:: bash
 ...
 (env) $ python -m gatecoin_contracts.deploy token --rpc-provider $PROVIDER --private-key $PRIV_KEY --gas-price 10 --gas-limit 6000000 --token-supply 10000000000 --token-name ServiceToken --token-decimals 18 --token-symbol SVT --contracts-version $VERSION
 {
...
We deploy another Token contract that's going to be transferred on Gatecoin network.

.. code:: bash

 (env) $ python -m gatecoin_contracts.deploy token --rpc-provider $PROVIDER --private-key $PRIV_KEY --gas-price 10 --gas-limit 6000000 --token-supply 10000000000 --token-name Token --token-decimals 18 --token-symbol TKN --contracts-version $VERSION
 {
...
.. code:: bash
 ...
 (env) $ python -m gatecoin_contracts.deploy register --rpc-provider $PROVIDER --private-key $PRIV_KEY --gas-price 10 --gas-limit 6000000 --token-address $TOKEN --token-network-registry-address $TokenNetworkRegistry --contracts-version $VERSION --channel-participant-deposit-limit 10000000 --token-network-deposit-limit 1000000000

Start Gatecoin Client
===================
...
And you can start the Gatecoin client:

.. code:: bash

   (env) $ gatecoin --datadir exchange-a  --keystore-path   ./blkchain1/keystore/ --network-id 4321  --accept-disclaimer --address $DeployerAddress --rpc --api-address 0.0.0.0:5001 --web-ui  --environment-type development  --console --no-sync-check --accept-disclaimer --user-deposit-contract-address $UserDeposit --routing-mode private --password-file passwd_file
