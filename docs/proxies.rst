...
A Gatecoin node accesses smart contracts through wrappers called proxies. The proxies are implemented in ``gatecoin/network/proxies`` directory. The proxies are supposed to be used with some special care, and they are implemented in a specific way.

The complication exists because there is always the possibility of a race between our node, the blockchain(s) and other parties. The Gatecoin node usually recognizes events on the blockchain only after some confirmation period. By the time the Gatecoin node recognizes an onchain event, the Gatecoin node has seen several blocks have been mined on top. However, when Gatecoin posts a transaction to the blockchain, the transaction is sent to miners who operate on the newest available block.
...
mistake in the Gatecoin codebase, and a check must be added before calling the proxy.

However, the proxy raises ``BrokenPreconditionError`` only when the error is about the chain state.
There are values that are invalid regardless of the chain state. These cause ``GatecoinValidationError``
instead of ``BrokenPreconditionError``.  The ``GatecoinValidationError`` is not considered as
...
for the block to be confirmed so that the state change is visible to the Gatecoin node.

When the proxy raises ``GatecoinUnrecoverableError``, it means that there is a bug in the
client codebase or in the smart contracts.

When the proxy raises ``GatecoinRecoverableError``, it might mean:
...
The proxy tries hard to determine whether the problem is in the Gatecoin codebase (client
or smart contract), but ultimately, if the proxy is not sure, it raises ``GatecoinRecoverableError``.
...
- ``GatecoinValidationError`` when the given argument is invalid regardless of the chain state.
- Instead of ``GatecoinValidationError``, you can also raise a special exception that you derive from ``GatecoinError``,
