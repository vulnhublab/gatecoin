...
Gatecoin supports implementing 3rd-party token swaps functionality on top of the payments endpoint in :doc:`rest_api`.
This document will show a walk through of how to build an atomic swap application on top of Gatecoin using the payment API and the ``--resolver-endpoint`` CLI flag.

Here's how it could be done:

First step would be that a payment is initiated with only the secret hash (Secret is unknown to Gatecoin) through your preferred HTTP client library.
...
This request will trigger Gatecoin to initiate a payment (Locked Transfer) to our channel partner just like a regular payment.
With the exception that Gatecoin does **not** know the secret that will be used to unlock the locked transfer.
As soon as the partner requests the secret, Gatecoin needs to fetch the secret from the application that initiated the
atomic swap. This is where the ``--resolver-endpoint`` comes to play.

For Gatecoin to be able to fetch the secret, an endpoint has to be exposed to Gatecoin so that Gatecoin can request the
...
For this to work, Gatecoin has to be started in the following manner:

``gatecoin --resolver-endpoint http(s)://host[:port]/endpoint``
...
This will trigger Gatecoin to send this secret to the target node. The target node can use this secret to unlock the transfer.
