...
The Gatecoin network includes auxiliary services, that enhance the overall user
...
suitable payment paths for Gatecoin nodes.
...
By default the Gatecoin client will chose a suitable Pathfinding service for you.
...
The PFS relies on information from the individual Gatecoin nodes to keep its
network representation up-to-date. Therefore the Gatecoin clients will send
...
.. note::
  ...
  issue. <https://github.com/gatecoin/gatecoin/issues/4951>`_
...
information
<https://gatecoin-specification.readthedocs.io/en/latest/monitoring_service.html#monitor-request>`_)
...
state itself (this is handled by the Gatecoin client automatically). The reward is
