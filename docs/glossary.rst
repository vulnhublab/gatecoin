...
.. glossary::
    ...
    Payment channel
        The on-chain payment channel between two Gatecoin nodes.
    ...
    Transfer
        A transfer in Gatecoin happens each time tokens are sent inside a :term:`payment channel`.

    Payment
        A payment in Gatecoin is the process of sending tokens from one account to another. Each payment has an :term:`initiator` and a :term:`target` and can be composed of multiple transfers.
    ...
    Preimage
    Secret
        The preimage, what we call the secret in Gatecoin, is 32 bytes of random cryptographically secure data whose keccak hash ends up being the :term:`hashlock`.
...
.. glossary::
    ...
    Initiator
        The initiator is the Gatecoin node which initiates (starts) a :term:`payment`.
    ...
    Target
        The target is the Gatecoin node which receives a payment from the :term:`initiator`.
...
.. glossary::
    ...
    User Deposit
        The Gatecoin services will ask for payment in RDN. The Monitoring Service and the Pathfinding Service require deposits to be made in advance of service usage. These deposits are handled by the User Deposit Contract.
