...
-  **Ethereum node in sync and running reliably:** Ensure that layer 1
   ...
   are any problems or bugs on the client then Gatecoin can not work
   reliably.
-  **Ethereum client always online:** Make sure that your Ethereum
   client is always online during operation of a Gatecoin node. As stated
   above, you can safely go offline if channel monitoring is enabled,
   but in order to use the Gatecoin nodes for doing transfers you have to
...
-  **Ethereum Client can not be changed:** Swapping the Ethereum client
   ...
   avoiding switching Ethereum clients once the Gatecoin node is running.
-  **Gatecoin online for operations:** Currently all nodes participating
   in a transfer need to be online in order for a transfer to be carried
   out. Hence, make sure that your Gatecoin node is always working, your
   network connection is stable and that the Gatecoin node is always
...
-  **Unique account for Gatecoin:** Gatecoin requires you to have a specific
   Ethereum account solely dedicated to Gatecoin. Creating any manual
   transaction with the Ethereum account that Gatecoin uses, while the
   Gatecoin client is running, can result in undefined behaviour. It is,
   however, safe to do manual transactions with the account if Gatecoin is
   not running.
-  **Gatecoin account has sufficient ETH:** Gatecoin will try to warn you if
   there is not enough ETH in your Gatecoin account in order to maintain
...
-  **Gatecoin account has sufficient UserDeposit:** If you are using
   ...
   Gatecoin services will not kick in, since they are not getting paid.
-  **Do not transfer too small amounts for mediated transfers:**
   Currently the Gatecoin client cancels payments that would require more
...
-  **Persistency of local DB:** Your local state database is located at
   ~/.gatecoin. This data should not be deleted by the user or tampered
   with in any way. Frequent backups are recommended. Deleting this
   directory can result in a loss of funds.
-  **Never expose the Gatecoin REST API to the public:** For Gatecoin’s
   operation, the client needs to be able to sign transactions at any
   point in time. Therefore you should never expose the Gatecoin Rest API
