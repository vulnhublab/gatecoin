...
Most of these issues are not Gatecoin specific, but rather apply to all blockchain applications.
...
-  **Compromised user system:** If the system of the user is compromised
   ...
   such: `gatecoin/gatecoin#579 <https://github.com/gatecoin/gatecoin/issues/579>`__
-  **Disk Full:** The client does not properly handle the cases where
   ...
   the Gatecoin node crashing. In the future, we want to handle the
   detection of a full disk and gracefully quit the app:
   `gatecoin/gatecoin#675
   <https://github.com/gatecoin/gatecoin/issues/675>`__
-  **Blockchain Congestion:** If the blockchain is congested and there
   is no space for the Gatecoin node to submit transactions on-chain, the
   ...
   `gatecoin/gatecoin#383 <https://github.com/gatecoin/gatecoin/issues/383>`__
...
change releases, Gatecoin will do automatic upgrades in the background, as soon as
...
- to recover all allocated funds, you should run the previous version of Gatecoin and
...
- if there are no funds worth recovering, you can also simply start over with a new
  ...
  :ref:`start over <running_gatecoin>`.
