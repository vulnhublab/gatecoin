Gatecoin Network Smart Contracts
==============================

.. image:: https://badges.gitter.im/Join%20Chat.svg
    :target: https://gitter.im/gatecoin/gatecoin?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge
    :alt: Chat on Gitter

`Gatecoin Network Smart Contract specs`_

.. _Gatecoin Network Smart Contract specs: https://gatecoin-specification.readthedocs.io/en/latest/smart_contracts.html
...
Recommended::
    ...
    pip install gatecoin-contracts
...
``gatecoin_contracts/data_<CONTRACTS_VERSION>/contracts.json`` file to get the ABI
and bytecode for initializing the contract instances.

You can find the addresses of the deployed contracts in
``gatecoin_contracts/data_<CONTRACTS_VERSION>/deployment_*.json``. All contracts
under ``gatecoin_contracts/contracts/test/`` are only for testing purposes and
they should not be used in production.

You can find other useful constants that you can import in
``gatecoin_contracts/constants.py``.

.. Note::
    This package supports many contract versions, including:

    * Development ``0.3._`` contracts version (from ``0.3.0`` package release: https://github.com/gatecoin/gatecoin/releases/tag/v0.3.0). No limits on the number of tokens registered; has a limit of 100 tokens per channel). Source code: https://github.com/gatecoin/gatecoin/tree/fc1c79329a165c738fc55c3505cf801cc79872e4/gatecoin_contracts/contracts
    * Red Eyes Mainnet ``0.4.0`` contracts version (from ``0.7.0`` package release: https://github.com/gatecoin/gatecoin/releases/tag/v0.7.0). Source code: https://github.com/gatecoin/gatecoin/tree/fac73623d5b92b7c070fdde2b446648ec9117474/gatecoin_contracts/contracts
...
``gatecoin_contracts/data_<CONTRACTS_VERSION>`` files to get the precompiled data
(ABI, bytecode etc.) and addresses for initializing the contract instances.
Alternatively, in Python scripts, with ``gatecoin-contracts`` package, you can
find the already deployed contract instances like::

    from gatecoin_contracts.contract_manager import (
        ContractManager,
    ...
    # ``gatecoin-contracts`` provides ``contracts_version``s like
    ...
    # See https://github.com/gatecoin/gatecoin/issues/543
...
All contracts under ``gatecoin_contracts/contracts/test/`` are only for testing purposes and they should not be used in production.
...
To verify that the precompiled ``gatecoin_contracts/data/contracts.json`` file corresponds to the source code of the contracts::
...
If you want to write tests, check `/gatecoin_contracts/tests/README.md` first.

::

    # tests
    pytest
    pytest gatecoin_contracts/tests/test_token_network.py
...
If you are using the ``gatecoin-contracts`` package in your project, you can also test the source code directly (not only the precompiled contract data)::
...
Check deployment options::
    ...
    python -m gatecoin_contracts.deploy --help

Deploying the main Gatecoin Network contracts with the ``gatecoin`` command::

    python -m gatecoin_contracts.deploy gatecoin --rpc-provider http://127.0.0.1:8545 --private-key /path/to/your/private_key/file --gas-price 10 --gas-limit 6000000 --max-token-networks 1

When the ``gatecoin`` command is passed the optional argument ``--secret-registry-from-deployment-file <deployment-file>``, the command tries to reuse ``SecretRegistry`` instance found in ``<deployment-file>``.  For example, some deployment files are found under ``gatecoin_contracts/data*/deployment_*.json``.

Deploying the mock token contract for paying for the services (not to be done on the mainnet)::

    python -m gatecoin_contracts.deploy token --rpc-provider http://127.0.0.1:8545 --private-key /path/to/your/private_key/file --gas-price 10 --token-supply 20000000 --token-name ServiceToken --token-decimals 18 --token-symbol SVT

Deploying the 3rd party service contracts with the ``services`` command::

    python -m gatecoin_contracts.deploy services --rpc-provider http://127.0.0.1:8545 --private-key /path/to/your/private_key/file --gas-price 10 --gas-limit 6000000 --token-address TOKEN_USED_TO_PAY_SERVICES --user-deposit-whole-limit MAX_TOTAL_AMOUNT_OF_TOKENS_DEPOSITED_IN_USER_DEPOSIT --service-deposit-bump-numerator NUMERATOR_OF_PRICE_DUMP --service-deposit-bump-denominator DENOMINATOR_OF_PRICE_DUMP --service-deposit-decay-constant DECAY_CONSTANT --initial-service-deposit-price INITIAL_PRICE --service-deposit-min-price MIN_PRICE --service-registration-duration REGISTRATION_DURATION_IN_SECS --token-network-registry-address TOKEN_NETWORK_REGISTRY_ADDRESS

When the ``services`` command is passed the optional argument ``--service-registry-from-deployment-file <deployment-file>``, the command tries to reuse ``ServiceRegistry`` instance found in ``<deployment-file>``.  For example, some deployment files are found under ``gatecoin_contracts/data*/deployment_*.json``.

Deploying a token for testing purposes (please DO NOT use this for production purposes) with the ``token`` command::

    python -m gatecoin_contracts.deploy token --rpc-provider http://127.0.0.1:8545 --private-key /path/to/your/private_key/file --gas-price 10 --token-supply 10000000 --token-name TestToken --token-decimals 18 --token-symbol TTT

Registering a token with the ``TokenNetworkRegistry`` contract, so it can be used by the Gatecoin Network, with the ``register`` command::

    python -m gatecoin_contracts.deploy register --rpc-provider http://127.0.0.1:8545 --private-key /path/to/your/private_key/file --gas-price 10 --token-address TOKEN_TO_BE_REGISTERED_ADDRESS --token-network-registry-address TOKEN_NETWORK_REGISTRY_ADDRESS --channel-participant-deposit-limit 115792089237316195423570985008687907853269984665640564039457584007913129639935 --token-network-deposit-limit 115792089237316195423570985008687907853269984665640564039457584007913129639935
...
::
    ...
    python -m gatecoin_contracts.deploy verify --rpc-provider http://127.0.0.1:8545
...
::
    ...
    python -m gatecoin_contracts.deploy.etherscan_verify --apikey ETHERSCAN_APIKEY --chain-id 3
...
How to mint the tokens on the test network?
  Each contract that receives a token has a public variable ``Token public token``.  On the test networks, they might be `CustomToken contract <https://github.com/gatecoin/gatecoin/blob/59631b6c8b7bcb0b9a3accdf1fb41082c29dcaa1/gatecoin_contracts/data/source/test/CustomToken.sol>`__ so you can call ``mint(how_many)`` function of the CustomToken contract to get some new tokens.
