...
For each contract version ``x.y.z``, deliverables are located in ``gatecoin_contracts/data_x.y.z``.  For the version currently under development, deliverables are located in ``gatecoin_contracts/data``

Deliverables:

- smart contracts source code from ``gatecoin_contracts/data*/source``
- compiled contracts data from ``gatecoin_contracts/data*/contracts.json``
- deployment information from ``gatecoin_contracts/data*/deployment_*.json``, with Ethereum addresses, transaction data (transaction hash, block number, constructor arguments, gas cost)
- gas costs information from ``gatecoin_contracts/data*/gas.json``
- scripts for deployment & contract verification on Etherscan from ``gatecoin_contracts/deploy``
- source code tests from ``gatecoin_contracts/tests``

Package Release Process
^^^^^^^^^^^^^^^^^^^^^^^

When we want to release another version of the ``gatecoin-contracts`` package, we need to:
...
First, identify the last release in `the GitHub page <https://github.com/gatecoin/gatecoin/releases>`__ and find the corresponding tag in Git.
...
Now, we do not manually need to check if the compiled data is correct, because this check is included in our Continuous Integration testing. Any source code change forces us to recompile the contracts with ``make compile_contracts``, otherwise our CI tests will fail: https://github.com/gatecoin/gatecoin/blob/9fd2124eb648a629aee886f37ade5e502431371f/gatecoin_contracts/tests/test_contracts_compilation.py. These tests check:
...
* Edit the version located here:
  * ``CONTRACTS_VERSION`` https://github.com/gatecoin/gatecoin/blob/9fd2124eb648a629aee886f37ade5e502431371f/gatecoin_contracts/constants.py#L4
...
Instructions for deploying the contracts can be found at https://github.com/gatecoin/gatecoin#deployment-on-a-testnet, using the ``python -m gatecoin_contracts.deploy`` script.
...
* ``contracts_version``: https://github.com/gatecoin/gatecoin/blob/9fd2124eb648a629aee886f37ade5e502431371f/gatecoin_contracts/deploy/__main__.py#L503
* ``chain_id``: https://github.com/gatecoin/gatecoin/blob/9fd2124eb648a629aee886f37ade5e502431371f/gatecoin_contracts/deploy/__main__.py#L504
* deployment addresses & transaction data for each contract, checked against the chain we are running the script on

These checks are performed at the end of the deployment script. They can also be run independently with ``python -m gatecoin_contracts.deploy verify``, as described in the above link.

We also have CI tests that check whether the deployment data returned by the deployment script contains all necessary information, for all the contracts we need to deploy: https://github.com/gatecoin/gatecoin/blob/9fd2124eb648a629aee886f37ade5e502431371f/gatecoin_contracts/tests/test_deploy_script.py.
...
Etherscan verification is documented here: https://github.com/gatecoin/gatecoin#verification-with-etherscan.
...
``gatecoin_contracts`` package provides ``data/gas.json`` showing the amount of gas that each operation requires. A ``pytest`` execution updates the JSON file:
...
* The script changes the version located here:
  * ``VERSION`` https://github.com/gatecoin/gatecoin/blob/9fd2124eb648a629aee886f37ade5e502431371f/setup.py#L15
* We are currently at a ``0.*`` version. Our first ``major`` bump will be made when a stable, not-limited version will be released on the main net.
...
Use `make upload-pypi` to build the python packages and upload them to PyPI. You will need access to the project at https://pypi.org/project/gatecoin-contracts/.
