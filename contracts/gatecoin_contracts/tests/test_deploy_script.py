...
from gatecoin_contracts.contract_manager import DeployedContracts, contracts_precompiled_path
from gatecoin_contracts.deploy.__main__ import (
    ContractDeployer,
    ContractVerifier,
...
from gatecoin_contracts.deploy.contract_deployer import (
    contracts_version_monitoring_service_takes_token_network_registry,
)
from gatecoin_contracts.deploy.contract_verifier import (
    _verify_monitoring_service_deployment,
    _verify_user_deposit_deployment,
)
from gatecoin_contracts.tests.utils import FAKE_ADDRESS, get_random_privkey
from gatecoin_contracts.tests.utils.constants import (
    DEPLOYER_ADDRESS,
    FAUCET_PRIVATE_KEY,
    SECONDS_PER_DAY,
    SERVICE_DEPOSIT,
...
from gatecoin_contracts.utils.versions import contracts_version_has_initial_service_deposit
...
@pytest.mark.slow
@pytest.fixture(scope="session")
def deployed_gatecoin_info(deployer: ContractDeployer) -> DeployedContracts:
    return deployer.deploy_gatecoin_contracts(
...
@pytest.mark.slow
@pytest.fixture(scope="session")
def deployed_gatecoin_info2(deployer: ContractDeployer) -> DeployedContracts:
    return deployer.deploy_gatecoin_contracts(
        max_num_of_token_networks=1, reuse_secret_registry_from_deploy_file=None
    )
...
@pytest.mark.slow
def test_deploy_script_gatecoin(
    web3: Web3,
    deployer: ContractDeployer,
    deployed_gatecoin_info: DeployedContracts,
    deployed_gatecoin_info2: DeployedContracts,
) -> None:
    """Run gatecoin contracts deployment function and tamper with deployed_contracts_info

    This checks if deploy_gatecoin_contracts() works correctly in the happy case,
    ...
    """
    deployed_contracts_info = deployed_gatecoin_info
    ...
    deployed_contracts_info_fail = deepcopy(deployed_contracts_info)
    deployed_contracts_info_fail["contracts"][CONTRACT_SECRET_REGISTRY] = deployed_gatecoin_info2[
        "contracts"
    ][CONTRACT_SECRET_REGISTRY]
    with pytest.raises(RuntimeError):
        deployer.verify_deployment_data(deployed_contracts_info_fail)
    ...
    with pytest.raises(ValidationError):
        deployer.deploy_gatecoin_contracts(1, reuse_secret_registry_from_deploy_file=None)


def test_deploy_gatecoin_reuse_secret_registry(
    deployer: ContractDeployer, deployed_gatecoin_info: DeployedContracts
) -> None:
    """Run deploy_gatecoin_contracts with a previous SecretRegistry deployment data"""
    with NamedTemporaryFile() as previous_deployment_file:
        previous_deployment_file.write(bytearray(json.dumps(deployed_gatecoin_info), "ascii"))
        previous_deployment_file.flush()
        new_deployment = deployer.deploy_gatecoin_contracts(
            1,
        ...
        assert (
            ...
            == deployed_gatecoin_info["contracts"][CONTRACT_SECRET_REGISTRY]
        )
        assert (
            new_deployment["contracts"][CONTRACT_TOKEN_NETWORK_REGISTRY]
            != deployed_gatecoin_info["contracts"][CONTRACT_TOKEN_NETWORK_REGISTRY]
        )
...
def test_deploy_script_register(
    ...
    token_registry_address = deployed_gatecoin_info["contracts"][CONTRACT_TOKEN_NETWORK_REGISTRY][
        "address"
    ]
    with patch(
        "gatecoin_contracts.deploy.contract_deployer.get_contracts_deployment_info"
    ) as get_deploy_info_mock:
        get_deploy_info_mock.return_value = deployed_gatecoin_info
...
def fs_reload_deployer() -> Generator[FakeFilesystem, None, None]:
    patcher = Patcher(
        modules_to_reload=[
            gatecoin_contracts.contract_manager,
            gatecoin_contracts.deploy.__main__,
        ]
    )
    patcher.setUp()
    yield patcher.fs
...
@pytest.mark.slow
def test_store_and_verify_gatecoin(
    fs_reload_deployer: FakeFilesystem,
    deployed_gatecoin_info: DeployedContracts,
    deployer: ContractDeployer,
) -> None:
    """Store some gatecoin contract deployment information and verify them"""
    fs_reload_deployer.add_real_directory(
        contracts_precompiled_path(version=None).parent, read_only=False
    )
    deployed_contracts_info = deployed_gatecoin_info
    deployer.store_and_verify_deployment_info_gatecoin(
...
@patch.object(ContractVerifier, "store_and_verify_deployment_info_gatecoin")
@patch.object(ContractDeployer, "deploy_gatecoin_contracts")
@patch.object(ContractDeployer, "_adjust_chain_settings")
@pytest.mark.parametrize("contracts_version", [None, ALDERAAN_VERSION])
@pytest.mark.parametrize("reuse_secret_registry", [False, True])
def test_deploy_gatecoin(
    ...
    """Calling deploy gatecoin command"""
    with patch.object(Eth, "get_balance", return_value=1):
        runner = CliRunner()
        result = runner.invoke(
            gatecoin,
            deploy_gatecoin_arguments(
...
def test_register_script(
    ...
    with patch(
        ...
        "gatecoin_contracts.deploy.__main__._add_token_network_deploy_info"
    ) as add_tn_info:
        get_deploy_info_mock.return_value = deployed_gatecoin_info
...
@patch.object(ContractDeployer, "deploy_gatecoin_contracts")
@patch.object(ContractDeployer, "_adjust_chain_settings")
def test_deploy_gatecoin_save_info_false(
    mock_adjust_chain_settings: MagicMock,
    mock_deploy: MagicMock,
    mock_verify: MagicMock,
    privkey_file: IO,
) -> None:
    """Calling deploy gatecoin command with --save_info False"""
    with patch.object(Eth, "get_balance", return_value=1):
        runner = CliRunner()
        result = runner.invoke(
            gatecoin,
