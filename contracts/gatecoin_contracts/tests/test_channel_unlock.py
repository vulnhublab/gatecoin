...
from gatecoin_contracts.constants import TEST_SETTLE_TIMEOUT, ChannelEvent, ParticipantInfoIndex
...
from gatecoin_contracts.utils.events import check_channel_unlocked
...
def test_locks_order(
    ...
    (
        locksroot,
        unlocked_amount,
    ) = network_utils.functions.getHashAndUnlockedAmountPublic(
        pending_transfers_tree.packed_transfers
    ).call()
    assert locksroot == pending_transfers_tree.hash_of_packed_transfers
    assert unlocked_amount == 9
