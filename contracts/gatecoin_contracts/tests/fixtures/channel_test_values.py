from gatecoin_contracts.tests.utils import UINT256_MAX, ChannelValues, LockedAmounts

# We must cover the edge cases documented in
# https://github.com/gatecoin/gatecoin/issues/188
# The scope is to make sure that if someone uses an old balance proof, this cannot be used as
# an attack to steal tokens.
# For invalid balance proofs (created and signed with an unofficial Gatecoin client),
# we cannot determine and guarantee corectness. There are specific constraints that the
# Gatecoin client must enforce that guarantee correctness.
